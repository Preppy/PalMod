...
void CPalModDlg::HandleCopyToClipboard(bool fIncludeNonBinaryText /* = true */)
{
    if (m_fEnabled)
    {
        ...
        if (pSource)
        {
            ...
            uint8_t uCopyFlag1;
            // We use a char_t / uint8_t value to store the size.  This is compatible with all versions of palmod.
            ...
            // This table is so that older or newer versions of PalMod know the bpp of the copied colors.
            ...
            for (int iPalIndex = 0; iPalIndex < nWorkingAmt; iPalIndex++)
            {
                if (pSelIndex[iPalIndex] || fCopyAll)
                {
                    ...
                    switch (cbColor)
                    {
                        ...
                        {
                            // Using the original source alpha here would be better, but let's just stomp to full at this level
                            const uint16_t uCurrData = CurrGame->ConvCol16(CurrPal->GetColorAtIndex(iPalIndex), 0xffff);
                        ...
                        case 3:
                        {
                            const uint32_t uCurrData = CurrGame->ConvCol24(CurrPal->GetColorAtIndex(iPalIndex));
                        ...
                        case 4:
                        {
                            const uint32_t uCurrData = CurrGame->ConvCol32(CurrPal->GetColorAtIndex(iPalIndex), CurrPal->GetColorAtIndex(iPalIndex));
                        ...
                        switch (cbColor)
                        {
                            case 1:
                            {
                                ...
                                switch (curAlphaMode)
                                {
                                    ...
                                    default:
                                    {
                                        const uint32_t nThisColor = CurrPal->GetColorAtIndex(iPalIndex);
                            ...
                            case 4:
                            {
                                ...
                                switch (curAlphaMode)
                                {
                                    ...
                                    case AlphaMode::GameUsesVariableAlpha:
                                    default:
                                        uCurrData = CurrGame->ConvCol32(CurrPal->GetColorAtIndex(iPalIndex), CurrPal->GetColorAtIndex(iPalIndex));
                                        break;
                                }
                        ...
                        if (pSelIndex[iPalIndex] || fCopyAll)
                        {
                            switch (cbColor)
                            {
                                ...
                                case 3:
                                {
                                    ...
                                    uint32_t uCurrData = CurrGame->ConvCol24(CurrPal->GetColorAtIndex(iPalIndex));
    ...
    if (m_fEnabled && m_fOleInit)
    {
        ...
        if (CurrPal && CurrGame)
        {
            ...
            if (pSource)
            {
                ...
                OutputDebugString(strUnicodeData.GetString());
                // Display to the user
                SetStatusText(strUnicodeData.GetString());
            }
        }
    }
}
    ...
    if (hmem)
    {
        ...
        if (szTempStr[0] == '(')
        {
            if ((szTempStr[1] - k_nASCIICharacterOffset) < NUM_GAMES) //GameFlag
            {
                ...
                if (cbColorSize != 0)
                {
                    nPaletteCount = static_cast<uint16_t>((strlen(szTempStr) - 3) / (static_cast<uint16_t>(cbColorSize) * 2));
                }

                if (nPaletteCount <= CRegProc::GetMaxPalettePageSize())
                {
                    uint16_t nTerminalLocation = static_cast<uint16_t>(min(strlen(szTempStr), static_cast<uint32_t>(nPaletteCount * (static_cast<uint16_t>(cbColorSize) * 2)) + 3));
                    ...
                    else
                    {
                        ...
                        nTerminalLocation = static_cast<uint16_t>(min(strlen(szTempStr), static_cast<uint32_t>(nPaletteCount * (static_cast<uint32_t>(cbColorSize) * 2)) + 3));
...
void CPalModDlg::HandlePasteFromRGB(bool fAdvanceNext /* = false*/, bool fRefreshUI /* = true */)
{
    ...
    {
        // ideally we would use rom-source alpha, but for paste purposes let's just stomp to Set
        colPasteCol = CurrGame->ConvPal16(CurrGame->ConvCol16(colPasteCol, 0xffff));
    }
