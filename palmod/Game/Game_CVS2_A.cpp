...
CGame_CVS2_A::CGame_CVS2_A()
{
    ...
    pButtonLabel = const_cast<CHAR*>((CHAR*)DEF_BUTTONLABEL_CVS2);
...
void CGame_CVS2_A::DumpAllCharacters()
{
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < ARRAYSIZE(CVS2_CharacterOffsetArray); iUnitCtr++)
    {
        ...
        for (UINT16 iButtonIndex = 0; iButtonIndex < k_nCharacterColorCount; iButtonIndex++)
        {
            ...
            strOutput.Format("const sGame_PaletteDataset CVS2_A_%s_PALETTES_%s[] =\r\n{\r\n", CVS2_CharacterOffsetArray[iUnitCtr].pszCodeDesc, DEF_BUTTONLABEL_CVS2[iButtonIndex]);
            OutputDebugString(strOutput);

            strOutput.Format("    { \"Main Sprite\", 0x%07x, 0x%07x, %s },\r\n", 
        ...
        strOutput.Format("const sDescTreeNode CVS2_A_%s_COLLECTION[] =\r\n{\r\n", CVS2_CharacterOffsetArray[iUnitCtr].pszCodeDesc);
        OutputDebugString(strOutput);

        for (UINT16 iButtonIndex = 0; iButtonIndex < k_nCharacterColorCount; iButtonIndex++)
        {
            strOutput.Format("    { \"%s\", DESC_NODETYPE_TREE, (void*)CVS2_A_%s_PALETTES_%s, ARRAYSIZE(CVS2_A_%s_PALETTES_%s) },\r\n",
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < ARRAYSIZE(CVS2_CharacterOffsetArray); iUnitCtr++)
    {
        ...
        strOutput.Format("    { \"%s\", DESC_NODETYPE_TREE, (void*)CVS2_A_%s_COLLECTION, ARRAYSIZE(CVS2_A_%s_COLLECTION) },\r\n", CVS2_CharacterOffsetArray[iUnitCtr].pszCharacterName,
...
sDescTreeNode* CGame_CVS2_A::InitDescTree()
{
    ...
    sprintf(NewDescTree->szDesc, "%s", g_GameFriendlyName[CVS2_A]);
    ...
    strMsg.Format("CGame_CVS2_A::InitDescTree: Building desc tree for CVS2_A %s extras...\n", fHaveExtras ? "with" : "without");
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < CVS2_A_EXTRALOC)
        {
            ...
            sprintf(UnitNode->szDesc, "%s", CVS2_A_UNITS[iUnitCtr].szDesc);
    ...
    strMsg.Format("CGame_CVS2_A::InitDescTree: Loaded %u palettes for CVS2\n", nTotalPaletteCount);
    OutputDebugString(strMsg);
...
LPCSTR CGame_CVS2_A::GetDescriptionForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    if (nUnitId == CVS2_A_EXTRALOC)
    {
...
#if CVS2_A_DEBUG
        CString strMsg;
        strMsg.Format("CGame_CVS2_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
...
BOOL CGame_CVS2_A::SaveFile(CFile* SaveFile, UINT16 nUnitId)
{
    ...
    CString strMsg;
    strMsg.Format("CGame_CVS2_A::SaveFile: Saved 0x%x palettes to disk for %u units\n", nTotalPalettesSaved, nUnitAmt);
    OutputDebugString(strMsg);
