...
uint32_t CGame_BREAKERS_A::GetExtraLoc(uint32_t nUnitId)
{
...
sFileRule CGame_BREAKERS_A::GetRule(uint32_t nUnitId)
{
...
uint32_t CGame_BREAKERS_A::GetCollectionCountForUnit(uint32_t nUnitId)
{
...
uint32_t CGame_BREAKERS_A::GetNodeCountForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
...
LPCWSTR CGame_BREAKERS_A::GetDescriptionForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetDescriptionForCollection(BREAKERS_A_UNITS, BREAKERS_A_EXTRALOC, nUnitId, nCollectionId);
}

uint32_t CGame_BREAKERS_A::GetPaletteCountForUnit(uint32_t nUnitId)
{
...
const sGame_PaletteDataset* CGame_BREAKERS_A::GetPaletteSet(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetPaletteSet(BREAKERS_A_UNITS, nUnitId, nCollectionId);
}

const sDescTreeNode* CGame_BREAKERS_A::GetNodeFromPaletteId(uint32_t nUnitId, uint32_t nPaletteId, bool fReturnBasicNodesOnly)
{
...
const sGame_PaletteDataset* CGame_BREAKERS_A::GetSpecificPalette(uint32_t nUnitId, uint32_t nPaletteId)
{
...
void CGame_BREAKERS_A::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
    ...
    else // BREAKERS_A_EXTRALOC
    {
        ...
        stExtraDef* pCurrDef = &BREAKERS_A_EXTRA_CUSTOM[GetExtraLoc(nUnitId) + nPalId];
