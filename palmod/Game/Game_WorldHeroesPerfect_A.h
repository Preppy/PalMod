...
class CGame_WorldHeroesPerfect_S : public CGameClassByDir
{
private:
    static inline const sDirectoryLoadingData m_sFileLoadingData =
    {
    ...
    const std::vector<sCRC32ValueSet> m_rgCRC32Data =
    {
        { L"World Heroes Perfect (Steam)", L"p1.bin", 0x5BA1AAAA, -0x100000 },
    };
    ...
    const sCoreGameData m_sCoreGameData
    {
        ...
        WorldHeroesPerfect_A_UNITS,
        ARRAYSIZE(WorldHeroesPerfect_A_UNITS),
        L"WorldHeroesPerfectSE.txt",    // Extra filename
        152,                            // Count of palettes listed in the header
        0x101000,                       // Lowest known location used for palettes
    };

public:
    CGame_WorldHeroesPerfect_S(uint32_t nConfirmedROMSize) { InitializeGame(nConfirmedROMSize, m_sCoreGameData); };

    static sFileRule GetRule(uint32_t nRuleId) { return CGameClassByDir::GetRule(nRuleId, m_sFileLoadingData); };
};

class CGame_WorldHeroesPerfect_PS2 : public CGameClassByDir
{
private:
    static inline const sDirectoryLoadingData m_sFileLoadingData =
    {
        {
            { L"090prg.bin", 0x200000 },
        },
        FileReadType::Sequential,
    };
    ...
    const std::vector<sCRC32ValueSet> m_rgCRC32Data =
    {
        { L"World Heroes Perfect (PS2)", L"090prg.bin", 0x75DF25DA },
    };
...
public:
    CGame_WorldHeroesPerfect_PS2(uint32_t nConfirmedROMSize) { InitializeGame(nConfirmedROMSize, m_sCoreGameData); };
