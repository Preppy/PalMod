...
uint32_t CGame_P4AU_NESICA::uRuleCtr = 0;
...
sFileRule CGame_P4AU_NESICA::GetRule(uint32_t nUnitId)
{
    sFileRule NewFileRule;

    const uint32_t nAdjustedUnitId = (nUnitId & RULE_COUNTER_DEMASK);
    _snwprintf_s(NewFileRule.szFileName, ARRAYSIZE(NewFileRule.szFileName), _TRUNCATE, L"%s", PersonaCharacterData_NESICA.at(nAdjustedUnitId).pszFileName.c_str());
    NewFileRule.uUnitId = nUnitId;
    NewFileRule.uVerifyVar = PersonaCharacterData_NESICA.at(nAdjustedUnitId).nExpectedFileSize;
...
sFileRule CGame_P4AU_NESICA::GetNextRule()
{
    ...
    if (uRuleCtr >= PersonaCharacterData_NESICA.size())
    {
...
sDescTreeNode* CGame_P4AU_NESICA::InitDescTree()
{
    ...
    for (uint32_t  iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", PersonaCharacterData_NESICA.at(iUnitCtr).pszCharacter.c_str());
        ...
        for (uint32_t  iCollectionCtr = 0; iCollectionCtr < nUnitChildCount; iCollectionCtr++)
        {
            ...
            for (uint32_t  nNodeIndex = 0; nNodeIndex < nListedChildrenCount; nNodeIndex++)
            {
                ...
                _snwprintf_s(ChildNode->szDesc, ARRAYSIZE(ChildNode->szDesc), _TRUNCATE, L"%s", PersonaCharacterData_NESICA.at(iUnitCtr).paletteInfo->at(nNodeIndex).strName.c_str());
...
uint32_t CGame_P4AU_NESICA::GetCollectionCountForUnit(uint32_t nUnitId)
{
    // Just one palette set per character
    return static_cast<uint32_t>(PersonaPaletteNodes.size());
}

uint32_t CGame_P4AU_NESICA::GetNodeCountForCollection(uint32_t nUnitId, uint32_t  /*nCollectionId*/)
{
    return static_cast<uint32_t>(PersonaCharacterData_NESICA.at(nUnitId).paletteInfo->size());
}

uint32_t CGame_P4AU_NESICA::GetPaletteCountForUnit(uint32_t nUnitId)
{
    return static_cast<uint32_t>((PersonaCharacterData_NESICA.at(nUnitId).paletteInfo->size() * PersonaPaletteNodes.size()));
}

LPCWSTR CGame_P4AU_NESICA::GetDescriptionForCollection(uint32_t  /*nUnitId */, uint32_t nCollectionId)
{
    return PersonaPaletteNodes.at(nCollectionId);
}

void CGame_P4AU_NESICA::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
    // These palettes are all 0x400 long
    const int cbPaletteSizeOnDisc = 0x400;

    uint32_t nAdjustedPalId = (uint16_t)(nPalId % PersonaCharacterData_NESICA.at(nUnitId).paletteInfo->size());
    uint32_t nPaletteSet = (uint16_t)(nPalId / PersonaCharacterData_NESICA.at(nUnitId).paletteInfo->size());

    m_pszCurrentPaletteName = PersonaCharacterData_NESICA.at(nUnitId).paletteInfo->at(nAdjustedPalId).strName.c_str();
    m_nCurrentPaletteROMLocation = PersonaCharacterData_NESICA.at(nUnitId).nInitialLocation + (cbPaletteSizeOnDisc * nAdjustedPalId) + (0x20 * nAdjustedPalId);
    m_nCurrentPaletteSizeInColors = cbPaletteSizeOnDisc / m_nSizeOfColorsInBytes;

    if (nPaletteSet)
    {
        // Each palette is spaced XXX bytes apart
        m_nCurrentPaletteROMLocation += nPaletteSet * PersonaCharacterData_NESICA.at(nUnitId).nPaletteSetLength;
    }
}
...
BOOL CGame_P4AU_NESICA::UpdatePalImg(int Node01, int Node02, int Node03, int Node04)
{
    ...
    int nSrcStart = (int)(uint16_t)(NodeGet->uPalId % PersonaCharacterData_NESICA.at(NodeGet->uUnitId).paletteInfo->size());
    uint32_t nSrcAmt = static_cast<uint32_t>(pButtonLabelSet.size());
    uint16_t nNodeIncrement = (uint16_t)PersonaCharacterData_NESICA.at(NodeGet->uUnitId).paletteInfo->size();

    //Change the image id if we need to, using the single image index list used for each color
    uint32_t nImgUnitId = PersonaCharacterData_NESICA.at(NodeGet->uUnitId).paletteInfo->at(nSrcStart).nImageSet;
    int nTargetImgId = PersonaCharacterData_NESICA.at(NodeGet->uUnitId).paletteInfo->at(nSrcStart).nImageIndex;
...
BOOL CGame_P4AU_NESICA::LoadFile(CFile* LoadedFile, uint32_t nUnitNumber)
{
    BOOL fSuccess = TRUE;
    CString strInfo;

    strInfo.Format(L"CGame_P4AU_NESICA_DIR::LoadFile: Preparing to load data for unit number %u (character %s)\n", nUnitNumber, PersonaCharacterData_NESICA.at(nUnitNumber).pszCharacter.c_str());
    OutputDebugString(strInfo);

    strInfo.Format(L"\tCGame_P4AU_NESICA_DIR::LoadFile: Loaded palettes starting at location 0x%x\n", PersonaCharacterData_NESICA.at(nUnitNumber).nInitialLocation);
    OutputDebugString(strInfo);
...
BOOL CGame_P4AU_NESICA::SaveFile(CFile* SaveFile, uint32_t nUnitId)
{
    ...
    CString strMsg;
    strMsg.Format(L"CGame_P4AU_NESICA::SaveFile: Saved 0x%x palettes to disk for unit %u\n", nTotalPalettesSaved, nUnitId);
    OutputDebugString(strMsg);
