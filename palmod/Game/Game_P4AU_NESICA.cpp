...
CGame_P4AU_NESICA::CGame_P4AU_NESICA(UINT32 nConfirmedROMSize /* = -1 */)
{
    ...
    pButtonLabelSet = PersonaPaletteNodes_NESICA;
...
sFileRule CGame_P4AU_NESICA::GetRule(uint32_t nUnitId)
{
    ...
    _snwprintf_s(NewFileRule.szFileName, ARRAYSIZE(NewFileRule.szFileName), _TRUNCATE, L"%s", PersonaCharacterData_NESICA[nAdjustedUnitId].pszFileName.c_str());
    NewFileRule.uUnitId = nUnitId;
    NewFileRule.uVerifyVar = PersonaCharacterData_NESICA[nAdjustedUnitId].nExpectedFileSize;
...
sDescTreeNode* CGame_P4AU_NESICA::InitDescTree()
{
    ...
    for (uint32_t  iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", PersonaCharacterData_NESICA[iUnitCtr].pszCharacter.c_str());
        ...
        for (uint32_t  iCollectionCtr = 0; iCollectionCtr < nUnitChildCount; iCollectionCtr++)
        {
            ...
            for (uint32_t  nNodeIndex = 0; nNodeIndex < nListedChildrenCount; nNodeIndex++)
            {
                ...
                _snwprintf_s(ChildNode->szDesc, ARRAYSIZE(ChildNode->szDesc), _TRUNCATE, L"%s", PersonaCharacterData_NESICA[iUnitCtr].paletteInfo->at(nNodeIndex).strName.c_str());
    ...
    strMsg.Format(L"CGame_P4AU_NESICA_DIR::InitDescTree: Loaded %u palettes.\n", nTotalPaletteCount);
    OutputDebugString(strMsg);
...
uint32_t CGame_P4AU_NESICA::GetCollectionCountForUnit(uint32_t nUnitId)
{
    ...
    return static_cast<uint32_t>(PersonaPaletteNodes_NESICA.size());
}

uint32_t CGame_P4AU_NESICA::GetNodeCountForCollection(uint32_t nUnitId, uint32_t  /*nCollectionId*/)
{
    return static_cast<uint32_t>(PersonaCharacterData_NESICA[nUnitId].paletteInfo->size());
}

uint32_t CGame_P4AU_NESICA::GetPaletteCountForUnit(uint32_t nUnitId)
{
    return static_cast<uint32_t>((PersonaCharacterData_NESICA[nUnitId].paletteInfo->size() * PersonaPaletteNodes_NESICA.size()));
}

LPCWSTR CGame_P4AU_NESICA::GetDescriptionForCollection(uint32_t  /*nUnitId */, uint32_t nCollectionId)
{
    return PersonaPaletteNodes_NESICA[nCollectionId];
}
...
void CGame_P4AU_NESICA::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
    ...
    if (nPaletteSet)
    {
        ...
        m_nCurrentPaletteROMLocation += nPaletteSet * PersonaCharacterData_NESICA[nUnitId].nPaletteSetLength;
    }
}
...
BOOL CGame_P4AU_NESICA::UpdatePalImg(int Node01, int Node02, int Node03, int Node04)
{
    ...
    int nTargetImgId = PersonaCharacterData_NESICA[NodeGet->uUnitId].paletteInfo->at(nSrcStart).nImageIndex;
