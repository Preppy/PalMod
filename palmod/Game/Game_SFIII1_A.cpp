...
CGame_SFIII1_A::CGame_SFIII1_A(UINT32 nConfirmedROMSize, int nSF3ROMToLoad)
{
    ...
    CString strInfo;
    strInfo.Format(_T("CGame_SFIII1_A::CGame_SFIII1_A: Loaded SFIII1_A with %u Extras\n"), GetExtraCt(m_nExtraUnit));
...
sDescTreeNode* CGame_SFIII1_A::InitDescTree(int nROMPaletteSetToUse)
{
    ...
    _sntprintf_s(NewDescTree->szDesc, ARRAYSIZE(NewDescTree->szDesc), _TRUNCATE, _T("%s"), g_GameFriendlyName[SFIII1_A]);
    ...
    CString strMsg;
    strMsg.Format(_T("CGame_SFIII1_A::InitDescTree: Building desc tree for SFIII1_A ROM %u...\n"), m_nSelectedRom);
    ...
    strMsg.Format(_T("CGame_SFIII1_A::InitDescTree: Loaded %u palettes for SFIII1 ROM %u\n"), nTotalPaletteCount, m_nSelectedRom);
    OutputDebugString(strMsg);
...
sFileRule CGame_SFIII1_A::GetRule(UINT16 nUnitId)
{
    ...
    _sntprintf_s(NewFileRule.szFileName, ARRAYSIZE(NewFileRule.szFileName), _TRUNCATE, _T("50"));
...
UINT16 CGame_SFIII1_A::GetPaletteCountForUnit(UINT16 nUnitId)
{
    ...
    else
    {
        ...
        CString strMsg;
        strMsg.Format(_T("CGame_SFIII1_A::GetPaletteCountForUnit: %u palettes for unit %u which has %u collections.\n"), nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
...
BOOL CGame_SFIII1_A::LoadFile(CFile* LoadedFile, UINT16 nUnitId)
{
    for (UINT16 nUnitCtr = 0; nUnitCtr < nUnitAmt; nUnitCtr++)
    {
        UINT16 nPalAmt = GetPaletteCountForUnit(nUnitCtr);
...
BOOL CGame_SFIII1_A::SaveFile(CFile* SaveFile, UINT16 nUnitId)
{
...
void CGame_SFIII1_A::CreateDefPal(sDescNode* srcNode, UINT16 nSepId)
{
