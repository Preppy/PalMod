...
{
    CString strMessage;
    strMessage.Format(_T("CGame_RBFF1_A::CGame_RBFF1_A: Loading ROM...\n"));
...
sDescTreeNode* CGame_RBFF1_A::InitDescTree()
{
    ...
    _sntprintf_s(NewDescTree->szDesc, ARRAYSIZE(NewDescTree->szDesc), _TRUNCATE, _T("%s"), g_GameFriendlyName[RBFF1_A]);
    ...
    strMsg.Format(_T("CGame_RBFF1_A::InitDescTree: Building desc tree for RBFF1_A %s extras...\n"), fHaveExtras ? _T("with") : _T("without"));
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < RBFF1_A_EXTRALOC)
        {
            ...
            _sntprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, _T("%s"), RBFF1_A_UNITS[iUnitCtr].szDesc);
    ...
    strMsg.Format(_T("CGame_RBFF1_A::InitDescTree: Loaded %u palettes for RBFF1\n"), nTotalPaletteCount);
    OutputDebugString(strMsg);
...
sFileRule CGame_RBFF1_A::GetRule(UINT16 nUnitId)
{
    ...
    _sntprintf_s(NewFileRule.szFileName, ARRAYSIZE(NewFileRule.szFileName), _TRUNCATE, _T("095-p1.bin"));
...
UINT32 CGame_RBFF1_A::GetKnownCRC32DatasetsForGame(const sCRC32ValueSet** ppKnownROMSet, bool* pfNeedToValidateCRCs)
{
    ...
    {
        { _T("Real Bout Fatal Fury (NEOGEO)"), _T("095-p1.bin"), 0, 0 },
    };
...
#if RBFF1_A_DEBUG
        CString strMsg;
        strMsg.Format(_T("CGame_RBFF1_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n"), nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
