...
{
    CString strMessage;
    strMessage.Format(_T("CGame_KOF03_A::CGame_KOF03_A: Loading ROM...\n"));
    ...
    m_nSafeCountForThisRom = GetExtraCt(m_nExtraUnit) + 904;
    ...
    nImgUnitAmt = KOF03_A_NUM_IMG_UNITS;
    m_prgGameImageSet = KOF03_A_IMG_UNITS;
...
int CGame_KOF03_A::GetExtraCt(UINT16 nUnitId, BOOL bCountVisibleOnly)
{
    ...
    {
        int nDefCtr = 0;
        memset(rgExtraCountAll, 0, ((KOF03_A_NUMUNIT + 1) * sizeof(int)));

        stExtraDef* pCurrDef = GetExtraDefForKOF03(0);
        ...
        while (pCurrDef->uUnitN != INVALID_UNIT_VALUE)
        {
            ...
            nDefCtr++;
            pCurrDef = GetExtraDefForKOF03(nDefCtr);
        }
    }
...
int CGame_KOF03_A::GetExtraLoc(UINT16 nUnitId)
{
    if (rgExtraLoc[0] == -1)
    {
        ...
        memset(rgExtraLoc, 0, (KOF03_A_NUMUNIT + 1) * sizeof(int));
...
sDescTreeNode* CGame_KOF03_A::InitDescTree()
{
    ...
    _sntprintf_s(NewDescTree->szDesc, ARRAYSIZE(NewDescTree->szDesc), _TRUNCATE, _T("%s"), g_GameFriendlyName[KOF03_A]);
    ...
    CString strMsg;
    bool fHaveExtras = (GetExtraCt(KOF03_A_EXTRALOC) > 0);
    strMsg.Format(_T("CGame_KOF03_A::InitDescTree: Building desc tree for KOF03_A %s extras...\n"), fHaveExtras ? _T("with") : _T("without"));
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < KOF03_A_EXTRALOC)
        {
            //Set each description
            _sntprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, _T("%s"), KOF03_A_UNITS[iUnitCtr].szDesc);
        ...
        if (bUseExtra)
        {
            ...
            CollectionNode = &((sDescTreeNode*)UnitNode->ChildNodes)[(KOF03_A_EXTRALOC > iUnitCtr) ? (nUnitChildCount - 1) : 0]; //Extra node
            ...
            for (UINT16 nExtraCtr = 0; nExtraCtr < nExtraCt; nExtraCtr++)
            {
                ...
                stExtraDef* pCurrDef = GetExtraDefForKOF03(nExtraPos + nCurrExtra);

                while (pCurrDef->isInvisible)
                {
                    nCurrExtra++;

                    pCurrDef = GetExtraDefForKOF03(nExtraPos + nCurrExtra);
                }

                _sntprintf_s(ChildNode->szDesc, ARRAYSIZE(ChildNode->szDesc), _TRUNCATE, pCurrDef->szDesc);

                ChildNode->uUnitId = iUnitCtr;
                ChildNode->uPalId = (((KOF03_A_EXTRALOC > iUnitCtr) ? 1 : 0) * nUnitChildCount * 2) + nCurrExtra;
    ...
    strMsg.Format(_T("CGame_KOF03_A::InitDescTree: Loaded %u palettes for KOF03\n"), nTotalPaletteCount);
    OutputDebugString(strMsg);

    m_nTotalPaletteCountForKOF03 = nTotalPaletteCount;
...
UINT16 CGame_KOF03_A::GetCollectionCountForUnit(UINT16 nUnitId)
{
    ...
    {
        return KOF03_A_UNITS[nUnitId].uChildAmt;
    }
}
...
UINT16 CGame_KOF03_A::GetNodeCountForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    {
        const sDescTreeNode* pCollectionNode = (const sDescTreeNode*)(KOF03_A_UNITS[nUnitId].ChildNodes);
...
LPCTSTR CGame_KOF03_A::GetDescriptionForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    {
        const sDescTreeNode* pCollection = (const sDescTreeNode*)KOF03_A_UNITS[nUnitId].ChildNodes;
...
UINT16 CGame_KOF03_A::GetPaletteCountForUnit(UINT16 nUnitId)
{
    ...
    else
    {
        ...
        const sDescTreeNode* pCompleteROMTree = KOF03_A_UNITS;
...
#if KOF03_A_DEBUG
        CString strMsg;
        strMsg.Format(_T("CGame_KOF03_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n"), nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
...
const sGame_PaletteDataset* CGame_KOF03_A::GetPaletteSet(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    const sDescTreeNode* pCurrentSet = (const sDescTreeNode*)KOF03_A_UNITS[nUnitId].ChildNodes;
...
const sDescTreeNode* CGame_KOF03_A::GetNodeFromPaletteId(UINT16 nUnitId, UINT16 nPaletteId, bool fReturnBasicNodesOnly)
{
    ...
    for (UINT16 nCollectionIndex = 0; nCollectionIndex < nTotalCollections; nCollectionIndex++)
    {
        ...
        {
            const sDescTreeNode* pCollectionNodeToCheck = (const sDescTreeNode*)(KOF03_A_UNITS[nUnitId].ChildNodes);
...
BOOL CGame_KOF03_A::LoadFile(CFile* LoadedFile, UINT16 nUnitId)
{
...
void CGame_KOF03_A::CreateDefPal(sDescNode* srcNode, UINT16 nSepId)
{
