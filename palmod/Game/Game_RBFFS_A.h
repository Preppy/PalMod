...
class CGame_RBFFS_PS2 : public CGameClassByDir
{
private:
    static inline const sDirectoryLoadingData m_sFileLoadingData =
    {
        {
            { L"223prg.bin", 0x500000 },
        },
        FileReadType::Sequential,
    };
    ...
    const std::vector<sCRC32ValueSet> m_rgCRC32Data =
    {
        { L"Real Bout Fatal Fury Special (PS2)", L"223prg.bin", 0x909949F2, 0 },
    };
...
public:
    CGame_RBFFS_PS2(uint32_t nConfirmedROMSize) { InitializeGame(nConfirmedROMSize, m_sCoreGameData); };
