...
sDescTreeNode* CGame_SAMSHO4_A::InitDescTree()
{
    ...
    strMsg.Format(L"CGame_SAMSHO4_A::InitDescTree: Building desc tree for SAMSHO4_A %s extras...\n", fHaveExtras ? L"with" : L"without");
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < SAMSHO4_A_EXTRALOC)
        {
            ...
            _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", SAMSHO4_A_UNITS[iUnitCtr].szDesc);
    ...
    strMsg.Format(L"CGame_SAMSHO4_A::InitDescTree: Loaded %u palettes for SAMSHO4\n", nTotalPaletteCount);
    OutputDebugString(strMsg);
...
#if SAMSHO4_A_DEBUG
        CString strMsg;
        strMsg.Format(L"CGame_SAMSHO4_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
