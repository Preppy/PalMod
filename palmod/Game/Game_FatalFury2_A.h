...
public:
    ...
    // Handle the size difference here
    static void SetSpecialRuleForFileName(std::wstring strFileName);

    static sFileRule GetRule(uint32_t nRuleId);
};

class CGame_FatalFury2_PS2 : public CGameClassByDir
{
private:
    static inline const sDirectoryLoadingData m_sFileLoadingData =
    {
        {
            { L"047prg.bin", 0x200000 },
        },
        FileReadType::Sequential,
    };
    ...
    const std::vector<sCRC32ValueSet> m_rgCRC32Data =
    {
        { L"Fatal Fury 2 (PS2)", L"047prg.bin", 0xF789E875, 0 },
    };
...
public:
    CGame_FatalFury2_PS2(uint32_t nConfirmedROMSize) { InitializeGame(nConfirmedROMSize, m_sCoreGameData); };
