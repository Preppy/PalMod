...
sDescTreeNode* CGame_MMPR_SNES::InitDescTree()
{
    ...
    strMsg.Format(L"CGame_MMPR_SNES::InitDescTree: Building desc tree for MMPR_SNES %s extras...\n", fHaveExtras ? L"with" : L"without");
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < MMPR_SNES_EXTRALOC)
        {
            ...
            _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", MMPR_SNES_UNITS[iUnitCtr].szDesc);
    ...
    strMsg.Format(L"CGame_MMPR_SNES::InitDescTree: Loaded %u palettes for MMPR\n", nTotalPaletteCount);
    OutputDebugString(strMsg);
...
#if MMPR_SNES_DEBUG
        CString strMsg;
        strMsg.Format(L"CGame_MMPR_SNES::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
