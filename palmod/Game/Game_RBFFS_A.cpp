...
CGame_RBFFS_A::CGame_RBFFS_A(UINT32 nConfirmedROMSize)
{
    ...
    nImgUnitAmt = RBFFS_A_NUM_IMG_UNITS;
...
sDescTreeNode* CGame_RBFFS_A::InitDescTree()
{
    ...
    _stprintf(NewDescTree->szDesc, _T("%s"), g_GameFriendlyName[RBFFS_A]);
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < RBFFS_A_EXTRALOC)
        {
            ...
            _stprintf(UnitNode->szDesc, _T("%s"), RBFFS_A_UNITS[iUnitCtr].szDesc);
...
sFileRule CGame_RBFFS_A::GetRule(UINT16 nUnitId)
{
    ...
    _stprintf_s(NewFileRule.szFileName, MAX_FILENAME_LENGTH, _T("223-p1.p1"));
...
LPCTSTR CGame_RBFFS_A::GetDescriptionForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    if (nUnitId == RBFFS_A_EXTRALOC)
    {
...
BOOL CGame_RBFFS_A::LoadFile(CFile* LoadedFile, UINT16 nUnitId)
{
...
void CGame_RBFFS_A::CreateDefPal(sDescNode* srcNode, UINT16 nSepId)
{
...
void CGame_RBFFS_A::UpdatePalData()
{
