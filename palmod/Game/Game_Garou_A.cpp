...
sDescTreeNode* CGame_Garou_A::InitDescTree()
{
    ...
    _sntprintf_s(NewDescTree->szDesc, ARRAYSIZE(NewDescTree->szDesc), _TRUNCATE, _T("%s"), g_GameFriendlyName[Garou_A]);
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < Garou_A_EXTRALOC)
        {
            ...
            _sntprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, _T("%s"), Garou_A_UNITS[iUnitCtr].szDesc);
...
sFileRule CGame_Garou_A::GetRule(UINT16 nUnitId)
{
    ...
    _sntprintf_s(NewFileRule.szFileName, ARRAYSIZE(NewFileRule.szFileName), _TRUNCATE, _T("kf.neo-sma"));
