...
sDescTreeNode* CGame_NGBC_A::InitDescTree()
{
    ...
    strMsg.Format(L"CGame_NGBC_A::InitDescTree: Building desc tree for NGBC_A %s extras...\n", fHaveExtras ? L"with" : L"without");
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < NGBC_A_EXTRALOC)
        {
            ...
            _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", NGBC_A_UNITS[iUnitCtr].szDesc);
    ...
    strMsg.Format(L"CGame_NGBC_A::InitDescTree: Loaded %u palettes for NGBC\n", nTotalPaletteCount);
    OutputDebugString(strMsg);
...
#if NGBC_A_DEBUG
        CString strMsg;
        strMsg.Format(L"CGame_NGBC_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
