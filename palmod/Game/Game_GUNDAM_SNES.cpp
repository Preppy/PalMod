...
{
    CString strMessage;
    strMessage.Format(L"CGame_GUNDAM_SNES::CGame_GUNDAM_SNES: Loading ROM...\n");
...
sDescTreeNode* CGame_GUNDAM_SNES::InitDescTree()
{
    ...
    strMsg.Format(L"CGame_GUNDAM_SNES::InitDescTree: Building desc tree for GUNDAM_SNES %s extras...\n", fHaveExtras ? L"with" : L"without");
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < GUNDAM_SNES_EXTRALOC)
        {
            ...
            _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", GUNDAM_SNES_UNITS[iUnitCtr].szDesc);
    ...
    strMsg.Format(L"CGame_GUNDAM_SNES::InitDescTree: Loaded %u palettes for GUNDAM\n", nTotalPaletteCount);
    OutputDebugString(strMsg);
...
#if GUNDAM_SNES_DEBUG
        CString strMsg;
        strMsg.Format(L"CGame_GUNDAM_SNES::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
