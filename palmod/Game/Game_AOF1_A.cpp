...
{
    CString strMessage;
    strMessage.Format(L"CGame_AOF1_A::CGame_AOF1_A: Loading ROM...\n");
...
sDescTreeNode* CGame_AOF1_A::InitDescTree()
{
    ...
    nExtraUnitLocation = AOF1_A_EXTRALOC;
    LoadExtraFileForGame(EXTRA_FILENAME_AOF1_A, AOF1_A_EXTRA, &AOF1_A_EXTRA_CUSTOM, AOF1_A_EXTRALOC, m_nConfirmedROMSize);
    fHaveExtras = GetExtraCt(AOF1_A_EXTRALOC);
    nUnitCt = AOF1_A_NUMUNIT + (fHaveExtras ? 1 : 0);

    sDescTreeNode* NewDescTree = new sDescTreeNode;

    //Create the main character tree
    _sntprintf_s(NewDescTree->szDesc, ARRAYSIZE(NewDescTree->szDesc), _TRUNCATE, L"%s", g_GameFriendlyName[AOF1_A]);
    ...
    CString strMsg;
    strMsg.Format(L"CGame_AOF1_A::InitDescTree: Building desc tree for AOF1_A %s extras...\n", fHaveExtras ? L"with" : L"without");
    ...
    strMsg.Format(L"CGame_AOF1_A::InitDescTree: Loaded %u palettes for AOF1\n", nTotalPaletteCount);
    OutputDebugString(strMsg);

    m_nTotalPaletteCountForAOF1 = nTotalPaletteCount;
...
#if AOF1_A_DEBUG
        CString strMsg;
        strMsg.Format(L"CGame_AOF1_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
