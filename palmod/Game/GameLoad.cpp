...
void CGameLoad::CrosscopyGame_Steam(CGameClass* CurrGame)
{
    ...
    strQuestion += L"If you have not already saved/patched your data so that it can be copied, please Cancel now and Patch.\n\n"
                   L"Press OK when you are ready.  You will then select the Steam ROM to copy to and the palettes will be copied over.";
                            ...
                            for (uint32_t nTargetUnit = 0; nTargetUnit < rgPortMap.size(); nTargetUnit++)
                            {
                                if (_wcsicmp(rgPortMap[nTargetUnit].strFileName.c_str(), CurrRule.szFileName) == 0)
                                {
                                        ...
                                        SourceFile.Open(strSourceFile, CFile::modeRead | CFile::typeBinary))
                                    {
                                        ...
                                        if (eCurGame == MVC2_S)
                                        {
                                            SourceFile.Seek(rgPortMap[nTargetUnit].nSteamOffset, CFile::begin);
                                        }
                                        else
                                        {
