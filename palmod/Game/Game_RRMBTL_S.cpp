...
int CGame_RRMBTL_S::PostSetPal(uint32_t nUnitId, uint32_t nPalId)
{
    CString strMessage;
    strMessage.Format(L"CGame_RRMBTL_S::PostSetPal : Updating left/right partner for unit %u palette %u.\n", nUnitId, nPalId);
    ...
    if (wcscmp(m_sCoreGameData.srgLoadingData.at(nUnitId).strCharacter.c_str(), L"Rentaro Satomi") == 0)
    {
