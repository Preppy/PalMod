...
{
    CString strMessage;
    strMessage.Format(L"CGame_SAVAGEREIGN_A::CGame_SAVAGEREIGN_A: Loading ROM...\n");
...
int CGame_SAVAGEREIGN_A::GetExtraCt(UINT16 nUnitId, BOOL bCountVisibleOnly)
{
    ...
    {
        int nDefCtr = 0;
        memset(rgExtraCountAll, 0, ((SAVAGEREIGN_A_NUMUNIT + 1) * sizeof(int)));

        stExtraDef* pCurrDef = GetExtraDefForSAVAGEREIGN(0);
        ...
        while (pCurrDef->uUnitN != INVALID_UNIT_VALUE)
        {
            ...
            nDefCtr++;
            pCurrDef = GetExtraDefForSAVAGEREIGN(nDefCtr);
        }
    }
...
int CGame_SAVAGEREIGN_A::GetExtraLoc(UINT16 nUnitId)
{
    if (rgExtraLoc[0] == -1)
    {
        ...
        memset(rgExtraLoc, 0, (SAVAGEREIGN_A_NUMUNIT + 1) * sizeof(int));
...
sDescTreeNode* CGame_SAVAGEREIGN_A::InitDescTree()
{
    ...
    CString strMsg;
    bool fHaveExtras = (GetExtraCt(SAVAGEREIGN_A_EXTRALOC) > 0);
    strMsg.Format(L"CGame_SAVAGEREIGN_A::InitDescTree: Building desc tree for SAVAGEREIGN_A %s extras...\n", fHaveExtras ? L"with" : L"without");
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < SAVAGEREIGN_A_EXTRALOC)
        {
            //Set each description
            _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", SAVAGEREIGN_A_UNITS[iUnitCtr].szDesc);
            UnitNode->ChildNodes = new sDescTreeNode[nUnitChildCount];
            //All children have collection trees
            UnitNode->uChildType = DESC_NODETYPE_TREE;
            UnitNode->uChildAmt = nUnitChildCount;

#if SAVAGEREIGN_A_DEBUG
        ...
        if (bUseExtra)
        {
            ...
            CollectionNode = &((sDescTreeNode*)UnitNode->ChildNodes)[(SAVAGEREIGN_A_EXTRALOC > iUnitCtr) ? (nUnitChildCount - 1) : 0]; //Extra node
            ...
            for (UINT16 nExtraCtr = 0; nExtraCtr < nExtraCt; nExtraCtr++)
            {
                ...
                stExtraDef* pCurrDef = GetExtraDefForSAVAGEREIGN(nExtraPos + nCurrExtra);

                while (pCurrDef->isInvisible)
                {
                    nCurrExtra++;

                    pCurrDef = GetExtraDefForSAVAGEREIGN(nExtraPos + nCurrExtra);
                }

                _snwprintf_s(ChildNode->szDesc, ARRAYSIZE(ChildNode->szDesc), _TRUNCATE, pCurrDef->szDesc);

                ChildNode->uUnitId = iUnitCtr;
                ChildNode->uPalId = (((SAVAGEREIGN_A_EXTRALOC > iUnitCtr) ? 1 : 0) * nUnitChildCount * 2) + nCurrExtra;
    ...
    strMsg.Format(L"CGame_SAVAGEREIGN_A::InitDescTree: Loaded %u palettes for SAVAGEREIGN\n", nTotalPaletteCount);
    OutputDebugString(strMsg);

    m_nTotalPaletteCountForSAVAGEREIGN = nTotalPaletteCount;
...
UINT16 CGame_SAVAGEREIGN_A::GetCollectionCountForUnit(UINT16 nUnitId)
{
    ...
    {
        return SAVAGEREIGN_A_UNITS[nUnitId].uChildAmt;
    }
}
...
UINT16 CGame_SAVAGEREIGN_A::GetNodeCountForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    {
        const sDescTreeNode* pCollectionNode = (const sDescTreeNode*)(SAVAGEREIGN_A_UNITS[nUnitId].ChildNodes);
...
LPCWSTR CGame_SAVAGEREIGN_A::GetDescriptionForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    {
        const sDescTreeNode* pCollection = (const sDescTreeNode*)SAVAGEREIGN_A_UNITS[nUnitId].ChildNodes;
...
UINT16 CGame_SAVAGEREIGN_A::GetPaletteCountForUnit(UINT16 nUnitId)
{
    ...
    else
    {
        ...
        const sDescTreeNode* pCompleteROMTree = SAVAGEREIGN_A_UNITS;
...
#if SAVAGEREIGN_A_DEBUG
        CString strMsg;
        strMsg.Format(L"CGame_SAVAGEREIGN_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
...
const sGame_PaletteDataset* CGame_SAVAGEREIGN_A::GetPaletteSet(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    const sDescTreeNode* pCurrentSet = (const sDescTreeNode*)SAVAGEREIGN_A_UNITS[nUnitId].ChildNodes;
...
const sDescTreeNode* CGame_SAVAGEREIGN_A::GetNodeFromPaletteId(UINT16 nUnitId, UINT16 nPaletteId, bool fReturnBasicNodesOnly)
{
    ...
    for (UINT16 nCollectionIndex = 0; nCollectionIndex < nTotalCollections; nCollectionIndex++)
    {
        ...
        {
            const sDescTreeNode* pCollectionNodeToCheck = (const sDescTreeNode*)(SAVAGEREIGN_A_UNITS[nUnitId].ChildNodes);
