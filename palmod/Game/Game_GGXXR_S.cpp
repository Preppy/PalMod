...
uint32_t CGame_GGXXR_S::uRuleCtr = 0;

CDescTree CGame_GGXXR_S::MainDescTree = nullptr;

#define GGXXR_S_DEBUG DEFAULT_GAME_DEBUG_STATE

CGame_GGXXR_S::CGame_GGXXR_S(uint32_t nConfirmedROMSize /* = -1 */)
{
...
sFileRule CGame_GGXXR_S::GetRule(uint32_t nUnitId)
{
...
sDescTreeNode* CGame_GGXXR_S::InitDescTree()
{
    ...
    uint32_t nUnitCt = ARRAYSIZE(GGXXR_S_CharacterData);
...
uint32_t CGame_GGXXR_S::GetCollectionCountForUnit(uint32_t nUnitId)
{
    // One core set per character, plus optional extras
    return (GGXXR_S_CharacterData[nUnitId].prgExtraPalettes.empty()) ? 1 : 2;
}

uint32_t CGame_GGXXR_S::GetNodeCountForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
    if (nCollectionId == 0)
    {
        return static_cast<uint32_t>(GGXXR_S_CharacterData[nUnitId].ppszPaletteList.size());
    }
    else
    {
        return static_cast<uint32_t>(GGXXR_S_CharacterData[nUnitId].prgExtraPalettes.size());
    }
}

uint32_t CGame_GGXXR_S::GetPaletteCountForUnit(uint32_t nUnitId)
{
    return static_cast<uint32_t>(GGXXR_S_CharacterData[nUnitId].ppszPaletteList.size() + GGXXR_S_CharacterData[nUnitId].prgExtraPalettes.size());
}

LPCWSTR CGame_GGXXR_S::GetDescriptionForCollection(uint32_t /*nUnitId */, uint32_t nCollectionId)
{
    if (nCollectionId == 0)
    {
...
void CGame_GGXXR_S::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
    ...
    {
        uint32_t nAdjustedPaletteId = nPalId - static_cast<uint32_t>(GGXXR_S_CharacterData[nUnitId].ppszPaletteList.size());
...
BOOL CGame_GGXXR_S::UpdatePalImg(int Node01, int Node02, int Node03, int Node04)
{
    ...
    if (NodeGet->uPalId < static_cast<uint32_t>(GGXXR_S_CharacterData[NodeGet->uUnitId].ppszPaletteList.size()))
    {
        // core palettes
        nSrcStart = 0;
        nSrcAmt = static_cast<uint32_t>(GGXXR_S_CharacterData[NodeGet->uUnitId].ppszPaletteList.size());
    ...
    {
        // effects palettes
        uint32_t nPalIdInNode = NodeGet->uPalId - static_cast<uint32_t>(GGXXR_S_CharacterData[NodeGet->uUnitId].ppszPaletteList.size());
...
BOOL CGame_GGXXR_S::LoadFile(CFile* LoadedFile, uint32_t nUnitNumber)
{
    BOOL fSuccess = TRUE;
    CString strInfo;
...
BOOL CGame_GGXXR_S::SaveFile(CFile* SaveFile, uint32_t nUnitId)
{
