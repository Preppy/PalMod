...
UINT16 CGame_GGXXR_S::uRuleCtr = 0;
...
sFileRule CGame_GGXXR_S::GetRule(UINT16 nUnitId)
{
...
UINT16 CGame_GGXXR_S::GetCollectionCountForUnit(UINT16 nUnitId)
{
    // One core set per character, plus optional extras
    return (GGXXR_S_CharacterData[nUnitId].prgExtraPalettes == nullptr) ? 1 : 2;
}

UINT16 CGame_GGXXR_S::GetNodeCountForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    if (nCollectionId == 0)
    {
        return GGXXR_S_CharacterData[nUnitId].nPaletteListSize;
    }
    else
    {
        return GGXXR_S_CharacterData[nUnitId].nCountExtras;
    }
}

UINT16 CGame_GGXXR_S::GetPaletteCountForUnit(UINT16 nUnitId)
{
    return GGXXR_S_CharacterData[nUnitId].nPaletteListSize + GGXXR_S_CharacterData[nUnitId].nCountExtras;
}

LPCWSTR CGame_GGXXR_S::GetDescriptionForCollection(UINT16 /*nUnitId */, UINT16 nCollectionId)
{
    if (nCollectionId == 0)
    {
...
void CGame_GGXXR_S::LoadSpecificPaletteData(UINT16 nUnitId, UINT16 nPalId)
{
    if (nPalId < GGXXR_S_CharacterData[nUnitId].nPaletteListSize)
    {
    ...
    {
        UINT16 nAdjustedPaletteId = nPalId - GGXXR_S_CharacterData[nUnitId].nPaletteListSize;
...
BOOL CGame_GGXXR_S::UpdatePalImg(int Node01, int Node02, int Node03, int Node04)
{
    ...
    if (NodeGet->uPalId < GGXXR_S_CharacterData[NodeGet->uUnitId].nPaletteListSize)
    {
        // core palettes
        nSrcStart = 0;
        nSrcAmt = GGXXR_S_CharacterData[NodeGet->uUnitId].nPaletteListSize;
        pButtonLabelSet = GGXXR_S_CharacterData[NodeGet->uUnitId].ppszPaletteList;
        m_nNumberOfColorOptions = GGXXR_S_CharacterData[NodeGet->uUnitId].nPaletteListSize;
        nImgUnitId = GGXXR_S_CharacterData[NodeGet->uUnitId].nSpriteIndex;
    }
    else
    {
        // effects palettes
        UINT16 nPalIdInNode = NodeGet->uPalId - GGXXR_S_CharacterData[NodeGet->uUnitId].nPaletteListSize;
...
BOOL CGame_GGXXR_S::LoadFile(CFile* LoadedFile, UINT16 nUnitNumber)
{
    BOOL fSuccess = TRUE;
    CString strInfo;
...
BOOL CGame_GGXXR_S::SaveFile(CFile* SaveFile, UINT16 nUnitId)
{
