...
CGame_BASARA_P::CGame_BASARA_P(uint32_t nConfirmedROMSize)
{
    ...
    m_prgGameImageSet.clear();  // BASARA_P_IMG_UNITS -- this is the vector of images present in imgdat once that happens
...
void CGame_BASARA_P::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
    ...
    else // BASARA_P_EXTRALOC
    {
        ...
        stExtraDef* pCurrDef = (stExtraDef*)&BASARA_P_EXTRA_CUSTOM[GetExtraLoc(nUnitId) + nPalId];
