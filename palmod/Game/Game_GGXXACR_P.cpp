...
uint32_t CGame_GGXXACR_P::uRuleCtr = 0;
...
sFileRule CGame_GGXXACR_P::GetRule(uint32_t nUnitId)
{
...
uint32_t CGame_GGXXACR_P::GetCollectionCountForUnit(uint32_t nUnitId)
{
    // One core set per character, plus optional extras
    return (GGXXACR_P_CharacterData[nUnitId].prgExtraPalettes.empty()) ? 1 : 2;
}

uint32_t CGame_GGXXACR_P::GetNodeCountForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
    if (nCollectionId == 0)
    {
        return static_cast<uint32_t>(GGXXACR_P_CharacterData[nUnitId].ppszPaletteList.size());
    }
    else
    {
        return static_cast<uint32_t>(GGXXACR_P_CharacterData[nUnitId].prgExtraPalettes.size());
    }
}

uint32_t CGame_GGXXACR_P::GetPaletteCountForUnit(uint32_t nUnitId)
{
    return static_cast<uint32_t>(GGXXACR_P_CharacterData[nUnitId].ppszPaletteList.size() + GGXXACR_P_CharacterData[nUnitId].prgExtraPalettes.size());
}

LPCWSTR CGame_GGXXACR_P::GetDescriptionForCollection(uint32_t /*nUnitId */, uint32_t nCollectionId )
{
    if (nCollectionId == 0)
    {
...
void CGame_GGXXACR_P::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
    if (nPalId < static_cast<uint32_t>(GGXXACR_P_CharacterData[nUnitId].ppszPaletteList.size()))
    {
    ...
    {
        uint32_t nAdjustedPaletteId = nPalId - static_cast<uint32_t>(GGXXACR_P_CharacterData[nUnitId].ppszPaletteList.size());
...
BOOL CGame_GGXXACR_P::UpdatePalImg(int Node01, int Node02, int Node03, int Node04)
{
    ...
    if (NodeGet->uPalId < GGXXACR_P_CharacterData[NodeGet->uUnitId].ppszPaletteList.size())
    {
        // core palettes
        nSrcStart = 0;
        nSrcAmt = static_cast<uint32_t>(GGXXACR_P_CharacterData[NodeGet->uUnitId].ppszPaletteList.size());
    ...
    {
        // effects palettes
        uint32_t nPalIdInNode = NodeGet->uPalId - static_cast<uint32_t>(GGXXACR_P_CharacterData[NodeGet->uUnitId].ppszPaletteList.size());
...
BOOL CGame_GGXXACR_P::LoadFile(CFile* LoadedFile, uint32_t nUnitNumber)
{
    BOOL fSuccess = TRUE;
    CString strInfo;
