...
CGame_VSAV2_A::CGame_VSAV2_A(uint32_t nConfirmedROMSize)
{
    ...
    m_nSafeCountForThisRom = 3387 + GetExtraCt(VSAV2_A_EXTRALOC);
...
uint32_t CGame_VSAV2_A::GetExtraCt(uint32_t nUnitId, BOOL fCountVisibleOnly)
{
    return _GetExtraCount(rgExtraCountAll, VSAV2_A_NUMUNIT, nUnitId, VSAV2_A_EXTRA_CUSTOM);
}

uint32_t CGame_VSAV2_A::GetExtraLoc(uint32_t nUnitId)
{
...
sFileRule CGame_VSAV2_A::GetRule(uint32_t nUnitId)
{
...
uint32_t CGame_VSAV2_A::GetCollectionCountForUnit(uint32_t nUnitId)
{
...
uint32_t CGame_VSAV2_A::GetNodeCountForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
...
LPCWSTR CGame_VSAV2_A::GetDescriptionForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetDescriptionForCollection(VSAV2_A_UNITS, VSAV2_A_EXTRALOC, nUnitId, nCollectionId);
}

uint32_t CGame_VSAV2_A::GetPaletteCountForUnit(uint32_t nUnitId)
{
...
const sGame_PaletteDataset* CGame_VSAV2_A::GetPaletteSet(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetPaletteSet(VSAV2_A_UNITS, nUnitId, nCollectionId);
}

const sDescTreeNode* CGame_VSAV2_A::GetNodeFromPaletteId(uint32_t nUnitId, uint32_t nPaletteId, bool fReturnBasicNodesOnly)
{
...
const sGame_PaletteDataset* CGame_VSAV2_A::GetSpecificPalette(uint32_t nUnitId, uint32_t nPaletteId)
{
...
void CGame_VSAV2_A::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
    ...
    else // VSAV2_A_EXTRALOC
    {
        ...
        stExtraDef* pCurrDef = &VSAV2_A_EXTRA_CUSTOM[GetExtraLoc(nUnitId) + nPalId];
