...
sDescTreeNode* CGame_VSAV2_A::InitDescTree()
{
    ...
    strMsg.Format(L"CGame_VSAV2_A::InitDescTree: Building desc tree for VSAV2 %s extras...\n", fHaveExtras ? L"with" : L"without");
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < VSAV2_A_EXTRALOC)
        {
            ...
            _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", VSAV2_UNITS[iUnitCtr].szDesc);
    ...
    strMsg.Format(L"CGame_VSAV2_A::InitDescTree: Loaded %u palettes for VSAV2\n", nTotalPaletteCount);
    OutputDebugString(strMsg);
...
#if VSAV2_DEBUG
        CString strMsg;
        strMsg.Format(L"CGame_VSAV2_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
