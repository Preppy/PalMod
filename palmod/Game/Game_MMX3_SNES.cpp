...
CGame_MMX3_SNES::CGame_MMX3_SNES(uint32_t nConfirmedROMSize)
{
...
uint32_t CGame_MMX3_SNES::rgExtraCountAll[MMX3_SNES_NUMUNIT + 1];
uint32_t CGame_MMX3_SNES::rgExtraLoc[MMX3_SNES_NUMUNIT + 1];
uint32_t CGame_MMX3_SNES::m_nTotalPaletteCountForMMX3 = 0;
uint32_t CGame_MMX3_SNES::m_nConfirmedROMSize = -1;
...
CGame_MMX3_SNES::~CGame_MMX3_SNES()
{
    safe_delete_array(CGame_MMX3_SNES::MMX3_SNES_EXTRA_CUSTOM);
    ClearDataBuffer();
...
uint32_t CGame_MMX3_SNES::GetExtraCt(uint32_t nUnitId, BOOL fCountVisibleOnly)
{
    return _GetExtraCount(rgExtraCountAll, MMX3_SNES_NUMUNIT, nUnitId, MMX3_SNES_EXTRA_CUSTOM);
}

uint32_t CGame_MMX3_SNES::GetExtraLoc(uint32_t nUnitId)
{
...
sDescTreeNode* CGame_MMX3_SNES::InitDescTree()
{
    ...
    LoadExtraFileForGame(EXTRA_FILENAME_MMX3_SNES, &MMX3_SNES_EXTRA_CUSTOM, MMX3_SNES_EXTRALOC, m_nConfirmedROMSize);

    uint16_t nUnitCt = MMX3_SNES_NUMUNIT + (GetExtraCt(MMX3_SNES_EXTRALOC) ? 1 : 0);
...
sFileRule CGame_MMX3_SNES::GetRule(uint32_t nUnitId)
{
...
uint32_t CGame_MMX3_SNES::GetCollectionCountForUnit(uint32_t nUnitId)
{
...
uint32_t CGame_MMX3_SNES::GetNodeCountForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
...
LPCWSTR CGame_MMX3_SNES::GetDescriptionForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetDescriptionForCollection(MMX3_SNES_UNITS, MMX3_SNES_EXTRALOC, nUnitId, nCollectionId);
}

uint32_t CGame_MMX3_SNES::GetPaletteCountForUnit(uint32_t nUnitId)
{
...
const sGame_PaletteDataset* CGame_MMX3_SNES::GetPaletteSet(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetPaletteSet(MMX3_SNES_UNITS, nUnitId, nCollectionId);
}

const sDescTreeNode* CGame_MMX3_SNES::GetNodeFromPaletteId(uint32_t nUnitId, uint32_t nPaletteId, bool fReturnBasicNodesOnly)
{
...
const sGame_PaletteDataset* CGame_MMX3_SNES::GetSpecificPalette(uint32_t nUnitId, uint32_t nPaletteId)
{
...
void CGame_MMX3_SNES::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
    ...
    else // MMX3_SNES_EXTRALOC
    {
        ...
        stExtraDef* pCurrDef = &MMX3_SNES_EXTRA_CUSTOM[GetExtraLoc(nUnitId) + nPalId];
