#pragma once

const std::vector<uint16_t> AsuraBuster_A_IMGIDS_USED =
{
    indexAsuraBuster_Alice,             // 0x31
    indexAsuraBuster_AliceBang,         // 0x32
    indexAsuraBuster_ChenMao,           // 0x33
    indexAsuraBuster_Goat,              // 0x34
    indexAsuraBuster_Leon,              // 0x35
    indexAsuraBuster_Nanami,            // 0x36
    indexAsuraBuster_Rokurouta,         // 0x37
    indexAsuraBuster_RoseMary,          // 0x38
    indexAsuraBuster_Sittara,           // 0x39
    indexAsuraBuster_Taros,             // 0x3a
    indexAsuraBuster_Vebel,             // 0x3b
    indexAsuraBuster_Yashaou,           // 0x3c
    indexAsuraBuster_ZamB,              // 0x3d
    indexAsuraBuster_Zinsuke,           // 0x3e
    indexAsuraBuster_Stages,            // 0x3f
    indexAsuraBuster_Bonus,             // 0x40
};

//  Yashaou
const sGame_PaletteDataset AsuraBuster_A_Yashaou_A[] =
{
    { L"Base A", 0x93492, 0x934b2, indexAsuraBuster_Yashaou, 0x00 },
    { L"Weapon Effects A", 0x934b2, 0x934d2, indexAsuraBuster_Yashaou, 0x01 },
    { L"Weapon A", 0x934d2, 0x934f2, indexAsuraBuster_Yashaou, 0x02 },
    { L"Nanami A", 0x934f2, 0x93512, indexAsuraBuster_Yashaou, 0x03 },
    { L"Unused A", 0x93512, 0x93532, indexAsuraBuster_Yashaou, 0x04 },
    { L"Nanami Lifebar Portrait A", 0x93532, 0x93552, indexAsuraBuster_Yashaou, 0x05 },

    { L"Yashaou A Portrait", 0x8a4ac, 0x8a4cc },
    { L"Yashaou A Super Portrait", 0x7fab0, 0x7fad0 },
    { L"Yashaou A Super Portrait Trail", 0x7fad0, 0x7faf0 },
};

const sGame_PaletteDataset AsuraBuster_A_Yashaou_B[] =
{
    { L"Base B", 0x93eb2, 0x93ed2, indexAsuraBuster_Yashaou, 0x00 },
    { L"Weapon Effects B", 0x93ed2, 0x93ef2, indexAsuraBuster_Yashaou, 0x01 },
    { L"Weapon B", 0x93ef2, 0x93f12, indexAsuraBuster_Yashaou, 0x02 },
    { L"Nanami B", 0x93f12, 0x93f32, indexAsuraBuster_Yashaou, 0x03 },
    { L"Unused B", 0x93f32, 0x93f52, indexAsuraBuster_Yashaou, 0x04 },
    { L"Nanami Lifebar Portrait B", 0x93f52, 0x93f72, indexAsuraBuster_Yashaou, 0x05 },
    { L"Yashaou B Portrait", 0x8a4cc, 0x8a4ec },
    { L"Yashaou B Super Portrait", 0x7faf0, 0x7fb10 },
    { L"Yashaou B Super Portrait Trail", 0x7fb10, 0x7fb30 },
};

const sGame_PaletteDataset AsuraBuster_A_Yashaou_AB[] =
{
    { L"Base AB", 0x948d2, 0x948f2, indexAsuraBuster_Yashaou, 0x00 },
    { L"Weapon Effects AB", 0x948f2, 0x94912, indexAsuraBuster_Yashaou, 0x01 },
    { L"Weapon AB", 0x94912, 0x94932, indexAsuraBuster_Yashaou, 0x02 },
    { L"Nanami AB", 0x94932, 0x94952, indexAsuraBuster_Yashaou, 0x03 },
    { L"Unused AB", 0x94952, 0x94972, indexAsuraBuster_Yashaou, 0x04 },
    { L"Nanami Lifebar Portrait AB", 0x94972, 0x94992, indexAsuraBuster_Yashaou, 0x05 },
    { L"Yashaou AB Portrait", 0x8a4ec, 0x8a50c },
    { L"Yashaou AB Super Portrait", 0x7fb30, 0x7fb50 },
    { L"Yashaou AB Super Portrait Trail", 0x7fb50, 0x7fb70 },
};

const sGame_PaletteDataset AsuraBuster_A_Yashaou_C[] =
{
    { L"Base C", 0x952f2, 0x95312, indexAsuraBuster_Yashaou, 0x00 },
    { L"Weapon Effects C", 0x95312, 0x95332, indexAsuraBuster_Yashaou, 0x01 },
    { L"Weapon C", 0x95332, 0x95352, indexAsuraBuster_Yashaou, 0x02 },
    { L"Nanami C", 0x95352, 0x95372, indexAsuraBuster_Yashaou, 0x03 },
    { L"Unused C", 0x95372, 0x95392, indexAsuraBuster_Yashaou, 0x04 },
    { L"Nanami Lifebar Portrait C", 0x95392, 0x953b2, indexAsuraBuster_Yashaou, 0x05 },

    { L"Yashaou C Portrait", 0x8a50c, 0x8a52c },
    { L"Yashaou C Super Portrait", 0x7fb70, 0x7fb90 },
    { L"Yashaou C Super Portrait Trail", 0x7fb90, 0x7fbb0 },
};

const sGame_PaletteDataset AsuraBuster_A_Yashaou_AC[] =
{
    { L"Base AC", 0x95d12, 0x95d32, indexAsuraBuster_Yashaou, 0x00 },
    { L"Weapon Effects AC", 0x95d32, 0x95d52, indexAsuraBuster_Yashaou, 0x01 },
    { L"Weapon AC", 0x95d52, 0x95d72, indexAsuraBuster_Yashaou, 0x02 },
    { L"Nanami AC", 0x95d72, 0x95d92, indexAsuraBuster_Yashaou, 0x03 },
    { L"Unused AC", 0x95d92, 0x95db2, indexAsuraBuster_Yashaou, 0x04 },
    { L"Nanami Lifebar Portrait AC", 0x95db2, 0x95dd2, indexAsuraBuster_Yashaou, 0x05 },

    { L"Yashaou AC Portrait", 0x8a52c, 0x8a54c },
    { L"Yashaou AC Super Portrait", 0x7fbb0, 0x7fbd0 },
    { L"Yashaou AC Super Portrait Trail", 0x7fbd0, 0x7fbf0 },
};

const sGame_PaletteDataset AsuraBuster_A_Yashaou_BC[] =
{
    { L"Base BC", 0x96732, 0x96752, indexAsuraBuster_Yashaou, 0x00 },
    { L"Weapon Effects BC", 0x96752, 0x96772, indexAsuraBuster_Yashaou, 0x01 },
    { L"Weapon BC", 0x96772, 0x96792, indexAsuraBuster_Yashaou, 0x02 },
    { L"Nanami BC", 0x96792, 0x967b2, indexAsuraBuster_Yashaou, 0x03 },
    { L"Unused BC", 0x967b2, 0x967d2, indexAsuraBuster_Yashaou, 0x04 },
    { L"Nanami Lifebar Portrait BC", 0x967d2, 0x967f2, indexAsuraBuster_Yashaou, 0x05 },

    { L"Yashaou BC Portrait", 0x8a54c, 0x8a56c },
    { L"Yashaou BC Super Portrait", 0x7fbf0, 0x7fc10 },
    { L"Yashaou BC Super Portrait Trail", 0x7fc10, 0x7fc30 },
};

const sGame_PaletteDataset AsuraBuster_A_Yashaou_ABC[] =
{
    { L"Base ABC", 0x97152, 0x97172, indexAsuraBuster_Yashaou, 0x00 },
    { L"Weapon Effects ABC", 0x97172, 0x97192, indexAsuraBuster_Yashaou, 0x01 },
    { L"Weapon ABC", 0x97192, 0x971b2, indexAsuraBuster_Yashaou, 0x02 },
    { L"Nanami ABC", 0x971b2, 0x971d2, indexAsuraBuster_Yashaou, 0x03 },
    { L"Unused ABC", 0x971d2, 0x971f2, indexAsuraBuster_Yashaou, 0x04 },
    { L"Nanami Lifebar Portrait ABC", 0x971f2, 0x97212, indexAsuraBuster_Yashaou, 0x05 },
    { L"Yashaou ABC Portrait", 0x8a56c, 0x8a58c },
    { L"Yashaou ABC Super Portrait", 0x7fc30, 0x7fc50 },
    { L"Yashaou ABC Super Portrait Trail", 0x7fc50, 0x7fc70 },
};

const sGame_PaletteDataset AsuraBuster_A_Yashaou_Shared[] =
{
    //  Yashaou Shared
    { L"Yashaou Shared Weapon Portrait", 0x8a5ac, 0x8a5cc },
};

const sGame_PaletteDataset AsuraBuster_A_Goat_A[] =
{
    // Goat
    { L"Base A", 0x93552, 0x93572, indexAsuraBuster_Goat, 0x00 },
    { L"Weapon A", 0x93572, 0x93592, indexAsuraBuster_Goat, 0x01 },
    { L"Weapon Effects A", 0x93592, 0x935b2, indexAsuraBuster_Goat, 0x02 },
    { L"Nanami A", 0x935b2, 0x935d2, indexAsuraBuster_Goat, 0x03 },
    { L"Unused A", 0x935d2, 0x935f2, indexAsuraBuster_Goat, 0x04 },
    { L"Nanami Lifebar Portrait A", 0x935f2, 0x93612, indexAsuraBuster_Goat, 0x05 },

    { L"Goat A Portrait", 0x8a5cc, 0x8a5ec },
    { L"Goat A Super Portrait", 0x7fc70, 0x7fc90 },
    { L"Goat A Super Portrait Trail", 0x7fc90, 0x7fcb0 },
};

const sGame_PaletteDataset AsuraBuster_A_Goat_B[] =
{
    { L"Base B", 0x93f72, 0x93f92, indexAsuraBuster_Goat, 0x00 },
    { L"Weapon B", 0x93f92, 0x93fb2, indexAsuraBuster_Goat, 0x01 },
    { L"Weapon Effects B", 0x93fb2, 0x93fd2, indexAsuraBuster_Goat, 0x02 },
    { L"Nanami B", 0x93fd2, 0x93ff2, indexAsuraBuster_Goat, 0x03 },
    { L"Unused B", 0x93ff2, 0x94012, indexAsuraBuster_Goat, 0x04 },
    { L"Nanami Lifebar Portrait B", 0x94012, 0x94032, indexAsuraBuster_Goat, 0x05 },

    { L"Goat B Portrait", 0x8a5ec, 0x8a60c },
    { L"Goat B Super Portrait", 0x7fcb0, 0x7fcd0 },
    { L"Goat B Super Portrait Trail", 0x7fcd0, 0x7fcf0 },
};

const sGame_PaletteDataset AsuraBuster_A_Goat_AB[] =
{
    { L"Base AB", 0x94992, 0x949b2, indexAsuraBuster_Goat, 0x00 },
    { L"Weapon AB", 0x949b2, 0x949d2, indexAsuraBuster_Goat, 0x01 },
    { L"Weapon Effects AB", 0x949d2, 0x949f2, indexAsuraBuster_Goat, 0x02 },
    { L"Nanami AB", 0x949f2, 0x94a12, indexAsuraBuster_Goat, 0x03 },
    { L"Unused AB", 0x94a12, 0x94a32, indexAsuraBuster_Goat, 0x04 },
    { L"Nanami Lifebar Portrait AB", 0x94a32, 0x94a52, indexAsuraBuster_Goat, 0x05 },

    { L"Goat AB Portrait", 0x8a60c, 0x8a62c },
    { L"Goat AB Super Portrait", 0x7fcf0, 0x7fd10 },
    { L"Goat AB Super Portrait Trail", 0x7fd10, 0x7fd30 },
};

const sGame_PaletteDataset AsuraBuster_A_Goat_C[] =
{
    { L"Base C", 0x953b2, 0x953d2, indexAsuraBuster_Goat, 0x00 },
    { L"Weapon C", 0x953d2, 0x953f2, indexAsuraBuster_Goat, 0x01 },
    { L"Weapon Effects C", 0x953f2, 0x95412, indexAsuraBuster_Goat, 0x02 },
    { L"Nanami C", 0x95412, 0x95432, indexAsuraBuster_Goat, 0x03 },
    { L"Unused C", 0x95432, 0x95452, indexAsuraBuster_Goat, 0x04 },
    { L"Nanami Lifebar Portrait C", 0x95452, 0x95472, indexAsuraBuster_Goat, 0x05 },

    { L"Goat C Portrait", 0x8a62c, 0x8a64c },
    { L"Goat C Super Portrait", 0x7fd30, 0x7fd50 },
    { L"Goat C Super Portrait Trail", 0x7fd50, 0x7fd70 },
};

const sGame_PaletteDataset AsuraBuster_A_Goat_AC[] =
{
    { L"Base AC", 0x95dd2, 0x95df2, indexAsuraBuster_Goat, 0x00 },
    { L"Weapon AC", 0x95df2, 0x95e12, indexAsuraBuster_Goat, 0x01 },
    { L"Weapon Effects AC", 0x95e12, 0x95e32, indexAsuraBuster_Goat, 0x02 },
    { L"Nanami AC", 0x95e32, 0x95e52, indexAsuraBuster_Goat, 0x03 },
    { L"Unused AC", 0x95e52, 0x95e72, indexAsuraBuster_Goat, 0x04 },
    { L"Nanami Lifebar Portrait AC", 0x95e72, 0x95e92, indexAsuraBuster_Goat, 0x05 },

    { L"Goat AC Portrait", 0x8a64c, 0x8a66c },
    { L"Goat AC Super Portrait", 0x7fd70, 0x7fd90 },
    { L"Goat AC Super Portrait Trail", 0x7fd90, 0x7fdb0 },
};

const sGame_PaletteDataset AsuraBuster_A_Goat_BC[] =
{
    { L"Base BC", 0x967f2, 0x96812, indexAsuraBuster_Goat, 0x00 },
    { L"Weapon BC", 0x96812, 0x96832, indexAsuraBuster_Goat, 0x01 },
    { L"Weapon Effects BC", 0x96832, 0x96852, indexAsuraBuster_Goat, 0x02 },
    { L"Nanami BC", 0x96852, 0x96872, indexAsuraBuster_Goat, 0x03 },
    { L"Unused BC", 0x96872, 0x96892, indexAsuraBuster_Goat, 0x04 },
    { L"Nanami Lifebar Portrait BC", 0x96892, 0x968b2, indexAsuraBuster_Goat, 0x05 },

    { L"Goat BC Portrait", 0x8a66c, 0x8a68c },
    { L"Goat BC Super Portrait", 0x7fdb0, 0x7fdd0 },
    { L"Goat BC Super Portrait Trail", 0x7fdd0, 0x7fdf0 },
};

const sGame_PaletteDataset AsuraBuster_A_Goat_ABC[] =
{
    { L"Base ABC", 0x97212, 0x97232, indexAsuraBuster_Goat, 0x00 },
    { L"Weapon ABC", 0x97232, 0x97252, indexAsuraBuster_Goat, 0x01 },
    { L"Weapon Effects ABC", 0x97252, 0x97272, indexAsuraBuster_Goat, 0x02 },
    { L"Nanami ABC", 0x97272, 0x97292, indexAsuraBuster_Goat, 0x03 },
    { L"Unused ABC", 0x97292, 0x972b2, indexAsuraBuster_Goat, 0x04 },
    { L"Nanami Lifebar Portrait ABC", 0x972b2, 0x972d2, indexAsuraBuster_Goat, 0x05 },

    { L"Goat ABC Portrait", 0x8a68c, 0x8a6ac },
    { L"Goat ABC Super Portrait", 0x7fdf0, 0x7fe10 },
    { L"Goat ABC Super Portrait Trail", 0x7fe10, 0x7fe30 },
};

const sGame_PaletteDataset AsuraBuster_A_Goat_Shared[] =
{
    //  Goat Shared
    { L"Goat Shared Weapon Portrait", 0x8a6cc, 0x8a6ec },
};

const sGame_PaletteDataset AsuraBuster_A_Leon_A[] =
{
    //  Leon
    { L"Base A", 0x93612, 0x93632, indexAsuraBuster_Leon, 0x00 },
    { L"Weapon and Effects A", 0x93632, 0x93652, indexAsuraBuster_Leon, 0x01 },
    { L"Nanami A", 0x93652, 0x93672, indexAsuraBuster_Leon, 0x02 },
    { L"Unused A", 0x93672, 0x93692, indexAsuraBuster_Leon, 0x03 },
    { L"Unused A", 0x93692, 0x936b2, indexAsuraBuster_Leon, 0x04 },
    { L"Nanami Lifebar Portrait A", 0x936b2, 0x936d2, indexAsuraBuster_Leon, 0x05 },

    { L"Leon A Portrait", 0x8a6ec, 0x8a70c },
    { L"Leon A Super Portrait", 0x7fe30, 0x7fe50 },
    { L"Leon A Super Portrait Trail", 0x7fe50, 0x7fe70 },
};

const sGame_PaletteDataset AsuraBuster_A_Leon_B[] =
{
    { L"Base B", 0x94032, 0x94052, indexAsuraBuster_Leon, 0x00 },
    { L"Weapon and Effects B", 0x94052, 0x94072, indexAsuraBuster_Leon, 0x01 },
    { L"Nanami B", 0x94072, 0x94092, indexAsuraBuster_Leon, 0x02 },
    { L"Unused B", 0x94092, 0x940b2, indexAsuraBuster_Leon, 0x03 },
    { L"Unused B", 0x940b2, 0x940d2, indexAsuraBuster_Leon, 0x04 },
    { L"Nanami Lifebar Portrait B", 0x940d2, 0x940f2, indexAsuraBuster_Leon, 0x05 },

    { L"Leon B Portrait", 0x8a70c, 0x8a72c },
    { L"Leon B Super Portrait", 0x7fe70, 0x7fe90 },
    { L"Leon B Super Portrait Trail", 0x7fe90, 0x7feb0 },
};

const sGame_PaletteDataset AsuraBuster_A_Leon_AB[] =
{
    { L"Base AB", 0x94a52, 0x94a72, indexAsuraBuster_Leon, 0x00 },
    { L"Weapon and Effects AB", 0x94a72, 0x94a92, indexAsuraBuster_Leon, 0x01 },
    { L"Nanami AB", 0x94a92, 0x94ab2, indexAsuraBuster_Leon, 0x02 },
    { L"Unused AB", 0x94ab2, 0x94ad2, indexAsuraBuster_Leon, 0x03 },
    { L"Unused AB", 0x94ad2, 0x94af2, indexAsuraBuster_Leon, 0x04 },
    { L"Nanami Lifebar Portrait AB", 0x94af2, 0x94b12, indexAsuraBuster_Leon, 0x05 },

    { L"Leon AB Portrait", 0x8a72c, 0x8a74c },
    { L"Leon AB Super Portrait", 0x7feb0, 0x7fed0 },
    { L"Leon AB Super Portrait Trail", 0x7fed0, 0x7fef0 },
};

const sGame_PaletteDataset AsuraBuster_A_Leon_C[] =
{
    { L"Base C", 0x95472, 0x95492, indexAsuraBuster_Leon, 0x00 },
    { L"Weapon and Effects C", 0x95492, 0x954b2, indexAsuraBuster_Leon, 0x01 },
    { L"Nanami C", 0x954b2, 0x954d2, indexAsuraBuster_Leon, 0x02 },
    { L"Unused C", 0x954d2, 0x954f2, indexAsuraBuster_Leon, 0x03 },
    { L"Unused C", 0x954f2, 0x95512, indexAsuraBuster_Leon, 0x04 },
    { L"Nanami Lifebar Portrait C", 0x95512, 0x95532, indexAsuraBuster_Leon, 0x05 },

    { L"Leon C Portrait", 0x8a74c, 0x8a76c },
    { L"Leon C Super Portrait", 0x7fef0, 0x7ff10 },
    { L"Leon C Super Portrait Trail", 0x7ff10, 0x7ff30 },
};

const sGame_PaletteDataset AsuraBuster_A_Leon_AC[] =
{
    { L"Base AC", 0x95e92, 0x95eb2, indexAsuraBuster_Leon, 0x00 },
    { L"Weapon and Effects AC", 0x95eb2, 0x95ed2, indexAsuraBuster_Leon, 0x01 },
    { L"Nanami AC", 0x95ed2, 0x95ef2, indexAsuraBuster_Leon, 0x02 },
    { L"Unused AC", 0x95ef2, 0x95f12, indexAsuraBuster_Leon, 0x03 },
    { L"Unused AC", 0x95f12, 0x95f32, indexAsuraBuster_Leon, 0x04 },
    { L"Nanami Lifebar Portrait AC", 0x95f32, 0x95f52, indexAsuraBuster_Leon, 0x05 },

    { L"Leon AC Portrait", 0x8a76c, 0x8a78c },
    { L"Leon AC Super Portrait", 0x7ff30, 0x7ff50 },
    { L"Leon AC Super Portrait Trail", 0x7ff50, 0x7ff70 },
};

const sGame_PaletteDataset AsuraBuster_A_Leon_BC[] =
{
    { L"Base BC", 0x968b2, 0x968d2, indexAsuraBuster_Leon, 0x00 },
    { L"Weapon and Effects BC", 0x968d2, 0x968f2, indexAsuraBuster_Leon, 0x01 },
    { L"Nanami BC", 0x968f2, 0x96912, indexAsuraBuster_Leon, 0x02 },
    { L"Unused BC", 0x96912, 0x96932, indexAsuraBuster_Leon, 0x03 },
    { L"Unused BC", 0x96932, 0x96952, indexAsuraBuster_Leon, 0x04 },
    { L"Nanami Lifebar Portrait BC", 0x96952, 0x96972, indexAsuraBuster_Leon, 0x05 },

    { L"Leon BC Portrait", 0x8a78c, 0x8a7ac },
    { L"Leon BC Super Portrait", 0x7ff70, 0x7ff90 },
    { L"Leon BC Super Portrait Trail", 0x7ff90, 0x7ffb0 },
};

const sGame_PaletteDataset AsuraBuster_A_Leon_ABC[] =
{
    { L"Base ABC", 0x972d2, 0x972f2, indexAsuraBuster_Leon, 0x00 },
    { L"Weapon and Effects ABC", 0x972f2, 0x97312, indexAsuraBuster_Leon, 0x01 },
    { L"Nanami ABC", 0x97312, 0x97332, indexAsuraBuster_Leon, 0x02 },
    { L"Unused ABC", 0x97332, 0x97352, indexAsuraBuster_Leon, 0x03 },
    { L"Unused ABC", 0x97352, 0x97372, indexAsuraBuster_Leon, 0x04 },
    { L"Nanami Lifebar Portrait ABC", 0x97372, 0x97392, indexAsuraBuster_Leon, 0x05 },

    { L"Leon ABC Portrait", 0x8a7ac, 0x8a7cc },
    { L"Leon ABC Super Portrait", 0x7ffb0, 0x7ffd0 },
    { L"Leon ABC Super Portrait Trail", 0x7ffd0, 0x7fff0 },
};

const sGame_PaletteDataset AsuraBuster_A_ChenMao_A[] =
{
    //  Chen Mao
    { L"Base A", 0x936d2, 0x936f2, indexAsuraBuster_ChenMao, 0x00 },
    { L"Effects 1 A", 0x936f2, 0x93712, indexAsuraBuster_ChenMao, 0x01 },
    { L"Effects 2 A", 0x93712, 0x93732, indexAsuraBuster_ChenMao, 0x02 },
    { L"Nanami A", 0x93732, 0x93752, indexAsuraBuster_ChenMao, 0x03 },
    { L"Unused A", 0x93752, 0x93772, indexAsuraBuster_ChenMao, 0x04 },
    { L"Nanami Lifebar Portrait A", 0x93772, 0x93792, indexAsuraBuster_ChenMao, 0x05 },

    { L"Chen-Mao A Portrait", 0x8a7ec, 0x8a80c },
    { L"Chen-Mao A Super Portrait", 0x7fff0, 0x80010 },
    { L"Chen-Mao A Super Portrait Trail", 0x80010, 0x80030 },
};

const sGame_PaletteDataset AsuraBuster_A_ChenMao_B[] =
{
    { L"Base B", 0x940f2, 0x94112, indexAsuraBuster_ChenMao, 0x00 },
    { L"Effects 1 B", 0x94112, 0x94132, indexAsuraBuster_ChenMao, 0x01 },
    { L"Effects 2 B", 0x94132, 0x94152, indexAsuraBuster_ChenMao, 0x02 },
    { L"Nanami B", 0x94152, 0x94172, indexAsuraBuster_ChenMao, 0x03 },
    { L"Unused B", 0x94172, 0x94192, indexAsuraBuster_ChenMao, 0x04 },
    { L"Nanami Lifebar Portrait B", 0x94192, 0x941b2, indexAsuraBuster_ChenMao, 0x05 },

    { L"Chen-Mao B Portrait", 0x8a80c, 0x8a82c },
    { L"Chen-Mao B Super Portrait", 0x80030, 0x80050 },
    { L"Chen-Mao B Super Portrait Trail", 0x80050, 0x80070 },
};

const sGame_PaletteDataset AsuraBuster_A_ChenMao_AB[] =
{
    { L"Base AB", 0x94b12, 0x94b32, indexAsuraBuster_ChenMao, 0x00 },
    { L"Effects 1 AB", 0x94b32, 0x94b52, indexAsuraBuster_ChenMao, 0x01 },
    { L"Effects 2 AB", 0x94b52, 0x94b72, indexAsuraBuster_ChenMao, 0x02 },
    { L"Nanami AB", 0x94b72, 0x94b92, indexAsuraBuster_ChenMao, 0x03 },
    { L"Unused AB", 0x94b92, 0x94bb2, indexAsuraBuster_ChenMao, 0x04 },
    { L"Nanami Lifebar Portrait AB", 0x94bb2, 0x94bd2, indexAsuraBuster_ChenMao, 0x05 },

    { L"Chen-Mao AB Portrait", 0x8a82c, 0x8a84c },
    { L"Chen-Mao AB Super Portrait", 0x80070, 0x80090 },
    { L"Chen-Mao AB Super Portrait Trail", 0x80090, 0x800b0 },
};

const sGame_PaletteDataset AsuraBuster_A_ChenMao_C[] =
{
    { L"Base C", 0x95532, 0x95552, indexAsuraBuster_ChenMao, 0x00 },
    { L"Effects 1 C", 0x95552, 0x95572, indexAsuraBuster_ChenMao, 0x01 },
    { L"Effects 2 C", 0x95572, 0x95592, indexAsuraBuster_ChenMao, 0x02 },
    { L"Nanami C", 0x95592, 0x955b2, indexAsuraBuster_ChenMao, 0x03 },
    { L"Unused C", 0x955b2, 0x955d2, indexAsuraBuster_ChenMao, 0x04 },
    { L"Nanami Lifebar Portrait C", 0x955d2, 0x955f2, indexAsuraBuster_ChenMao, 0x05 },

    { L"Chen-Mao C Portrait", 0x8a84c, 0x8a86c },
    { L"Chen-Mao C Super Portrait", 0x800b0, 0x800d0 },
    { L"Chen-Mao C Super Portrait Trail", 0x800d0, 0x800f0 },
};

const sGame_PaletteDataset AsuraBuster_A_ChenMao_AC[] =
{
    { L"Base AC", 0x95f52, 0x95f72, indexAsuraBuster_ChenMao, 0x00 },
    { L"Effects 1 AC", 0x95f72, 0x95f92, indexAsuraBuster_ChenMao, 0x01 },
    { L"Effects 2 AC", 0x95f92, 0x95fb2, indexAsuraBuster_ChenMao, 0x02 },
    { L"Nanami AC", 0x95fb2, 0x95fd2, indexAsuraBuster_ChenMao, 0x03 },
    { L"Unused AC", 0x95fd2, 0x95ff2, indexAsuraBuster_ChenMao, 0x04 },
    { L"Nanami Lifebar Portrait AC", 0x95ff2, 0x96012, indexAsuraBuster_ChenMao, 0x05 },

    { L"Chen-Mao AC Portrait", 0x8a86c, 0x8a88c },
    { L"Chen-Mao AC Super Portrait", 0x800f0, 0x80110 },
    { L"Chen-Mao AC Super Portrait Trail", 0x80110, 0x80130 },
};

const sGame_PaletteDataset AsuraBuster_A_ChenMao_BC[] =
{
    { L"Base BC", 0x96972, 0x96992, indexAsuraBuster_ChenMao, 0x00 },
    { L"Effects 1 BC", 0x96992, 0x969b2, indexAsuraBuster_ChenMao, 0x01 },
    { L"Effects 2 BC", 0x969b2, 0x969d2, indexAsuraBuster_ChenMao, 0x02 },
    { L"Nanami BC", 0x969d2, 0x969f2, indexAsuraBuster_ChenMao, 0x03 },
    { L"Unused BC", 0x969f2, 0x96a12, indexAsuraBuster_ChenMao, 0x04 },
    { L"Nanami Lifebar Portrait BC", 0x96a12, 0x96a32, indexAsuraBuster_ChenMao, 0x05 },

    { L"Chen-Mao BC Portrait", 0x8a88c, 0x8a8ac },
    { L"Chen-Mao BC Super Portrait", 0x80130, 0x80150 },
    { L"Chen-Mao BC Super Portrait Trail", 0x80150, 0x80170 },
};

const sGame_PaletteDataset AsuraBuster_A_ChenMao_ABC[] =
{
    { L"Base ABC", 0x97392, 0x973b2, indexAsuraBuster_ChenMao, 0x00 },
    { L"Effects 1 ABC", 0x973b2, 0x973d2, indexAsuraBuster_ChenMao, 0x01 },
    { L"Effects 2 ABC", 0x973d2, 0x973f2, indexAsuraBuster_ChenMao, 0x02 },
    { L"Nanami ABC", 0x973f2, 0x97412, indexAsuraBuster_ChenMao, 0x03 },
    { L"Unused ABC", 0x97412, 0x97432, indexAsuraBuster_ChenMao, 0x04 },
    { L"Nanami Lifebar Portrait ABC", 0x97432, 0x97452, indexAsuraBuster_ChenMao, 0x05 },

    { L"Chen-Mao ABC Portrait", 0x8a8ac, 0x8a8cc },
    { L"Chen-Mao ABC Super Portrait", 0x80170, 0x80190 },
    { L"Chen-Mao ABC Super Portrait Trail", 0x80190, 0x801b0 },
};

const sGame_PaletteDataset AsuraBuster_A_ZamB_A[] =
{
    //  Zam-B
    { L"Base A", 0x93792, 0x937b2, indexAsuraBuster_ZamB, 0x00 },
    { L"Weapon A", 0x937b2, 0x937d2, indexAsuraBuster_ZamB, 0x01 },
    { L"Nanami A", 0x937d2, 0x937f2, indexAsuraBuster_ZamB, 0x02 },
    { L"Unknown A", 0x937f2, 0x93812, indexAsuraBuster_ZamB, 0x03 },
    { L"Unused A", 0x93812, 0x93832, indexAsuraBuster_ZamB, 0x04 },
    { L"Nanami Lifebar Portrait A", 0x93832, 0x93852, indexAsuraBuster_ZamB, 0x05 },

    { L"Zam-B A Portrait 1", 0x8a8ec, 0x8a90c },
    { L"Zam-B A Portrait 2", 0x8a9ec, 0x8aa0c },
    { L"Zam-B A Super Portrait 1", 0x801b0, 0x801d0 },
    { L"Zam-B A Super Portrait Trail 1", 0x801d0, 0x801f0 },
    { L"Zam-B X Super Portrait 2", 0x801f0, 0x80210 },
};

const sGame_PaletteDataset AsuraBuster_A_ZamB_B[] =
{
    { L"Base B", 0x941b2, 0x941d2, indexAsuraBuster_ZamB, 0x00 },
    { L"Weapon B", 0x941d2, 0x941f2, indexAsuraBuster_ZamB, 0x01 },
    { L"Nanami B", 0x941f2, 0x94212, indexAsuraBuster_ZamB, 0x02 },
    { L"Unknown B", 0x94212, 0x94232, indexAsuraBuster_ZamB, 0x03 },
    { L"Unused B", 0x94232, 0x94252, indexAsuraBuster_ZamB, 0x04 },
    { L"Nanami Lifebar Portrait B", 0x94252, 0x94272, indexAsuraBuster_ZamB, 0x05 },

    { L"Zam-B B Portrait 1", 0x8a90c, 0x8a92c },
    { L"Zam-B B Portrait 2", 0x8aa0c, 0x8aa2c },
    { L"Zam-B B Super Portrait", 0x80210, 0x80230 },
    { L"Zam-B B Super Portrait Trail", 0x80230, 0x80250 },
    { L"Zam-B X Super Portrait 2", 0x80250, 0x80270 },
};

const sGame_PaletteDataset AsuraBuster_A_ZamB_AB[] =
{
    { L"Base AB", 0x94bd2, 0x94bf2, indexAsuraBuster_ZamB, 0x00 },
    { L"Weapon AB", 0x94bf2, 0x94c12, indexAsuraBuster_ZamB, 0x01 },
    { L"Nanami AB", 0x94c12, 0x94c32, indexAsuraBuster_ZamB, 0x02 },
    { L"Unknown AB", 0x94c32, 0x94c52, indexAsuraBuster_ZamB, 0x03 },
    { L"Unused AB", 0x94c52, 0x94c72, indexAsuraBuster_ZamB, 0x04 },
    { L"Nanami Lifebar Portrait AB", 0x94c72, 0x94c92, indexAsuraBuster_ZamB, 0x05 },

    { L"Zam-B AB Portrait 1", 0x8a92c, 0x8a94c },
    { L"Zam-B AB Portrait 2", 0x8aa2c, 0x8aa4c },
    { L"Zam-B AB Super Portrait", 0x80270, 0x80290 },
    { L"Zam-B AB Super Portrait Trail", 0x80290, 0x802b0 },
    { L"Zam-B AB Super Portrait 2", 0x802b0, 0x802d0 },
};

const sGame_PaletteDataset AsuraBuster_A_ZamB_C[] =
{
    { L"Base C", 0x955f2, 0x95612, indexAsuraBuster_ZamB, 0x00 },
    { L"Weapon C", 0x95612, 0x95632, indexAsuraBuster_ZamB, 0x01 },
    { L"Nanami C", 0x95632, 0x95652, indexAsuraBuster_ZamB, 0x02 },
    { L"Unknown C", 0x95652, 0x95672, indexAsuraBuster_ZamB, 0x03 },
    { L"Unused C", 0x95672, 0x95692, indexAsuraBuster_ZamB, 0x04 },
    { L"Nanami Lifebar Portrait C", 0x95692, 0x956b2, indexAsuraBuster_ZamB, 0x05 },

    { L"Zam-B C Portrait 1", 0x8a94c, 0x8a96c },
    { L"Zam-B C Portrait 2", 0x8aa4c, 0x8aa6c },
    { L"Zam-B C Super Portrait", 0x802d0, 0x802f0 },
    { L"Zam-B C Super Portrait Trail", 0x802f0, 0x80310 },
    { L"Zam-B C Super Portrait 2", 0x80310, 0x80330 },
};

const sGame_PaletteDataset AsuraBuster_A_ZamB_AC[] =
{
    { L"Base AC", 0x96012, 0x96032, indexAsuraBuster_ZamB, 0x00 },
    { L"Weapon AC", 0x96032, 0x96052, indexAsuraBuster_ZamB, 0x01 },
    { L"Nanami AC", 0x96052, 0x96072, indexAsuraBuster_ZamB, 0x02 },
    { L"Unknown AC", 0x96072, 0x96092, indexAsuraBuster_ZamB, 0x03 },
    { L"Unused AC", 0x96092, 0x960b2, indexAsuraBuster_ZamB, 0x04 },
    { L"Nanami Lifebar Portrait AC", 0x960b2, 0x960d2, indexAsuraBuster_ZamB, 0x05 },

    { L"Zam-B AC Portrait 1", 0x8a96c, 0x8a98c },
    { L"Zam-B AC Portrait 2", 0x8aa6c, 0x8aa8c },
    { L"Zam-B AC Super Portrait", 0x80330, 0x80350 },
    { L"Zam-B AC Super Portrait Trail", 0x80350, 0x80370 },
    { L"Zam-B AC Super Portrait 2", 0x80370, 0x80390 },
};

const sGame_PaletteDataset AsuraBuster_A_ZamB_BC[] =
{
    { L"Base BC", 0x96a32, 0x96a52, indexAsuraBuster_ZamB, 0x00 },
    { L"Weapon BC", 0x96a52, 0x96a72, indexAsuraBuster_ZamB, 0x01 },
    { L"Nanami BC", 0x96a72, 0x96a92, indexAsuraBuster_ZamB, 0x02 },
    { L"Unknown BC", 0x96a92, 0x96ab2, indexAsuraBuster_ZamB, 0x03 },
    { L"Unused BC", 0x96ab2, 0x96ad2, indexAsuraBuster_ZamB, 0x04 },
    { L"Nanami Lifebar Portrait BC", 0x96ad2, 0x96af2, indexAsuraBuster_ZamB, 0x05 },

    { L"Zam-B BC Portrait 1", 0x8a98c, 0x8a9ac },
    { L"Zam-B BC Portrait 2", 0x8aa8c, 0x8aaac },
    { L"Zam-B BC Super Portrait", 0x80390, 0x803b0 },
    { L"Zam-B BC Super Portrait Trail", 0x803b0, 0x803d0 },
    { L"Zam-B BC Super Portrait 2", 0x803d0, 0x803f0 },
};

const sGame_PaletteDataset AsuraBuster_A_ZamB_ABC[] =
{
    { L"Base ABC", 0x97452, 0x97472, indexAsuraBuster_ZamB, 0x00 },
    { L"Weapon ABC", 0x97472, 0x97492, indexAsuraBuster_ZamB, 0x01 },
    { L"Nanami ABC", 0x97492, 0x974b2, indexAsuraBuster_ZamB, 0x02 },
    { L"Unknown ABC", 0x974b2, 0x974d2, indexAsuraBuster_ZamB, 0x03 },
    { L"Unused ABC", 0x974d2, 0x974f2, indexAsuraBuster_ZamB, 0x04 },
    { L"Nanami Lifebar Portrait ABC", 0x974f2, 0x97512, indexAsuraBuster_ZamB, 0x05 },

    { L"Zam-B ABC Portrait 1", 0x8a9ac, 0x8a9cc },
    { L"Zam-B ABC Portrait 2", 0x8aaac, 0x8aacc },
    { L"Zam-B ABC Super Portrait", 0x803f0, 0x80410 },
    { L"Zam-B ABC Super Portrait Trail", 0x80410, 0x80430 },
    { L"Zam-B ABC Super Portrait 2", 0x80430, 0x80450 },
};

const sGame_PaletteDataset AsuraBuster_A_Taros_A[] =
{
    //  Taros
    { L"Base A", 0x93852, 0x93872, indexAsuraBuster_Taros, 0x00 },
    { L"Effects 1 A", 0x93872, 0x93892, indexAsuraBuster_Taros, 0x01 },
    { L"Effects 2 A", 0x93892, 0x938b2, indexAsuraBuster_Taros, 0x02 },
    { L"Nanami A", 0x938b2, 0x938d2, indexAsuraBuster_Taros, 0x03 },
    { L"Unused A", 0x938d2, 0x938f2, indexAsuraBuster_Taros, 0x04 },
    { L"Nanami Lifebar Portrait A", 0x938f2, 0x93912, indexAsuraBuster_Taros, 0x05 },

    { L"Taros A Portrait", 0x8adec, 0x8ae0c },
    { L"Taros A Super Portrait", 0x807d0, 0x807f0 },
    { L"Taros A Super Portrait Trail", 0x807f0, 0x80810 },
};

const sGame_PaletteDataset AsuraBuster_A_Taros_B[] =
{
    { L"Base B", 0x94272, 0x94292, indexAsuraBuster_Taros, 0x00 },
    { L"Effects 1 B", 0x94292, 0x942b2, indexAsuraBuster_Taros, 0x01 },
    { L"Effects 2 B", 0x942b2, 0x942d2, indexAsuraBuster_Taros, 0x02 },
    { L"Nanami B", 0x942d2, 0x942f2, indexAsuraBuster_Taros, 0x03 },
    { L"Unused B", 0x942f2, 0x94312, indexAsuraBuster_Taros, 0x04 },
    { L"Nanami Lifebar Portrait B", 0x94312, 0x94332, indexAsuraBuster_Taros, 0x05 },

    { L"Taros B Portrait", 0x8ae0c, 0x8ae2c },
    { L"Taros B Super Portrait", 0x80810, 0x80830 },
    { L"Taros B Super Portrait Trail", 0x80830, 0x80850 },
};

const sGame_PaletteDataset AsuraBuster_A_Taros_AB[] =
{
    { L"Base AB", 0x94c92, 0x94cb2, indexAsuraBuster_Taros, 0x00 },
    { L"Effects 1 AB", 0x94cb2, 0x94cd2, indexAsuraBuster_Taros, 0x01 },
    { L"Effects 2 AB", 0x94cd2, 0x94cf2, indexAsuraBuster_Taros, 0x02 },
    { L"Nanami AB", 0x94cf2, 0x94d12, indexAsuraBuster_Taros, 0x03 },
    { L"Unused AB", 0x94d12, 0x94d32, indexAsuraBuster_Taros, 0x04 },
    { L"Nanami Lifebar Portrait AB", 0x94d32, 0x94d52, indexAsuraBuster_Taros, 0x05 },

    { L"Taros AB Portrait", 0x8ae2c, 0x8ae4c },
    { L"Taros AB Super Portrait", 0x80850, 0x80870 },
    { L"Taros AB Super Portrait Trail", 0x80870, 0x80890 },
};

const sGame_PaletteDataset AsuraBuster_A_Taros_C[] =
{
    { L"Base C", 0x956b2, 0x956d2, indexAsuraBuster_Taros, 0x00 },
    { L"Effects 1 C", 0x956d2, 0x956f2, indexAsuraBuster_Taros, 0x01 },
    { L"Effects 2 C", 0x956f2, 0x95712, indexAsuraBuster_Taros, 0x02 },
    { L"Nanami C", 0x95712, 0x95732, indexAsuraBuster_Taros, 0x03 },
    { L"Unused C", 0x95732, 0x95752, indexAsuraBuster_Taros, 0x04 },
    { L"Nanami Lifebar Portrait C", 0x95752, 0x95772, indexAsuraBuster_Taros, 0x05 },

    { L"Taros C Portrait", 0x8ae4c, 0x8ae6c },
    { L"Taros C Super Portrait", 0x80890, 0x808b0 },
    { L"Taros C Super Portrait Trail", 0x808b0, 0x808d0 },
};

const sGame_PaletteDataset AsuraBuster_A_Taros_AC[] =
{
    { L"Base AC", 0x960d2, 0x960f2, indexAsuraBuster_Taros, 0x00 },
    { L"Effects 1 AC", 0x960f2, 0x96112, indexAsuraBuster_Taros, 0x01 },
    { L"Effects 2 AC", 0x96112, 0x96132, indexAsuraBuster_Taros, 0x02 },
    { L"Nanami AC", 0x96132, 0x96152, indexAsuraBuster_Taros, 0x03 },
    { L"Unused AC", 0x96152, 0x96172, indexAsuraBuster_Taros, 0x04 },
    { L"Nanami Lifebar Portrait AC", 0x96172, 0x96192, indexAsuraBuster_Taros, 0x05 },

    { L"Taros AC Portrait", 0x8ae6c, 0x8ae8c },
    { L"Taros AC Super Portrait", 0x808d0, 0x808f0 },
    { L"Taros AC Super Portrait Trail", 0x808f0, 0x80910 },
};

const sGame_PaletteDataset AsuraBuster_A_Taros_BC[] =
{
    { L"Base BC", 0x96af2, 0x96b12, indexAsuraBuster_Taros, 0x00 },
    { L"Effects 1 BC", 0x96b12, 0x96b32, indexAsuraBuster_Taros, 0x01 },
    { L"Effects 2 BC", 0x96b32, 0x96b52, indexAsuraBuster_Taros, 0x02 },
    { L"Nanami BC", 0x96b52, 0x96b72, indexAsuraBuster_Taros, 0x03 },
    { L"Unused BC", 0x96b72, 0x96b92, indexAsuraBuster_Taros, 0x04 },
    { L"Nanami Lifebar Portrait BC", 0x96b92, 0x96bb2, indexAsuraBuster_Taros, 0x05 },

    { L"Taros BC Portrait", 0x8ae8c, 0x8aeac },
    { L"Taros BC Super Portrait", 0x80910, 0x80930 },
    { L"Taros BC Super Portrait Trail", 0x80930, 0x80950 },
};

const sGame_PaletteDataset AsuraBuster_A_Taros_ABC[] =
{
    { L"Base ABC", 0x97512, 0x97532, indexAsuraBuster_Taros, 0x00 },
    { L"Effects 1 ABC", 0x97532, 0x97552, indexAsuraBuster_Taros, 0x01 },
    { L"Effects 2 ABC", 0x97552, 0x97572, indexAsuraBuster_Taros, 0x02 },
    { L"Nanami ABC", 0x97572, 0x97592, indexAsuraBuster_Taros, 0x03 },
    { L"Unused ABC", 0x97592, 0x975b2, indexAsuraBuster_Taros, 0x04 },
    { L"Nanami Lifebar Portrait ABC", 0x975b2, 0x975d2, indexAsuraBuster_Taros, 0x05 },

    { L"Taros ABC Portrait", 0x8aeac, 0x8aecc },
    { L"Taros ABC Super Portrait", 0x80950, 0x80970 },
    { L"Taros ABC Super Portrait Trail", 0x80970, 0x80990 },
};

const sGame_PaletteDataset AsuraBuster_A_Alice_A[] =
{
    //  Alice
    { L"Nanami A", 0x93912, 0x93932, indexAsuraBuster_Alice, 0x00 },
    { L"Effects 1 A", 0x93932, 0x93952, indexAsuraBuster_Alice, 0x01 },
    { L"Base A", 0x93952, 0x93972, indexAsuraBuster_Alice, 0x02 },
    { L"Skeleton A", 0x93972, 0x93992, indexAsuraBuster_Alice, 0x03 },
    { L"Effects 2 A", 0x93992, 0x939b2, indexAsuraBuster_Alice, 0x04 },
    { L"Nanami Lifebar Portrait A", 0x939b2, 0x939d2, indexAsuraBuster_Alice, 0x05 },

    { L"Alice Skeleton A", 0x93d92, 0x93db2 },
    { L"Alice A Portrait 1", 0x8abec, 0x8ac0c },
    { L"Alice A Portrait 2", 0x8acec, 0x8ad0c },
    { L"Alice A Super Portrait", 0x80610, 0x80630 },
    { L"Alice A Super Portrait Trail", 0x80630, 0x80650 },
};

const sGame_PaletteDataset AsuraBuster_A_Alice_B[] =
{
    { L"Nanami B", 0x94332, 0x94352, indexAsuraBuster_Alice, 0x00 },
    { L"Effects 1 B", 0x94352, 0x94372, indexAsuraBuster_Alice, 0x01 },
    { L"Base B", 0x94372, 0x94392, indexAsuraBuster_Alice, 0x02 },
    { L"Skeleton B", 0x94392, 0x943b2, indexAsuraBuster_Alice, 0x03 },
    { L"Effects 2 B", 0x943b2, 0x943d2, indexAsuraBuster_Alice, 0x04 },
    { L"Nanami Lifebar Portrait B", 0x943d2, 0x943f2, indexAsuraBuster_Alice, 0x05 },

    { L"Alice Skeleton B", 0x947b2, 0x947d2 },
    { L"Alice B Portrait 1", 0x8ac0c, 0x8ac2c },
    { L"Alice B Portrait 2", 0x8ad0c, 0x8ad2c },
    { L"Alice B Super Portrait", 0x80650, 0x80670 },
    { L"Alice B Super Portrait Trail", 0x80670, 0x80690 },
};

const sGame_PaletteDataset AsuraBuster_A_Alice_AB[] =
{
    { L"Nanami AB", 0x94d52, 0x94d72, indexAsuraBuster_Alice, 0x00 },
    { L"Effects 1 AB", 0x94d72, 0x94d92, indexAsuraBuster_Alice, 0x01 },
    { L"Base AB", 0x94d92, 0x94db2, indexAsuraBuster_Alice, 0x02 },
    { L"Skeleton AB", 0x94db2, 0x94dd2, indexAsuraBuster_Alice, 0x03 },
    { L"Effects 2 AB", 0x94dd2, 0x94df2, indexAsuraBuster_Alice, 0x04 },
    { L"Nanami Lifebar Portrait AB", 0x94df2, 0x94e12, indexAsuraBuster_Alice, 0x05 },

    { L"Alice Skeleton AB", 0x951d2, 0x951f2 },
    { L"Alice AB Portrait 1", 0x8ac2c, 0x8ac4c },
    { L"Alice AB Portrait 2", 0x8ad2c, 0x8ad4c },
    { L"Alice AB Super Portrait", 0x80690, 0x806b0 },
    { L"Alice AB Super Portrait Trail", 0x806b0, 0x806d0 },
};

const sGame_PaletteDataset AsuraBuster_A_Alice_C[] =
{
    { L"Nanami C", 0x95772, 0x95792, indexAsuraBuster_Alice, 0x00 },
    { L"Effects 1 C", 0x95792, 0x957b2, indexAsuraBuster_Alice, 0x01 },
    { L"Base C", 0x957b2, 0x957d2, indexAsuraBuster_Alice, 0x02 },
    { L"Skeleton C", 0x957d2, 0x957f2, indexAsuraBuster_Alice, 0x03 },
    { L"Effects 2 C", 0x957f2, 0x95812, indexAsuraBuster_Alice, 0x04 },
    { L"Nanami Lifebar Portrait C", 0x95812, 0x95832, indexAsuraBuster_Alice, 0x05 },

    { L"Alice Skeleton C", 0x95bf2, 0x95c12 },
    { L"Alice C Portrait 1", 0x8ac4c, 0x8ac6c },
    { L"Alice C Portrait 2", 0x8ad4c, 0x8ad6c },
    { L"Alice C Super Portrait", 0x806d0, 0x806f0 },
    { L"Alice C Super Portrait Trail", 0x806f0, 0x80710 },
};

const sGame_PaletteDataset AsuraBuster_A_Alice_AC[] =
{
    { L"Nanami AC", 0x96192, 0x961b2, indexAsuraBuster_Alice, 0x00 },
    { L"Effects 1 AC", 0x961b2, 0x961d2, indexAsuraBuster_Alice, 0x01 },
    { L"Base AC", 0x961d2, 0x961f2, indexAsuraBuster_Alice, 0x02 },
    { L"Skeleton AC", 0x961f2, 0x96212, indexAsuraBuster_Alice, 0x03 },
    { L"Effects 2 AC", 0x96212, 0x96232, indexAsuraBuster_Alice, 0x04 },
    { L"Nanami Lifebar Portrait AC", 0x96232, 0x96252, indexAsuraBuster_Alice, 0x05 },

    { L"Alice Skeleton AC", 0x96612, 0x96632 },
    { L"Alice AC Portrait 1", 0x8ac6c, 0x8ac8c },
    { L"Alice AC Portrait 2", 0x8ad6c, 0x8ad8c },
    { L"Alice AC Super Portrait", 0x80710, 0x80730 },
    { L"Alice AC Super Portrait Trail", 0x80730, 0x80750 },
};

const sGame_PaletteDataset AsuraBuster_A_Alice_BC[] =
{
    { L"Nanami BC", 0x96bb2, 0x96bd2, indexAsuraBuster_Alice, 0x00 },
    { L"Effects 1 BC", 0x96bd2, 0x96bf2, indexAsuraBuster_Alice, 0x01 },
    { L"Base BC", 0x96bf2, 0x96c12, indexAsuraBuster_Alice, 0x02 },
    { L"Skeleton BC", 0x96c12, 0x96c32, indexAsuraBuster_Alice, 0x03 },
    { L"Effects 2 BC", 0x96c32, 0x96c52, indexAsuraBuster_Alice, 0x04 },
    { L"Nanami Lifebar Portrait BC", 0x96c52, 0x96c72, indexAsuraBuster_Alice, 0x05 },

    { L"Alice Skeleton BC", 0x97032, 0x97052 },
    { L"Alice BC Portrait 1", 0x8ac8c, 0x8acac },
    { L"Alice BC Portrait 2", 0x8ad8c, 0x8adac },
    { L"Alice BC Super Portrait", 0x80750, 0x80770 },
    { L"Alice BC Super Portrait Trail", 0x80770, 0x80790 },
};

const sGame_PaletteDataset AsuraBuster_A_Alice_ABC[] =
{
    { L"Nanami ABC", 0x975d2, 0x975f2, indexAsuraBuster_Alice, 0x00 },
    { L"Effects 1 ABC", 0x975f2, 0x97612, indexAsuraBuster_Alice, 0x01 },
    { L"Base ABC", 0x97612, 0x97632, indexAsuraBuster_Alice, 0x02 },
    { L"Skeleton ABC", 0x97632, 0x97652, indexAsuraBuster_Alice, 0x03 },
    { L"Effects 2 ABC", 0x97652, 0x97672, indexAsuraBuster_Alice, 0x04 },
    { L"Nanami Lifebar Portrait ABC", 0x97672, 0x97692, indexAsuraBuster_Alice, 0x05 },

    { L"Alice Skeleton ABC", 0x97a52, 0x97a72 },
    { L"Alice ABC Portrait 1", 0x8acac, 0x8accc },
    { L"Alice ABC Portrait 2", 0x8adac, 0x8adcc },
    { L"Alice ABC Super Portrait", 0x80790, 0x807b0 },
    { L"Alice ABC Super Portrait Trail", 0x807b0, 0x807d0 },
};

const sGame_PaletteDataset AsuraBuster_A_RoseMary_A[] =
{
    //  Rose Mary
    { L"Base A", 0x939d2, 0x939f2, indexAsuraBuster_RoseMary, 0x00 },
    { L"Weapon A", 0x939f2, 0x93a12, indexAsuraBuster_RoseMary, 0x01 },
    { L"Effects 1 A", 0x93a12, 0x93a32, indexAsuraBuster_RoseMary, 0x02 },
    { L"Nanami A", 0x93a32, 0x93a52, indexAsuraBuster_RoseMary, 0x03 },
    { L"Effects 2 A", 0x93a52, 0x93a72, indexAsuraBuster_RoseMary, 0x04 },
    { L"Nanami Lifebar Portrait A", 0x93a72, 0x93a92, indexAsuraBuster_RoseMary, 0x05 },

    { L"Rose Mary A Portrait", 0x8aaec, 0x8ab0c },
    { L"Rose Mary A Super Portrait", 0x80450, 0x80470 },
    { L"Rose Mary A Super Portrait Trail", 0x80470, 0x80490 },
};

const sGame_PaletteDataset AsuraBuster_A_RoseMary_B[] =
{
    { L"Base B", 0x943f2, 0x94412, indexAsuraBuster_RoseMary, 0x00 },
    { L"Weapon B", 0x94412, 0x94432, indexAsuraBuster_RoseMary, 0x01 },
    { L"Effects 1 B", 0x94432, 0x94452, indexAsuraBuster_RoseMary, 0x02 },
    { L"Nanami B", 0x94452, 0x94472, indexAsuraBuster_RoseMary, 0x03 },
    { L"Effects 2 B", 0x94472, 0x94492, indexAsuraBuster_RoseMary, 0x04 },
    { L"Nanami Lifebar Portrait B", 0x94492, 0x944b2, indexAsuraBuster_RoseMary, 0x05 },

    { L"Rose Mary B Portrait", 0x8ab0c, 0x8ab2c },
    { L"Rose Mary B Super Portrait", 0x80490, 0x804b0 },
    { L"Rose Mary B Super Portrait Trail", 0x804b0, 0x804d0 },
};

const sGame_PaletteDataset AsuraBuster_A_RoseMary_AB[] =
{
    { L"Base AB", 0x94e12, 0x94e32, indexAsuraBuster_RoseMary, 0x00 },
    { L"Weapon AB", 0x94e32, 0x94e52, indexAsuraBuster_RoseMary, 0x01 },
    { L"Effects 1 AB", 0x94e52, 0x94e72, indexAsuraBuster_RoseMary, 0x02 },
    { L"Nanami AB", 0x94e72, 0x94e92, indexAsuraBuster_RoseMary, 0x03 },
    { L"Effects 2 AB", 0x94e92, 0x94eb2, indexAsuraBuster_RoseMary, 0x04 },
    { L"Nanami Lifebar Portrait AB", 0x94eb2, 0x94ed2, indexAsuraBuster_RoseMary, 0x05 },

    { L"Rose Mary AB Portrait", 0x8ab2c, 0x8ab4c },
    { L"Rose Mary AB Super Portrait", 0x804d0, 0x804f0 },
    { L"Rose Mary AB Super Portrait Trail", 0x804f0, 0x80510 },
};

const sGame_PaletteDataset AsuraBuster_A_RoseMary_C[] =
{
    { L"Base C", 0x95832, 0x95852, indexAsuraBuster_RoseMary, 0x00 },
    { L"Weapon C", 0x95852, 0x95872, indexAsuraBuster_RoseMary, 0x01 },
    { L"Effects 1 C", 0x95872, 0x95892, indexAsuraBuster_RoseMary, 0x02 },
    { L"Nanami C", 0x95892, 0x958b2, indexAsuraBuster_RoseMary, 0x03 },
    { L"Effects 2 C", 0x958b2, 0x958d2, indexAsuraBuster_RoseMary, 0x04 },
    { L"Nanami Lifebar Portrait C", 0x958d2, 0x958f2, indexAsuraBuster_RoseMary, 0x05 },

    { L"Rose Mary C Portrait", 0x8ab4c, 0x8ab6c },
    { L"Rose Mary C Super Portrait", 0x80510, 0x80530 },
    { L"Rose Mary C Super Portrait Trail", 0x80530, 0x80550 },
};

const sGame_PaletteDataset AsuraBuster_A_RoseMary_AC[] =
{
    { L"Base AC", 0x96252, 0x96272, indexAsuraBuster_RoseMary, 0x00 },
    { L"Weapon AC", 0x96272, 0x96292, indexAsuraBuster_RoseMary, 0x01 },
    { L"Effects 1 AC", 0x96292, 0x962b2, indexAsuraBuster_RoseMary, 0x02 },
    { L"Nanami AC", 0x962b2, 0x962d2, indexAsuraBuster_RoseMary, 0x03 },
    { L"Effects 2 AC", 0x962d2, 0x962f2, indexAsuraBuster_RoseMary, 0x04 },
    { L"Nanami Lifebar Portrait AC", 0x962f2, 0x96312, indexAsuraBuster_RoseMary, 0x05 },

    { L"Rose Mary AC Portrait", 0x8ab6c, 0x8ab8c },
    { L"Rose Mary AC Super Portrait", 0x80550, 0x80570 },
    { L"Rose Mary AC Super Portrait Trail", 0x80570, 0x80590 },
};

const sGame_PaletteDataset AsuraBuster_A_RoseMary_BC[] =
{
    { L"Base BC", 0x96c72, 0x96c92, indexAsuraBuster_RoseMary, 0x00 },
    { L"Weapon BC", 0x96c92, 0x96cb2, indexAsuraBuster_RoseMary, 0x01 },
    { L"Effects 1 BC", 0x96cb2, 0x96cd2, indexAsuraBuster_RoseMary, 0x02 },
    { L"Nanami BC", 0x96cd2, 0x96cf2, indexAsuraBuster_RoseMary, 0x03 },
    { L"Effects 2 BC", 0x96cf2, 0x96d12, indexAsuraBuster_RoseMary, 0x04 },
    { L"Nanami Lifebar Portrait BC", 0x96d12, 0x96d32, indexAsuraBuster_RoseMary, 0x05 },

    { L"Rose Mary BC Portrait", 0x8ab8c, 0x8abac },
    { L"Rose Mary BC Super Portrait", 0x80590, 0x805b0 },
    { L"Rose Mary BC Super Portrait Trail", 0x805b0, 0x805d0 },
};

const sGame_PaletteDataset AsuraBuster_A_RoseMary_ABC[] =
{
    { L"Base ABC", 0x97692, 0x976b2, indexAsuraBuster_RoseMary, 0x00 },
    { L"Weapon ABC", 0x976b2, 0x976d2, indexAsuraBuster_RoseMary, 0x01 },
    { L"Effects 1 ABC", 0x976d2, 0x976f2, indexAsuraBuster_RoseMary, 0x02 },
    { L"Nanami ABC", 0x976f2, 0x97712, indexAsuraBuster_RoseMary, 0x03 },
    { L"Effects 2 ABC", 0x97712, 0x97732, indexAsuraBuster_RoseMary, 0x04 },
    { L"Nanami Lifebar Portrait ABC", 0x97732, 0x97752, indexAsuraBuster_RoseMary, 0x05 },

    { L"Rose Mary ABC Portrait", 0x8abac, 0x8abcc },
    { L"Rose Mary ABC Super Portrait", 0x805d0, 0x805f0 },
    { L"Rose Mary ABC Super Portrait Trail", 0x805f0, 0x80610 },
};

const sGame_PaletteDataset AsuraBuster_A_Sittara_A[] =
{
    //  Sittara
    { L"Base A", 0x93a92, 0x93ab2, indexAsuraBuster_Sittara, 0x00 },
    { L"Weapon A", 0x93ab2, 0x93ad2, indexAsuraBuster_Sittara, 0x01 },
    { L"Weapon Effects A", 0x93ad2, 0x93af2, indexAsuraBuster_Sittara, 0x02 },
    { L"Nanami A", 0x93af2, 0x93b12, indexAsuraBuster_Sittara, 0x03 },
    { L"Unused A", 0x93b12, 0x93b32, indexAsuraBuster_Sittara, 0x04 },
    { L"Nanami Lifebar Portrait A", 0x93b32, 0x93b52, indexAsuraBuster_Sittara, 0x05 },

    { L"Sittara A Portrait", 0x8aeec, 0x8af0c },
    { L"Sittara A Super Portrait", 0x80990, 0x809b0 },
    { L"Sittara A Super Portrait Trail", 0x809b0, 0x809d0 },
};

const sGame_PaletteDataset AsuraBuster_A_Sittara_B[] =
{
    { L"Base B", 0x944b2, 0x944d2, indexAsuraBuster_Sittara, 0x00 },
    { L"Weapon B", 0x944d2, 0x944f2, indexAsuraBuster_Sittara, 0x01 },
    { L"Weapon Effects B", 0x944f2, 0x94512, indexAsuraBuster_Sittara, 0x02 },
    { L"Nanami B", 0x94512, 0x94532, indexAsuraBuster_Sittara, 0x03 },
    { L"Unused B", 0x94532, 0x94552, indexAsuraBuster_Sittara, 0x04 },
    { L"Nanami Lifebar Portrait B", 0x94552, 0x94572, indexAsuraBuster_Sittara, 0x05 },

    { L"Sittara B Portrait", 0x8af0c, 0x8af2c },
    { L"Sittara B Super Portrait", 0x809d0, 0x809f0 },
    { L"Sittara B Super Portrait Trail", 0x809f0, 0x80a10 },
};

const sGame_PaletteDataset AsuraBuster_A_Sittara_AB[] =
{
    { L"Base AB", 0x94ed2, 0x94ef2, indexAsuraBuster_Sittara, 0x00 },
    { L"Weapon AB", 0x94ef2, 0x94f12, indexAsuraBuster_Sittara, 0x01 },
    { L"Weapon Effects AB", 0x94f12, 0x94f32, indexAsuraBuster_Sittara, 0x02 },
    { L"Nanami AB", 0x94f32, 0x94f52, indexAsuraBuster_Sittara, 0x03 },
    { L"Unused AB", 0x94f52, 0x94f72, indexAsuraBuster_Sittara, 0x04 },
    { L"Nanami Lifebar Portrait AB", 0x94f72, 0x94f92, indexAsuraBuster_Sittara, 0x05 },

    { L"Sittara AB Portrait", 0x8af2c, 0x8af4c },
    { L"Sittara AB Super Portrait", 0x80a10, 0x80a30 },
    { L"Sittara AB Super Portrait Trail", 0x80a30, 0x80a50 },
};

const sGame_PaletteDataset AsuraBuster_A_Sittara_C[] =
{
    { L"Base C", 0x958f2, 0x95912, indexAsuraBuster_Sittara, 0x00 },
    { L"Weapon C", 0x95912, 0x95932, indexAsuraBuster_Sittara, 0x01 },
    { L"Weapon Effects C", 0x95932, 0x95952, indexAsuraBuster_Sittara, 0x02 },
    { L"Nanami C", 0x95952, 0x95972, indexAsuraBuster_Sittara, 0x03 },
    { L"Unused C", 0x95972, 0x95992, indexAsuraBuster_Sittara, 0x04 },
    { L"Nanami Lifebar Portrait C", 0x95992, 0x959b2, indexAsuraBuster_Sittara, 0x05 },

    { L"Sittara C Portrait", 0x8af4c, 0x8af6c },
    { L"Sittara C Super Portrait", 0x80a50, 0x80a70 },
    { L"Sittara C Super Portrait Trail", 0x80a70, 0x80a90 },
};

const sGame_PaletteDataset AsuraBuster_A_Sittara_AC[] =
{
    { L"Base AC", 0x96312, 0x96332, indexAsuraBuster_Sittara, 0x00 },
    { L"Weapon AC", 0x96332, 0x96352, indexAsuraBuster_Sittara, 0x01 },
    { L"Weapon Effects AC", 0x96352, 0x96372, indexAsuraBuster_Sittara, 0x02 },
    { L"Nanami AC", 0x96372, 0x96392, indexAsuraBuster_Sittara, 0x03 },
    { L"Unused AC", 0x96392, 0x963b2, indexAsuraBuster_Sittara, 0x04 },
    { L"Nanami Lifebar Portrait AC", 0x963b2, 0x963d2, indexAsuraBuster_Sittara, 0x05 },

    { L"Sittara AC Portrait", 0x8af6c, 0x8af8c },
    { L"Sittara AC Super Portrait", 0x80a90, 0x80ab0 },
    { L"Sittara AC Super Portrait Trail", 0x80ab0, 0x80ad0 },
};

const sGame_PaletteDataset AsuraBuster_A_Sittara_BC[] =
{
    { L"Base BC", 0x96d32, 0x96d52, indexAsuraBuster_Sittara, 0x00 },
    { L"Weapon BC", 0x96d52, 0x96d72, indexAsuraBuster_Sittara, 0x01 },
    { L"Weapon Effects BC", 0x96d72, 0x96d92, indexAsuraBuster_Sittara, 0x02 },
    { L"Nanami BC", 0x96d92, 0x96db2, indexAsuraBuster_Sittara, 0x03 },
    { L"Unused BC", 0x96db2, 0x96dd2, indexAsuraBuster_Sittara, 0x04 },
    { L"Nanami Lifebar Portrait BC", 0x96dd2, 0x96df2, indexAsuraBuster_Sittara, 0x05 },

    { L"Sittara BC Portrait", 0x8af8c, 0x8afac },
    { L"Sittara BC Super Portrait", 0x80ad0, 0x80af0 },
    { L"Sittara BC Super Portrait Trail", 0x80af0, 0x80b10 },
};

const sGame_PaletteDataset AsuraBuster_A_Sittara_ABC[] =
{
    { L"Base ABC", 0x97752, 0x97772, indexAsuraBuster_Sittara, 0x00 },
    { L"Weapon ABC", 0x97772, 0x97792, indexAsuraBuster_Sittara, 0x01 },
    { L"Weapon Effects ABC", 0x97792, 0x977b2, indexAsuraBuster_Sittara, 0x02 },
    { L"Nanami ABC", 0x977b2, 0x977d2, indexAsuraBuster_Sittara, 0x03 },
    { L"Unused ABC", 0x977d2, 0x977f2, indexAsuraBuster_Sittara, 0x04 },
    { L"Nanami Lifebar Portrait ABC", 0x977f2, 0x97812, indexAsuraBuster_Sittara, 0x05 },

    { L"Sittara ABC Portrait", 0x8afac, 0x8afcc },
    { L"Sittara ABC Super Portrait", 0x80b10, 0x80b30 },
    { L"Sittara ABC Super Portrait Trail", 0x80b30, 0x80b50 },
};

const sGame_PaletteDataset AsuraBuster_A_Sittara_Shared[] =
{
    //  Sittara Shared
    { L"Sittara Shared Weapon Portrait", 0x8afec, 0x8b00c },
};

const sGame_PaletteDataset AsuraBuster_A_Rokurouta_A[] =
{
    //  Rokurouta
    { L"Base A", 0x93b52, 0x93b72, indexAsuraBuster_Rokurouta, 0x00 },
    { L"Effects 1 A", 0x93b72, 0x93b92, indexAsuraBuster_Rokurouta, 0x01 },
    { L"Effects 2 A", 0x93b92, 0x93bb2, indexAsuraBuster_Rokurouta, 0x02 },
    { L"Effects 3 A", 0x93bb2, 0x93bd2, indexAsuraBuster_Rokurouta, 0x03 },
    { L"Nanami A", 0x93bd2, 0x93bf2, indexAsuraBuster_Rokurouta, 0x04 },
    { L"Nanami Lifebar Portrait A", 0x93bf2, 0x93c12, indexAsuraBuster_Rokurouta, 0x05 },

    { L"Rokurouta A Portrait", 0x8b00c, 0x8b02c },
    { L"Rokurouta A Super Portrait", 0x80b50, 0x80b70 },
    { L"Rokurouta A Super Portrait Trail", 0x80b70, 0x80b90 },
};

const sGame_PaletteDataset AsuraBuster_A_Rokurouta_B[] =
{
    { L"Base B", 0x94572, 0x94592, indexAsuraBuster_Rokurouta, 0x00 },
    { L"Effects 1 B", 0x94592, 0x945b2, indexAsuraBuster_Rokurouta, 0x01 },
    { L"Effects 2 B", 0x945b2, 0x945d2, indexAsuraBuster_Rokurouta, 0x02 },
    { L"Effects 3 B", 0x945d2, 0x945f2, indexAsuraBuster_Rokurouta, 0x03 },
    { L"Nanami B", 0x945f2, 0x94612, indexAsuraBuster_Rokurouta, 0x04 },
    { L"Nanami Lifebar Portrait B", 0x94612, 0x94632, indexAsuraBuster_Rokurouta, 0x05 },

    { L"Rokurouta B Portrait", 0x8b02c, 0x8b04c },
    { L"Rokurouta B Super Portrait", 0x80b90, 0x80bb0 },
    { L"Rokurouta B Super Portrait Trail", 0x80bb0, 0x80bd0 },
};

const sGame_PaletteDataset AsuraBuster_A_Rokurouta_AB[] =
{
    { L"Base AB", 0x94f92, 0x94fb2, indexAsuraBuster_Rokurouta, 0x00 },
    { L"Effects 1 AB", 0x94fb2, 0x94fd2, indexAsuraBuster_Rokurouta, 0x01 },
    { L"Effects 2 AB", 0x94fd2, 0x94ff2, indexAsuraBuster_Rokurouta, 0x02 },
    { L"Effects 3 AB", 0x94ff2, 0x95012, indexAsuraBuster_Rokurouta, 0x03 },
    { L"Nanami AB", 0x95012, 0x95032, indexAsuraBuster_Rokurouta, 0x04 },
    { L"Nanami Lifebar Portrait AB", 0x95032, 0x95052, indexAsuraBuster_Rokurouta, 0x05 },

    { L"Rokurouta AB Portrait", 0x8b04c, 0x8b06c },
    { L"Rokurouta AB Super Portrait", 0x80bd0, 0x80bf0 },
    { L"Rokurouta AB Super Portrait Trail", 0x80bf0, 0x80c10 },
};

const sGame_PaletteDataset AsuraBuster_A_Rokurouta_C[] =
{
    { L"Base C", 0x959b2, 0x959d2, indexAsuraBuster_Rokurouta, 0x00 },
    { L"Effects 1 C", 0x959d2, 0x959f2, indexAsuraBuster_Rokurouta, 0x01 },
    { L"Effects 2 C", 0x959f2, 0x95a12, indexAsuraBuster_Rokurouta, 0x02 },
    { L"Effects 3 C", 0x95a12, 0x95a32, indexAsuraBuster_Rokurouta, 0x03 },
    { L"Nanami C", 0x95a32, 0x95a52, indexAsuraBuster_Rokurouta, 0x04 },
    { L"Nanami Lifebar Portrait C", 0x95a52, 0x95a72, indexAsuraBuster_Rokurouta, 0x05 },

    { L"Rokurouta C Portrait", 0x8b06c, 0x8b08c },
    { L"Rokurouta C Super Portrait", 0x80c10, 0x80c30 },
    { L"Rokurouta C Super Portrait Trail", 0x80c30, 0x80c50 },
};

const sGame_PaletteDataset AsuraBuster_A_Rokurouta_AC[] =
{
    { L"Base AC", 0x963d2, 0x963f2, indexAsuraBuster_Rokurouta, 0x00 },
    { L"Effects 1 AC", 0x963f2, 0x96412, indexAsuraBuster_Rokurouta, 0x01 },
    { L"Effects 2 AC", 0x96412, 0x96432, indexAsuraBuster_Rokurouta, 0x02 },
    { L"Effects 3 AC", 0x96432, 0x96452, indexAsuraBuster_Rokurouta, 0x03 },
    { L"Nanami AC", 0x96452, 0x96472, indexAsuraBuster_Rokurouta, 0x04 },
    { L"Nanami Lifebar Portrait AC", 0x96472, 0x96492, indexAsuraBuster_Rokurouta, 0x05 },

    { L"Rokurouta AC Portrait", 0x8b08c, 0x8b0ac },
    { L"Rokurouta AC Super Portrait", 0x80c50, 0x80c70 },
    { L"Rokurouta AC Super Portrait Trail", 0x80c70, 0x80c90 },
};

const sGame_PaletteDataset AsuraBuster_A_Rokurouta_BC[] =
{
    { L"Base BC", 0x96df2, 0x96e12, indexAsuraBuster_Rokurouta, 0x00 },
    { L"Effects 1 BC", 0x96e12, 0x96e32, indexAsuraBuster_Rokurouta, 0x01 },
    { L"Effects 2 BC", 0x96e32, 0x96e52, indexAsuraBuster_Rokurouta, 0x02 },
    { L"Effects 3 BC", 0x96e52, 0x96e72, indexAsuraBuster_Rokurouta, 0x03 },
    { L"Nanami BC", 0x96e72, 0x96e92, indexAsuraBuster_Rokurouta, 0x04 },
    { L"Nanami Lifebar Portrait BC", 0x96e92, 0x96eb2, indexAsuraBuster_Rokurouta, 0x05 },

    { L"Rokurouta BC Portrait", 0x8b0ac, 0x8b0cc },
    { L"Rokurouta BC Super Portrait", 0x80c90, 0x80cb0 },
    { L"Rokurouta BC Super Portrait Trail", 0x80cb0, 0x80cd0 },
};

const sGame_PaletteDataset AsuraBuster_A_Rokurouta_ABC[] =
{
    { L"Base ABC", 0x97812, 0x97832, indexAsuraBuster_Rokurouta, 0x00 },
    { L"Effects 1 ABC", 0x97832, 0x97852, indexAsuraBuster_Rokurouta, 0x01 },
    { L"Effects 2 ABC", 0x97852, 0x97872, indexAsuraBuster_Rokurouta, 0x02 },
    { L"Effects 3 ABC", 0x97872, 0x97892, indexAsuraBuster_Rokurouta, 0x03 },
    { L"Nanami ABC", 0x97892, 0x978b2, indexAsuraBuster_Rokurouta, 0x04 },
    { L"Nanami Lifebar Portrait ABC", 0x978b2, 0x978d2, indexAsuraBuster_Rokurouta, 0x05 },

    { L"Rokurouta ABC Portrait", 0x8b0cc, 0x8b0ec },
    { L"Rokurouta ABC Super Portrait", 0x80cd0, 0x80cf0 },
    { L"Rokurouta ABC Super Portrait Trail", 0x80cf0, 0x80d10 },
};

const sGame_PaletteDataset AsuraBuster_A_Zinsuke_A[] =
{
    //  Zinsuke
    { L"Base A", 0x93c12, 0x93c32, indexAsuraBuster_Zinsuke, 0x00 },
    { L"Hat and Weapon A", 0x93c32, 0x93c52, indexAsuraBuster_Zinsuke, 0x01 },
    { L"Nanami A", 0x93c52, 0x93c72, indexAsuraBuster_Zinsuke, 0x02 },
    { L"Unused A", 0x93c72, 0x93c92, indexAsuraBuster_Zinsuke, 0x03 },
    { L"Unused A", 0x93c92, 0x93cb2, indexAsuraBuster_Zinsuke, 0x04 },
    { L"Nanami Lifebar Portrait A", 0x93cb2, 0x93cd2, indexAsuraBuster_Zinsuke, 0x05 },

    { L"Zinsuke A Portrait", 0x8b10c, 0x8b12c },
    { L"Zinsuke A Super Portrait", 0x80d10, 0x80d30 },
    { L"Zinsuke A Super Portrait Trail", 0x80d30, 0x80d50 },
};

const sGame_PaletteDataset AsuraBuster_A_Zinsuke_B[] =
{
    { L"Base B", 0x94632, 0x94652, indexAsuraBuster_Zinsuke, 0x00 },
    { L"Hat and Weapon B", 0x94652, 0x94672, indexAsuraBuster_Zinsuke, 0x01 },
    { L"Nanami B", 0x94672, 0x94692, indexAsuraBuster_Zinsuke, 0x02 },
    { L"Unused B", 0x94692, 0x946b2, indexAsuraBuster_Zinsuke, 0x03 },
    { L"Unused B", 0x946b2, 0x946d2, indexAsuraBuster_Zinsuke, 0x04 },
    { L"Nanami Lifebar Portrait B", 0x946d2, 0x946f2, indexAsuraBuster_Zinsuke, 0x05 },

    { L"Zinsuke B Portrait", 0x8b12c, 0x8b14c },
    { L"Zinsuke B Super Portrait", 0x80d50, 0x80d70 },
    { L"Zinsuke B Super Portrait Trail", 0x80d70, 0x80d90 },
};

const sGame_PaletteDataset AsuraBuster_A_Zinsuke_AB[] =
{
    { L"Base AB", 0x95052, 0x95072, indexAsuraBuster_Zinsuke, 0x00 },
    { L"Hat and Weapon AB", 0x95072, 0x95092, indexAsuraBuster_Zinsuke, 0x01 },
    { L"Nanami AB", 0x95092, 0x950b2, indexAsuraBuster_Zinsuke, 0x02 },
    { L"Unused AB", 0x950b2, 0x950d2, indexAsuraBuster_Zinsuke, 0x03 },
    { L"Unused AB", 0x950d2, 0x950f2, indexAsuraBuster_Zinsuke, 0x04 },
    { L"Nanami Lifebar Portrait AB", 0x950f2, 0x95112, indexAsuraBuster_Zinsuke, 0x05 },

    { L"Zinsuke AB Portrait", 0x8b14c, 0x8b16c },
    { L"Zinsuke AB Super Portrait", 0x80d90, 0x80db0 },
    { L"Zinsuke AB Super Portrait Trail", 0x80db0, 0x80dd0 },
};

const sGame_PaletteDataset AsuraBuster_A_Zinsuke_C[] =
{
    { L"Base C", 0x95a72, 0x95a92, indexAsuraBuster_Zinsuke, 0x00 },
    { L"Hat and Weapon C", 0x95a92, 0x95ab2, indexAsuraBuster_Zinsuke, 0x01 },
    { L"Nanami C", 0x95ab2, 0x95ad2, indexAsuraBuster_Zinsuke, 0x02 },
    { L"Unused C", 0x95ad2, 0x95af2, indexAsuraBuster_Zinsuke, 0x03 },
    { L"Unused C", 0x95af2, 0x95b12, indexAsuraBuster_Zinsuke, 0x04 },
    { L"Nanami Lifebar Portrait C", 0x95b12, 0x95b32, indexAsuraBuster_Zinsuke, 0x05 },

    { L"Zinsuke C Portrait", 0x8b16c, 0x8b18c },
    { L"Zinsuke C Super Portrait", 0x80dd0, 0x80df0 },
    { L"Zinsuke C Super Portrait Trail", 0x80df0, 0x80e10 },
};

const sGame_PaletteDataset AsuraBuster_A_Zinsuke_AC[] =
{
    { L"Base AC", 0x96492, 0x964b2, indexAsuraBuster_Zinsuke, 0x00 },
    { L"Hat and Weapon AC", 0x964b2, 0x964d2, indexAsuraBuster_Zinsuke, 0x01 },
    { L"Nanami AC", 0x964d2, 0x964f2, indexAsuraBuster_Zinsuke, 0x02 },
    { L"Unused AC", 0x964f2, 0x96512, indexAsuraBuster_Zinsuke, 0x03 },
    { L"Unused AC", 0x96512, 0x96532, indexAsuraBuster_Zinsuke, 0x04 },
    { L"Nanami Lifebar Portrait AC", 0x96532, 0x96552, indexAsuraBuster_Zinsuke, 0x05 },

    { L"Zinsuke AC Portrait", 0x8b18c, 0x8b1ac },
    { L"Zinsuke AC Super Portrait", 0x80e10, 0x80e30 },
    { L"Zinsuke AC Super Portrait Trail", 0x80e30, 0x80e50 },
};

const sGame_PaletteDataset AsuraBuster_A_Zinsuke_BC[] =
{
    { L"Base BC", 0x96eb2, 0x96ed2, indexAsuraBuster_Zinsuke, 0x00 },
    { L"Hat and Weapon BC", 0x96ed2, 0x96ef2, indexAsuraBuster_Zinsuke, 0x01 },
    { L"Nanami BC", 0x96ef2, 0x96f12, indexAsuraBuster_Zinsuke, 0x02 },
    { L"Unused BC", 0x96f12, 0x96f32, indexAsuraBuster_Zinsuke, 0x03 },
    { L"Unused BC", 0x96f32, 0x96f52, indexAsuraBuster_Zinsuke, 0x04 },
    { L"Nanami Lifebar Portrait BC", 0x96f52, 0x96f72, indexAsuraBuster_Zinsuke, 0x05 },

    { L"Zinsuke BC Portrait", 0x8b1ac, 0x8b1cc },
    { L"Zinsuke BC Super Portrait", 0x80e50, 0x80e70 },
    { L"Zinsuke BC Super Portrait Trail", 0x80e70, 0x80e90 },
};

const sGame_PaletteDataset AsuraBuster_A_Zinsuke_ABC[] =
{
    { L"Base ABC", 0x978d2, 0x978f2, indexAsuraBuster_Zinsuke, 0x00 },
    { L"Hat and Weapon ABC", 0x978f2, 0x97912, indexAsuraBuster_Zinsuke, 0x01 },
    { L"Nanami ABC", 0x97912, 0x97932, indexAsuraBuster_Zinsuke, 0x02 },
    { L"Unused ABC", 0x97932, 0x97952, indexAsuraBuster_Zinsuke, 0x03 },
    { L"Unused ABC", 0x97952, 0x97972, indexAsuraBuster_Zinsuke, 0x04 },
    { L"Nanami Lifebar Portrait ABC", 0x97972, 0x97992, indexAsuraBuster_Zinsuke, 0x05 },

    { L"Zinsuke ABC Portrait", 0x8b1cc, 0x8b1ec },
    { L"Zinsuke ABC Super Portrait", 0x80e90, 0x80eb0 },
    { L"Zinsuke ABC Super Portrait Trail", 0x80eb0, 0x80ed0 },
};

const sGame_PaletteDataset AsuraBuster_A_Zinsuke_Shared[] =
{
    //  Zinsuke Shared
    { L"Zinsuke Shared Hat Portrait", 0x8b20c, 0x8b22c },
};

const sGame_PaletteDataset AsuraBuster_A_AliceBang_A[] =
{
    //  Alice!
    { L"Base A", 0x93cd2, 0x93cf2, indexAsuraBuster_AliceBang, 0x00 },
    { L"Effects 1 A", 0x93cf2, 0x93d12, indexAsuraBuster_AliceBang, 0x01 },
    { L"Effects 2 A", 0x93d12, 0x93d32, indexAsuraBuster_AliceBang, 0x02 },
    { L"Effects 3 A", 0x93d32, 0x93d52, indexAsuraBuster_AliceBang, 0x03 },
    { L"Effects 4 A", 0x93d52, 0x93d72, indexAsuraBuster_AliceBang, 0x04 },
    { L"Nanami A", 0x93d72, 0x93d92, indexAsuraBuster_AliceBang, 0x05 },

    { L"Alice! A Portrait", 0x8b22c, 0x8b24c },
    { L"Alice! A Super Portrait", 0x80ed0, 0x80ef0 },
    { L"Alice! A Super Portrait Trail", 0x80ef0, 0x80f10 },
};

const sGame_PaletteDataset AsuraBuster_A_AliceBang_B[] =
{
    { L"Base B", 0x946f2, 0x94712, indexAsuraBuster_AliceBang, 0x00 },
    { L"Effects 1 B", 0x94712, 0x94732, indexAsuraBuster_AliceBang, 0x01 },
    { L"Effects 2 B", 0x94732, 0x94752, indexAsuraBuster_AliceBang, 0x02 },
    { L"Effects 3 B", 0x94752, 0x94772, indexAsuraBuster_AliceBang, 0x03 },
    { L"Effects 4 B", 0x94772, 0x94792, indexAsuraBuster_AliceBang, 0x04 },
    { L"Nanami B", 0x94792, 0x947b2, indexAsuraBuster_AliceBang, 0x05 },

    { L"Alice! B Portrait", 0x8b24c, 0x8b26c },
    { L"Alice! B Super Portrait", 0x80f10, 0x80f30 },
    { L"Alice! B Super Portrait Trail", 0x80f30, 0x80f50 },
};

const sGame_PaletteDataset AsuraBuster_A_AliceBang_AB[] =
{
    { L"Base AB", 0x95112, 0x95132, indexAsuraBuster_AliceBang, 0x00 },
    { L"Effects 1 AB", 0x95132, 0x95152, indexAsuraBuster_AliceBang, 0x01 },
    { L"Effects 2 AB", 0x95152, 0x95172, indexAsuraBuster_AliceBang, 0x02 },
    { L"Effects 3 AB", 0x95172, 0x95192, indexAsuraBuster_AliceBang, 0x03 },
    { L"Effects 4 AB", 0x95192, 0x951b2, indexAsuraBuster_AliceBang, 0x04 },
    { L"Nanami AB", 0x951b2, 0x951d2, indexAsuraBuster_AliceBang, 0x05 },

    { L"Alice! AB Portrait", 0x8b26c, 0x8b28c },
    { L"Alice! AB Super Portrait", 0x80f50, 0x80f70 },
    { L"Alice! AB Super Portrait Trail", 0x80f70, 0x80f90 },
};

const sGame_PaletteDataset AsuraBuster_A_AliceBang_C[] =
{
    { L"Base C", 0x95b32, 0x95b52, indexAsuraBuster_AliceBang, 0x00 },
    { L"Effects 1 C", 0x95b52, 0x95b72, indexAsuraBuster_AliceBang, 0x01 },
    { L"Effects 2 C", 0x95b72, 0x95b92, indexAsuraBuster_AliceBang, 0x02 },
    { L"Effects 3 C", 0x95b92, 0x95bb2, indexAsuraBuster_AliceBang, 0x03 },
    { L"Effects 4 C", 0x95bb2, 0x95bd2, indexAsuraBuster_AliceBang, 0x04 },
    { L"Nanami C", 0x95bd2, 0x95bf2, indexAsuraBuster_AliceBang, 0x05 },

    { L"Alice! C Portrait", 0x8b28c, 0x8b2ac },
    { L"Alice! C Super Portrait", 0x80f90, 0x80fb0 },
    { L"Alice! C Super Portrait Trail", 0x80fb0, 0x80fd0 },
};

const sGame_PaletteDataset AsuraBuster_A_AliceBang_AC[] =
{
    { L"Base AC", 0x96552, 0x96572, indexAsuraBuster_AliceBang, 0x00 },
    { L"Effects 1 AC", 0x96572, 0x96592, indexAsuraBuster_AliceBang, 0x01 },
    { L"Effects 2 AC", 0x96592, 0x965b2, indexAsuraBuster_AliceBang, 0x02 },
    { L"Effects 3 AC", 0x965b2, 0x965d2, indexAsuraBuster_AliceBang, 0x03 },
    { L"Effects 4 AC", 0x965d2, 0x965f2, indexAsuraBuster_AliceBang, 0x04 },
    { L"Nanami AC", 0x965f2, 0x96612, indexAsuraBuster_AliceBang, 0x05 },

    { L"Alice! AC Portrait", 0x8b2ac, 0x8b2cc },
    { L"Alice! AC Super Portrait", 0x80fd0, 0x80ff0 },
    { L"Alice! AC Super Portrait Trail", 0x80ff0, 0x81010 },
};

const sGame_PaletteDataset AsuraBuster_A_AliceBang_BC[] =
{
    { L"Base BC", 0x96f72, 0x96f92, indexAsuraBuster_AliceBang, 0x00 },
    { L"Effects 1 BC", 0x96f92, 0x96fb2, indexAsuraBuster_AliceBang, 0x01 },
    { L"Effects 2 BC", 0x96fb2, 0x96fd2, indexAsuraBuster_AliceBang, 0x02 },
    { L"Effects 3 BC", 0x96fd2, 0x96ff2, indexAsuraBuster_AliceBang, 0x03 },
    { L"Effects 4 BC", 0x96ff2, 0x97012, indexAsuraBuster_AliceBang, 0x04 },
    { L"Nanami BC", 0x97012, 0x97032, indexAsuraBuster_AliceBang, 0x05 },

    { L"Alice! BC Portrait", 0x8b2cc, 0x8b2ec },
    { L"Alice! BC Super Portrait", 0x81010, 0x81030 },
    { L"Alice! BC Super Portrait Trail", 0x81030, 0x81050 },
};

const sGame_PaletteDataset AsuraBuster_A_AliceBang_ABC[] =
{
    { L"Base ABC", 0x97992, 0x979b2, indexAsuraBuster_AliceBang, 0x00 },
    { L"Effects 1 ABC", 0x979b2, 0x979d2, indexAsuraBuster_AliceBang, 0x01 },
    { L"Effects 2 ABC", 0x979d2, 0x979f2, indexAsuraBuster_AliceBang, 0x02 },
    { L"Effects 3 ABC", 0x979f2, 0x97a12, indexAsuraBuster_AliceBang, 0x03 },
    { L"Effects 4 ABC", 0x97a12, 0x97a32, indexAsuraBuster_AliceBang, 0x04 },
    { L"Nanami ABC", 0x97a32, 0x97a52, indexAsuraBuster_AliceBang, 0x05 },

    { L"Alice! ABC Portrait", 0x8b2ec, 0x8b30c },
    { L"Alice! ABC Super Portrait", 0x81050, 0x81070 },
    { L"Alice! ABC Super Portrait Trail", 0x81070, 0x81090 },
};

const sGame_PaletteDataset AsuraBuster_A_Vebel_A[] =
{
    { L"Base A", 0x93db2, 0x93dd2, indexAsuraBuster_Vebel, 0x00 },
    { L"Effects 1 A", 0x93dd2, 0x93df2, indexAsuraBuster_Vebel, 0x01 },
    { L"Unused A", 0x93df2, 0x93e12, indexAsuraBuster_Vebel, 0x02 },
    { L"Unused A", 0x93e12, 0x93e32, indexAsuraBuster_Vebel, 0x03 },
    { L"Unused A", 0x93e32, 0x93e52, indexAsuraBuster_Vebel, 0x04 },
    { L"Nanami A", 0x93e52, 0x93e72, indexAsuraBuster_Vebel, 0x05 },
};

const sGame_PaletteDataset AsuraBuster_A_Vebel_B[] =
{
    { L"Base B", 0x947d2, 0x947f2, indexAsuraBuster_Vebel, 0x00 },
    { L"Effects 1 B", 0x947f2, 0x94812, indexAsuraBuster_Vebel, 0x01 },
    { L"Unused B", 0x94812, 0x94832, indexAsuraBuster_Vebel, 0x02 },
    { L"Unused B", 0x94832, 0x94852, indexAsuraBuster_Vebel, 0x03 },
    { L"Unused B", 0x94852, 0x94872, indexAsuraBuster_Vebel, 0x04 },
    { L"Nanami B", 0x94872, 0x94892, indexAsuraBuster_Vebel, 0x05 },
};

const sGame_PaletteDataset AsuraBuster_A_Vebel_AB[] =
{
    { L"Base AB", 0x951f2, 0x95212, indexAsuraBuster_Vebel, 0x00 },
    { L"Effects 1 AB", 0x95212, 0x95232, indexAsuraBuster_Vebel, 0x01 },
    { L"Unused AB", 0x95232, 0x95252, indexAsuraBuster_Vebel, 0x02 },
    { L"Unused AB", 0x95252, 0x95272, indexAsuraBuster_Vebel, 0x03 },
    { L"Unused AB", 0x95272, 0x95292, indexAsuraBuster_Vebel, 0x04 },
    { L"Nanami AB", 0x95292, 0x952b2, indexAsuraBuster_Vebel, 0x05 },
};

const sGame_PaletteDataset AsuraBuster_A_Vebel_C[] =
{
    { L"Base C", 0x95c12, 0x95c32, indexAsuraBuster_Vebel, 0x00 },
    { L"Effects 1 C", 0x95c32, 0x95c52, indexAsuraBuster_Vebel, 0x01 },
    { L"Unused C", 0x95c52, 0x95c72, indexAsuraBuster_Vebel, 0x02 },
    { L"Unused C", 0x95c72, 0x95c92, indexAsuraBuster_Vebel, 0x03 },
    { L"Unused C", 0x95c92, 0x95cb2, indexAsuraBuster_Vebel, 0x04 },
    { L"Nanami C", 0x95cb2, 0x95cd2, indexAsuraBuster_Vebel, 0x05 },
};

const sGame_PaletteDataset AsuraBuster_A_Vebel_AC[] =
{
    { L"Base AC", 0x96632, 0x96652, indexAsuraBuster_Vebel, 0x00 },
    { L"Effects 1 AC", 0x96652, 0x96672, indexAsuraBuster_Vebel, 0x01 },
    { L"Unused AC", 0x96672, 0x96692, indexAsuraBuster_Vebel, 0x02 },
    { L"Unused AC", 0x96692, 0x966b2, indexAsuraBuster_Vebel, 0x03 },
    { L"Unused AC", 0x966b2, 0x966d2, indexAsuraBuster_Vebel, 0x04 },
    { L"Nanami AC", 0x966d2, 0x966f2, indexAsuraBuster_Vebel, 0x05 },
};

const sGame_PaletteDataset AsuraBuster_A_Vebel_BC[] =
{
    { L"Base BC", 0x97052, 0x97072, indexAsuraBuster_Vebel, 0x00 },
    { L"Effects 1 BC", 0x97072, 0x97092, indexAsuraBuster_Vebel, 0x01 },
    { L"Unused BC", 0x97092, 0x970b2, indexAsuraBuster_Vebel, 0x02 },
    { L"Unused BC", 0x970b2, 0x970d2, indexAsuraBuster_Vebel, 0x03 },
    { L"Unused BC", 0x970d2, 0x970f2, indexAsuraBuster_Vebel, 0x04 },
    { L"Nanami BC", 0x970f2, 0x97112, indexAsuraBuster_Vebel, 0x05 },
};

const sGame_PaletteDataset AsuraBuster_A_Vebel_ABC[] =
{
    { L"Base ABC", 0x97a72, 0x97a92, indexAsuraBuster_Vebel, 0x00 },
    { L"Effects 1 ABC", 0x97a92, 0x97ab2, indexAsuraBuster_Vebel, 0x01 },
    { L"Unused ABC", 0x97ab2, 0x97ad2, indexAsuraBuster_Vebel, 0x02 },
    { L"Unused ABC", 0x97ad2, 0x97af2, indexAsuraBuster_Vebel, 0x03 },
    { L"Unused ABC", 0x97af2, 0x97b12, indexAsuraBuster_Vebel, 0x04 },
    { L"Nanami ABC", 0x97b12, 0x97b32, indexAsuraBuster_Vebel, 0x05 },
};

const sGame_PaletteDataset AsuraBuster_A_Nanami_PORTRAITS[] =
{
    { L"Nanami A Portrait", 0x8b32c, 0x8b34c },
    { L"Nanami B Portrait", 0x8b34c, 0x8b36c },
    { L"Nanami AB Portrait", 0x8b36c, 0x8b38c },
    { L"Nanami C Portrait", 0x8b38c, 0x8b3ac },
    { L"Nanami AC Portrait", 0x8b3ac, 0x8b3cc },
    { L"Nanami BC Portrait", 0x8b3cc, 0x8b3ec },
    { L"Nanami ABC Portrait", 0x8b3ec, 0x8b40c },
};

const sGame_PaletteDataset AsuraBuster_A_Stages[] =
{
    { L"Yashaou/Sittara Stage (1/2) 0x4eaf0", 0x4eaf0, 0x4ecf0 },
    { L"Yashaou/Sittara Stage (2/2) 0x4ecf0", 0x4ecf0, 0x4eef0 },
    { L"Yashaou/Sittara Stage Extras 1", 0x4ea50, 0x4eaf0 },
    { L"Yashaou/Sittara Stage Extras 2", 0x4eef0, 0x4ef10 },
    { L"Goat Stage (1/2) 0x4d930", 0x4d930, 0x4db30 },
    { L"Goat Stage (2/2) 0x4db30", 0x4db30, 0x4dd30 },
    { L"Goat Stage Extra", 0x4dd30, 0x4dd50 },
    { L"Leon/Rose Mary Stage (1/2) 0x4e210", 0x4e210, 0x4e410 },
    { L"Leon/Rose Mary Stage (2/2) 0x4e410", 0x4e410, 0x4e610 },
    { L"Leon/Rose Mary Stage Extra 1", 0x4e190, 0x4e210 },
    { L"Leon/Rose Mary Stage Extra 2", 0x4e610, 0x4e630 },
    { L"Chen-Mao Stage (1/2) 0x4dd50", 0x4dd50, 0x4df50 },
    { L"Chen-Mao Stage (2/2) 0x4df50", 0x4df50, 0x4e150 },
    { L"Chen-Mao Extra", 0x4e150, 0x4e170 },
    { L"Zam-B/Rokurouta Stage (1/2) 0x4c850", 0x4c850, 0x4ca50 },
    { L"Zam-B/Rokurouta Stage (2/2) 0x4ca50", 0x4ca50, 0x4cc50 },
    { L"Zam-B/Rokurouta Stage Extra", 0x4cc50, 0x4ccd0 },
    { L"Taros Stage (1/2) 0x4e630", 0x4e630, 0x4e830 },
    { L"Taros Stage (2/2) 0x4e830", 0x4e830, 0x4ea30 },
    { L"Taros Stage Extra", 0x4ea30, 0x4ea50 },
    { L"Alice Stage (1/2) 0x4c350", 0x4c350, 0x4c550 },
    { L"Alice Stage (2/2) 0x4c550", 0x4c550, 0x4c750 },
    { L"Alice Stage Extra", 0x4c750, 0x4c7b0 },
    { L"Zinsuke Stage (1/2) 0x4d510", 0x4d510, 0x4d710 },
    { L"Zinsuke Stage (2/2) 0x4d710", 0x4d710, 0x4d910 },
    { L"Zinsuke Stage Extra", 0x4d910, 0x4d930 },
    { L"Vebel Stage (1/2) 0x4ef10", 0x4ef10, 0x4f110 },
    { L"Vebel Stage (2/2) 0x4f110", 0x4f110, 0x4f310 },
    { L"Vebel Stage Extra", 0x4f310, 0x4f350 },
    { L"Story Mode Stage 1 (1/2) 0x4d0f0", 0x4d0f0, 0x4d2f0 },
    { L"Story Mode Stage 1 (2/2) 0x4d2f0", 0x4d2f0, 0x4d4f0 },
    { L"Story Mode Stage 1 Extra", 0x4d4f0, 0x4d510 },
    { L"Bonus Game Stage (1/2) 0x4f530", 0x4f530, 0x4f730 },
    { L"Bonus Game Stage (2/2) 0x4f730", 0x4f730, 0x4f930 },
    { L"Bonus Game Stage Extra", 0x4f930, 0x4f950 },
    { L"King Stage", 0x4fe50, 0x50050 },
    { L"King Stage Extra", 0x50050, 0x50070 },
};

const sDescTreeNode AsuraBuster_A_Yashaou_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Yashaou_A, ARRAYSIZE(AsuraBuster_A_Yashaou_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Yashaou_B, ARRAYSIZE(AsuraBuster_A_Yashaou_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Yashaou_AB, ARRAYSIZE(AsuraBuster_A_Yashaou_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Yashaou_C, ARRAYSIZE(AsuraBuster_A_Yashaou_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Yashaou_AC, ARRAYSIZE(AsuraBuster_A_Yashaou_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Yashaou_BC, ARRAYSIZE(AsuraBuster_A_Yashaou_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Yashaou_ABC, ARRAYSIZE(AsuraBuster_A_Yashaou_ABC) },
    { L"Shared", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Yashaou_Shared, ARRAYSIZE(AsuraBuster_A_Yashaou_Shared) },
};

const sDescTreeNode AsuraBuster_A_Goat_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Goat_A, ARRAYSIZE(AsuraBuster_A_Goat_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Goat_B, ARRAYSIZE(AsuraBuster_A_Goat_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Goat_AB, ARRAYSIZE(AsuraBuster_A_Goat_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Goat_C, ARRAYSIZE(AsuraBuster_A_Goat_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Goat_AC, ARRAYSIZE(AsuraBuster_A_Goat_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Goat_BC, ARRAYSIZE(AsuraBuster_A_Goat_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Goat_ABC, ARRAYSIZE(AsuraBuster_A_Goat_ABC) },
    { L"Shared", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Goat_Shared, ARRAYSIZE(AsuraBuster_A_Goat_Shared) },
};

const sDescTreeNode AsuraBuster_A_Leon_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Leon_A, ARRAYSIZE(AsuraBuster_A_Leon_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Leon_B, ARRAYSIZE(AsuraBuster_A_Leon_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Leon_AB, ARRAYSIZE(AsuraBuster_A_Leon_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Leon_C, ARRAYSIZE(AsuraBuster_A_Leon_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Leon_AC, ARRAYSIZE(AsuraBuster_A_Leon_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Leon_BC, ARRAYSIZE(AsuraBuster_A_Leon_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Leon_ABC, ARRAYSIZE(AsuraBuster_A_Leon_ABC) },
};

const sDescTreeNode AsuraBuster_A_ChenMao_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ChenMao_A, ARRAYSIZE(AsuraBuster_A_ChenMao_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ChenMao_B, ARRAYSIZE(AsuraBuster_A_ChenMao_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ChenMao_AB, ARRAYSIZE(AsuraBuster_A_ChenMao_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ChenMao_C, ARRAYSIZE(AsuraBuster_A_ChenMao_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ChenMao_AC, ARRAYSIZE(AsuraBuster_A_ChenMao_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ChenMao_BC, ARRAYSIZE(AsuraBuster_A_ChenMao_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ChenMao_ABC, ARRAYSIZE(AsuraBuster_A_ChenMao_ABC) },
};

const sDescTreeNode AsuraBuster_A_ZamB_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ZamB_A, ARRAYSIZE(AsuraBuster_A_ZamB_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ZamB_B, ARRAYSIZE(AsuraBuster_A_ZamB_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ZamB_AB, ARRAYSIZE(AsuraBuster_A_ZamB_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ZamB_C, ARRAYSIZE(AsuraBuster_A_ZamB_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ZamB_AC, ARRAYSIZE(AsuraBuster_A_ZamB_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ZamB_BC, ARRAYSIZE(AsuraBuster_A_ZamB_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ZamB_ABC, ARRAYSIZE(AsuraBuster_A_ZamB_ABC) },
};

const sDescTreeNode AsuraBuster_A_Taros_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Taros_A, ARRAYSIZE(AsuraBuster_A_Taros_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Taros_B, ARRAYSIZE(AsuraBuster_A_Taros_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Taros_AB, ARRAYSIZE(AsuraBuster_A_Taros_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Taros_C, ARRAYSIZE(AsuraBuster_A_Taros_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Taros_AC, ARRAYSIZE(AsuraBuster_A_Taros_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Taros_BC, ARRAYSIZE(AsuraBuster_A_Taros_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Taros_ABC, ARRAYSIZE(AsuraBuster_A_Taros_ABC) },
};

const sDescTreeNode AsuraBuster_A_Alice_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Alice_A, ARRAYSIZE(AsuraBuster_A_Alice_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Alice_B, ARRAYSIZE(AsuraBuster_A_Alice_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Alice_AB, ARRAYSIZE(AsuraBuster_A_Alice_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Alice_C, ARRAYSIZE(AsuraBuster_A_Alice_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Alice_AC, ARRAYSIZE(AsuraBuster_A_Alice_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Alice_BC, ARRAYSIZE(AsuraBuster_A_Alice_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Alice_ABC, ARRAYSIZE(AsuraBuster_A_Alice_ABC) },
};

const sDescTreeNode AsuraBuster_A_AliceBang_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_AliceBang_A, ARRAYSIZE(AsuraBuster_A_AliceBang_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_AliceBang_B, ARRAYSIZE(AsuraBuster_A_AliceBang_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_AliceBang_AB, ARRAYSIZE(AsuraBuster_A_AliceBang_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_AliceBang_C, ARRAYSIZE(AsuraBuster_A_AliceBang_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_AliceBang_AC, ARRAYSIZE(AsuraBuster_A_AliceBang_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_AliceBang_BC, ARRAYSIZE(AsuraBuster_A_AliceBang_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_AliceBang_ABC, ARRAYSIZE(AsuraBuster_A_AliceBang_ABC) },
};

const sDescTreeNode AsuraBuster_A_RoseMary_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_RoseMary_A, ARRAYSIZE(AsuraBuster_A_RoseMary_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_RoseMary_B, ARRAYSIZE(AsuraBuster_A_RoseMary_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_RoseMary_AB, ARRAYSIZE(AsuraBuster_A_RoseMary_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_RoseMary_C, ARRAYSIZE(AsuraBuster_A_RoseMary_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_RoseMary_AC, ARRAYSIZE(AsuraBuster_A_RoseMary_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_RoseMary_BC, ARRAYSIZE(AsuraBuster_A_RoseMary_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_RoseMary_ABC, ARRAYSIZE(AsuraBuster_A_RoseMary_ABC) },
};

const sDescTreeNode AsuraBuster_A_Sittara_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Sittara_A, ARRAYSIZE(AsuraBuster_A_Sittara_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Sittara_B, ARRAYSIZE(AsuraBuster_A_Sittara_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Sittara_AB, ARRAYSIZE(AsuraBuster_A_Sittara_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Sittara_C, ARRAYSIZE(AsuraBuster_A_Sittara_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Sittara_AC, ARRAYSIZE(AsuraBuster_A_Sittara_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Sittara_BC, ARRAYSIZE(AsuraBuster_A_Sittara_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Sittara_ABC, ARRAYSIZE(AsuraBuster_A_Sittara_ABC) },
    { L"Shared", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Sittara_Shared, ARRAYSIZE(AsuraBuster_A_Sittara_Shared) },
};

const sDescTreeNode AsuraBuster_A_Rokurouta_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Rokurouta_A, ARRAYSIZE(AsuraBuster_A_Rokurouta_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Rokurouta_B, ARRAYSIZE(AsuraBuster_A_Rokurouta_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Rokurouta_AB, ARRAYSIZE(AsuraBuster_A_Rokurouta_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Rokurouta_C, ARRAYSIZE(AsuraBuster_A_Rokurouta_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Rokurouta_AC, ARRAYSIZE(AsuraBuster_A_Rokurouta_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Rokurouta_BC, ARRAYSIZE(AsuraBuster_A_Rokurouta_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Rokurouta_ABC, ARRAYSIZE(AsuraBuster_A_Rokurouta_ABC) },
};

const sDescTreeNode AsuraBuster_A_Zinsuke_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Zinsuke_A, ARRAYSIZE(AsuraBuster_A_Zinsuke_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Zinsuke_B, ARRAYSIZE(AsuraBuster_A_Zinsuke_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Zinsuke_AB, ARRAYSIZE(AsuraBuster_A_Zinsuke_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Zinsuke_C, ARRAYSIZE(AsuraBuster_A_Zinsuke_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Zinsuke_AC, ARRAYSIZE(AsuraBuster_A_Zinsuke_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Zinsuke_BC, ARRAYSIZE(AsuraBuster_A_Zinsuke_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Zinsuke_ABC, ARRAYSIZE(AsuraBuster_A_Zinsuke_ABC) },
    { L"Shared", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Zinsuke_Shared, ARRAYSIZE(AsuraBuster_A_Zinsuke_Shared) },
};

const sDescTreeNode AsuraBuster_A_Vebel_COLLECTION[] =
{
    { L"A", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Vebel_A, ARRAYSIZE(AsuraBuster_A_Vebel_A) },
    { L"B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Vebel_B, ARRAYSIZE(AsuraBuster_A_Vebel_B) },
    { L"AB", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Vebel_AB, ARRAYSIZE(AsuraBuster_A_Vebel_AB) },
    { L"C", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Vebel_C, ARRAYSIZE(AsuraBuster_A_Vebel_C) },
    { L"AC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Vebel_AC, ARRAYSIZE(AsuraBuster_A_Vebel_AC) },
    { L"BC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Vebel_BC, ARRAYSIZE(AsuraBuster_A_Vebel_BC) },
    { L"ABC", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Vebel_ABC, ARRAYSIZE(AsuraBuster_A_Vebel_ABC) },
};

const sDescTreeNode AsuraBuster_A_Nanami_COLLECTION[] =
{
    { L"Portraits", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Nanami_PORTRAITS, ARRAYSIZE(AsuraBuster_A_Nanami_PORTRAITS) },
};

const sDescTreeNode AsuraBuster_A_Stages_COLLECTION[] =
{
    { L"Palettes", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Stages, ARRAYSIZE(AsuraBuster_A_Stages) },
};

const sDescTreeNode AsuraBuster_A_UNITS[] =
{
    { L"Yashaou", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Yashaou_COLLECTION, ARRAYSIZE(AsuraBuster_A_Yashaou_COLLECTION) },
    { L"Goat", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Goat_COLLECTION, ARRAYSIZE(AsuraBuster_A_Goat_COLLECTION) },
    { L"Leon", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Leon_COLLECTION, ARRAYSIZE(AsuraBuster_A_Leon_COLLECTION) },
    { L"Chen-Mao", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ChenMao_COLLECTION, ARRAYSIZE(AsuraBuster_A_ChenMao_COLLECTION) },
    { L"Zam-B", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_ZamB_COLLECTION, ARRAYSIZE(AsuraBuster_A_ZamB_COLLECTION) },
    { L"Taros", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Taros_COLLECTION, ARRAYSIZE(AsuraBuster_A_Taros_COLLECTION) },
    { L"Alice", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Alice_COLLECTION, ARRAYSIZE(AsuraBuster_A_Alice_COLLECTION) },
    { L"Rose Mary", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_RoseMary_COLLECTION, ARRAYSIZE(AsuraBuster_A_RoseMary_COLLECTION) },
    { L"Sittara", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Sittara_COLLECTION, ARRAYSIZE(AsuraBuster_A_Sittara_COLLECTION) },
    { L"Rokurouta", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Rokurouta_COLLECTION, ARRAYSIZE(AsuraBuster_A_Rokurouta_COLLECTION) },
    { L"Zinsuke", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Zinsuke_COLLECTION, ARRAYSIZE(AsuraBuster_A_Zinsuke_COLLECTION) },
    { L"Alice!", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_AliceBang_COLLECTION, ARRAYSIZE(AsuraBuster_A_AliceBang_COLLECTION) },
    { L"Vebel", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Vebel_COLLECTION, ARRAYSIZE(AsuraBuster_A_Vebel_COLLECTION) },
    { L"Nanami", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Nanami_COLLECTION, ARRAYSIZE(AsuraBuster_A_Nanami_COLLECTION) },
    { L"Stages", DESC_NODETYPE_TREE, (void*)AsuraBuster_A_Stages_COLLECTION, ARRAYSIZE(AsuraBuster_A_Stages_COLLECTION) },
};
