...
uint32_t CGame_KOF00N_A::rgExtraCountAll[KOF00N_A_NUMUNIT + 1];
uint32_t CGame_KOF00N_A::rgExtraLoc[KOF00N_A_NUMUNIT + 1];

uint32_t CGame_KOF00N_A::m_nTotalPaletteCountForKOF00N = 0;
uint32_t CGame_KOF00N_A::m_nExpectedGameROMSize = 0x400000;  // 4194304 bytes
uint32_t CGame_KOF00N_A::m_nConfirmedROMSize = -1;
...
CGame_KOF00N_A::CGame_KOF00N_A(uint32_t nConfirmedROMSize)
{
...
uint32_t CGame_KOF00N_A::GetExtraCt(uint32_t nUnitId, BOOL fCountVisibleOnly)
{
    return _GetExtraCount(rgExtraCountAll, KOF00N_A_NUMUNIT, nUnitId, KOF00N_A_EXTRA_CUSTOM);
}

uint32_t CGame_KOF00N_A::GetExtraLoc(uint32_t nUnitId)
{
...
sDescTreeNode* CGame_KOF00N_A::InitDescTree()
{
    ...
    LoadExtraFileForGame(EXTRA_FILENAME_KOF00N_A, &KOF00N_A_EXTRA_CUSTOM, KOF00N_A_EXTRALOC, m_nConfirmedROMSize);

    uint16_t nUnitCt = KOF00N_A_NUMUNIT + (GetExtraCt(KOF00N_A_EXTRALOC) ? 1 : 0);
...
sFileRule CGame_KOF00N_A::GetRule(uint32_t nUnitId)
{
...
uint32_t CGame_KOF00N_A::GetCollectionCountForUnit(uint32_t nUnitId)
{
...
uint32_t CGame_KOF00N_A::GetNodeCountForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
...
LPCWSTR CGame_KOF00N_A::GetDescriptionForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetDescriptionForCollection(KOF00N_A_UNITS, KOF00N_A_EXTRALOC, nUnitId, nCollectionId);
}

uint32_t CGame_KOF00N_A::GetPaletteCountForUnit(uint32_t nUnitId)
{
...
const sGame_PaletteDataset* CGame_KOF00N_A::GetPaletteSet(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetPaletteSet(KOF00N_A_UNITS, nUnitId, nCollectionId);
}

const sDescTreeNode* CGame_KOF00N_A::GetNodeFromPaletteId(uint32_t nUnitId, uint32_t nPaletteId, bool fReturnBasicNodesOnly)
{
...
const sGame_PaletteDataset* CGame_KOF00N_A::GetSpecificPalette(uint32_t nUnitId, uint32_t nPaletteId)
{
...
void CGame_KOF00N_A::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
