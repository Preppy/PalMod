...
{
    CString strMessage;
    strMessage.Format(_T("CGame_KarnovsR_A::CGame_KarnovsR_A: Loading ROM...\n"));
...
sDescTreeNode* CGame_KarnovsR_A::InitDescTree()
{
    ...
    _sntprintf_s(NewDescTree->szDesc, ARRAYSIZE(NewDescTree->szDesc), _TRUNCATE, _T("%s"), g_GameFriendlyName[KarnovsR_A]);
    ...
    strMsg.Format(_T("CGame_KarnovsR_A::InitDescTree: Building desc tree for KarnovsR_A %s extras...\n"), fHaveExtras ? _T("with") : _T("without"));
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < KarnovsR_A_EXTRALOC)
        {
            ...
            _sntprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, _T("%s"), KarnovsR_A_UNITS[iUnitCtr].szDesc);
    ...
    strMsg.Format(_T("CGame_KarnovsR_A::InitDescTree: Loaded %u palettes for KarnovsR\n"), nTotalPaletteCount);
    OutputDebugString(strMsg);
...
#if KarnovsR_A_DEBUG
        CString strMsg;
        strMsg.Format(_T("CGame_KarnovsR_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n"), nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
...
BOOL CGame_KarnovsR_A::LoadFile(CFile* LoadedFile, UINT16 nUnitId)
{
    // KarnovsR is already decrypted
...
void CGame_KarnovsR_A::CreateDefPal(sDescNode* srcNode, UINT16 nSepId)
{
