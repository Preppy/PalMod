...
CGame_MVC_A::CGame_MVC_A(uint32_t nConfirmedROMSize)
{
    ...
    m_nSafeCountForThisRom = GetExtraCt(MVC_A_EXTRALOC) + 1337;
...
uint32_t CGame_MVC_A::GetExtraLoc(uint32_t nUnitId)
{
...
sFileRule CGame_MVC_A::GetRule(uint32_t nUnitId)
{
...
uint32_t CGame_MVC_A::GetCollectionCountForUnit(uint32_t nUnitId)
{
    return _GetCollectionCountForUnit(MVC_A_UNITS, rgExtraCountAll, MVC_A_NUMUNIT, MVC_A_EXTRALOC, nUnitId, MVC_A_EXTRA_CUSTOM);
}

uint32_t CGame_MVC_A::GetNodeCountForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
...
LPCWSTR CGame_MVC_A::GetDescriptionForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetDescriptionForCollection(MVC_A_UNITS, MVC_A_EXTRALOC, nUnitId, nCollectionId);
}

uint32_t CGame_MVC_A::GetPaletteCountForUnit(uint32_t nUnitId)
{
    return _GetPaletteCountForUnit(MVC_A_UNITS, rgExtraCountAll, MVC_A_NUMUNIT, MVC_A_EXTRALOC, nUnitId, MVC_A_EXTRA_CUSTOM);
}

const sGame_PaletteDataset* CGame_MVC_A::GetPaletteSet(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetPaletteSet(MVC_A_UNITS, nUnitId, nCollectionId);
}

uint32_t CGame_MVC_A::GetNodeSizeFromPaletteId(uint32_t nUnitId, uint32_t nPaletteId)
{
...
const sDescTreeNode* CGame_MVC_A::GetNodeFromPaletteId(uint32_t nUnitId, uint32_t nPaletteId, bool fReturnBasicNodesOnly)
{
...
const sGame_PaletteDataset* CGame_MVC_A::GetSpecificPalette(uint32_t nUnitId, uint32_t nPaletteId)
{
...
void CGame_MVC_A::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
    ...
    else // MVC_A_EXTRALOC
    {
        ...
        stExtraDef* pCurrDef = &MVC_A_EXTRA_CUSTOM[GetExtraLoc(nUnitId) + nPalId];
...
BOOL CGame_MVC_A::LoadFile(CFile* LoadedFile, uint32_t nUnitId)
{
...
BOOL CGame_MVC_A::UpdatePalImg(int Node01, int Node02, int Node03, int Node04)
{
    ...
    uint32_t nSrcStart = (int)NodeGet->uPalId;
    uint32_t nSrcAmt = 1;
        ...
        if (paletteDataSet)
        {
            ...
            if (paletteDataSet->pPalettePairingInfo)
            {
                if ((NodeGet->uUnitId == indexMVCAssists) && (paletteDataSet->pPalettePairingInfo == &pairMVCDevilotNormal))
                {
                    ...
                    int8_t nPeerPaletteDistance3 = 3;
                    int8_t nPeerPaletteDistance4 = 4;
