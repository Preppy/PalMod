...
sDescTreeNode* CGame_BLEACH_DS::InitDescTree()
{
    ...
    strMsg.Format(L"CGame_BLEACH_DS::InitDescTree: Building desc tree for BLEACH_DS %s extras...\n", fHaveExtras ? L"with" : L"without");
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < BLEACH_DS_EXTRALOC)
        {
            ...
            _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", BLEACH_DS_UNITS[iUnitCtr].szDesc);
    ...
    strMsg.Format(L"CGame_BLEACH_DS::InitDescTree: Loaded %u palettes for BleachDS\n", nTotalPaletteCount);
    OutputDebugString(strMsg);
...
#if BLEACH_DS_DEBUG
        CString strMsg;
        strMsg.Format(L"CGame_BLEACH_DS::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
