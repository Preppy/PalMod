...
{
    CString strMessage;
    strMessage.Format(L"CGame_GEMFIGHTER_A::CGame_GEMFIGHTER_A: Loading ROM...\n");
...
sDescTreeNode* CGame_GEMFIGHTER_A::InitDescTree()
{
    ...
    strMsg.Format(L"CGame_GEMFIGHTER_A::InitDescTree: Building desc tree for GEMFIGHTER_A %s extras...\n", fHaveExtras ? L"with" : L"without");
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < GEMFIGHTER_A_EXTRALOC)
        {
            ...
            _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", GEMFIGHTER_A_UNITS[iUnitCtr].szDesc);
    ...
    strMsg.Format(L"CGame_GEMFIGHTER_A::InitDescTree: Loaded %u palettes for GEMFIGHTER\n", nTotalPaletteCount);
    OutputDebugString(strMsg);
...
#if GEMFIGHTER_A_DEBUG
        CString strMsg;
        strMsg.Format(L"CGame_GEMFIGHTER_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
