...
uint32_t CGameClassByFile::InitDescTreeForFileSet(sDescTreeNode* pNewDescTree)
{
    ...
    const uint32_t nUnitCt = static_cast<uint32_t>(m_psCurrentGameLoadingData->srgLoadingData.size());
    ...
    for (uint32_t iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        for (uint32_t iCollectionCtr = 0; iCollectionCtr < nUnitChildCount; iCollectionCtr++)
        {
            ...
            const uint32_t nListedChildrenCount = GetNodeCountForCollection(iUnitCtr, iCollectionCtr);
...
BOOL CGameClassByFile::SaveFile(CFile* SaveFile, uint32_t nUnitId)
{
    ...
    {
        strMsg.Format(L"CGameClassByFile::SaveFile: No changes for unit %u\n", nUnitId);
    }
    OutputDebugString(strMsg);
...
int CGameClassByFile::PostSetPal(uint32_t nUnitId, uint32_t nPalId)
{
    int nTotalPalettesChanged = 0;

    if (nUnitId < m_psCurrentGameLoadingData->srgLoadingData.size())
    {
        CString strMessage;
        strMessage.Format(L"CGameClassByFile::PostSetPal : Checking additional change requirements for unit %u palette %u.\n", nUnitId, nPalId);
