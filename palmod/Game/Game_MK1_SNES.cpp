...
CGame_MK1_SNES::CGame_MK1_SNES(uint32_t nConfirmedROMSize)
{
...
uint32_t CGame_MK1_SNES::rgExtraCountAll[MK1_SNES_NUMUNIT + 1];
uint32_t CGame_MK1_SNES::rgExtraLoc[MK1_SNES_NUMUNIT + 1];
uint32_t CGame_MK1_SNES::m_nTotalPaletteCountForMK1 = 0;
uint32_t CGame_MK1_SNES::m_nConfirmedROMSize = -1;
...
uint32_t CGame_MK1_SNES::GetExtraCt(uint32_t nUnitId, BOOL fCountVisibleOnly)
{
    return _GetExtraCount(rgExtraCountAll, MK1_SNES_NUMUNIT, nUnitId, MK1_SNES_EXTRA_CUSTOM);
}

uint32_t CGame_MK1_SNES::GetExtraLoc(uint32_t nUnitId)
{
...
sDescTreeNode* CGame_MK1_SNES::InitDescTree()
{
    ...
    LoadExtraFileForGame(EXTRA_FILENAME_MK1_SNES, &MK1_SNES_EXTRA_CUSTOM, MK1_SNES_EXTRALOC, m_nConfirmedROMSize);

    uint16_t nUnitCt = MK1_SNES_NUMUNIT + (GetExtraCt(MK1_SNES_EXTRALOC) ? 1 : 0);
...
sFileRule CGame_MK1_SNES::GetRule(uint32_t nUnitId)
{
...
uint32_t CGame_MK1_SNES::GetCollectionCountForUnit(uint32_t nUnitId)
{
...
uint32_t CGame_MK1_SNES::GetNodeCountForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
...
LPCWSTR CGame_MK1_SNES::GetDescriptionForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetDescriptionForCollection(MK1_SNES_UNITS, MK1_SNES_EXTRALOC, nUnitId, nCollectionId);
}

uint32_t CGame_MK1_SNES::GetPaletteCountForUnit(uint32_t nUnitId)
{
...
const sGame_PaletteDataset* CGame_MK1_SNES::GetPaletteSet(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetPaletteSet(MK1_SNES_UNITS, nUnitId, nCollectionId);
}

const sDescTreeNode* CGame_MK1_SNES::GetNodeFromPaletteId(uint32_t nUnitId, uint32_t nPaletteId, bool fReturnBasicNodesOnly)
{
...
const sGame_PaletteDataset* CGame_MK1_SNES::GetSpecificPalette(uint32_t nUnitId, uint32_t nPaletteId)
{
...
void CGame_MK1_SNES::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
    ...
    else // MK1_SNES_EXTRALOC
    {
        ...
        stExtraDef* pCurrDef = &MK1_SNES_EXTRA_CUSTOM[GetExtraLoc(nUnitId) + nPalId];
