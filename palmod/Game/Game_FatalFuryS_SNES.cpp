...
{
    CString strMessage;
    strMessage.Format(_T("CGame_FatalFuryS_SNES::CGame_FatalFuryS_SNES: Loading ROM...\n"));
...
int CGame_FatalFuryS_SNES::GetExtraCt(UINT16 nUnitId, BOOL bCountVisibleOnly)
{
    ...
    {
        int nDefCtr = 0;
        memset(rgExtraCountAll, 0, ((FatalFuryS_SNES_NUMUNIT + 1) * sizeof(int)));

        stExtraDef* pCurrDef = GetExtraDefForFatalFuryS(0);
        ...
        while (pCurrDef->uUnitN != INVALID_UNIT_VALUE)
        {
            ...
            nDefCtr++;
            pCurrDef = GetExtraDefForFatalFuryS(nDefCtr);
        }
    }
...
int CGame_FatalFuryS_SNES::GetExtraLoc(UINT16 nUnitId)
{
    if (rgExtraLoc[0] == -1)
    {
        ...
        memset(rgExtraLoc, 0, (FatalFuryS_SNES_NUMUNIT + 1) * sizeof(int));
...
sDescTreeNode* CGame_FatalFuryS_SNES::InitDescTree()
{
    ...
    _sntprintf_s(NewDescTree->szDesc, ARRAYSIZE(NewDescTree->szDesc), _TRUNCATE, _T("%s"), g_GameFriendlyName[FatalFuryS_SNES]);
    ...
    CString strMsg;
    bool fHaveExtras = (GetExtraCt(FatalFuryS_SNES_EXTRALOC) > 0);
    strMsg.Format(_T("CGame_FatalFuryS_SNES::InitDescTree: Building desc tree for FatalFuryS_SNES %s extras...\n"), fHaveExtras ? _T("with") : _T("without"));
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < FatalFuryS_SNES_EXTRALOC)
        {
            //Set each description
            _sntprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, _T("%s"), FatalFuryS_SNES_UNITS[iUnitCtr].szDesc);
            UnitNode->ChildNodes = new sDescTreeNode[nUnitChildCount];
            //All children have collection trees
            UnitNode->uChildType = DESC_NODETYPE_TREE;
            UnitNode->uChildAmt = nUnitChildCount;

#if FatalFuryS_SNES_DEBUG
        ...
        if (bUseExtra)
        {
            ...
            CollectionNode = &((sDescTreeNode*)UnitNode->ChildNodes)[(FatalFuryS_SNES_EXTRALOC > iUnitCtr) ? (nUnitChildCount - 1) : 0]; //Extra node
            ...
            for (UINT16 nExtraCtr = 0; nExtraCtr < nExtraCt; nExtraCtr++)
            {
                ...
                stExtraDef* pCurrDef = GetExtraDefForFatalFuryS(nExtraPos + nCurrExtra);

                while (pCurrDef->isInvisible)
                {
                    nCurrExtra++;

                    pCurrDef = GetExtraDefForFatalFuryS(nExtraPos + nCurrExtra);
                }

                _sntprintf_s(ChildNode->szDesc, ARRAYSIZE(ChildNode->szDesc), _TRUNCATE, pCurrDef->szDesc);

                ChildNode->uUnitId = iUnitCtr;
                ChildNode->uPalId = (((FatalFuryS_SNES_EXTRALOC > iUnitCtr) ? 1 : 0) * nUnitChildCount * 2) + nCurrExtra;
    ...
    strMsg.Format(_T("CGame_FatalFuryS_SNES::InitDescTree: Loaded %u palettes for FatalFuryS\n"), nTotalPaletteCount);
    OutputDebugString(strMsg);
...
UINT16 CGame_FatalFuryS_SNES::GetCollectionCountForUnit(UINT16 nUnitId)
{
    ...
    {
        return FatalFuryS_SNES_UNITS[nUnitId].uChildAmt;
    }
}
...
UINT16 CGame_FatalFuryS_SNES::GetNodeCountForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    {
        const sDescTreeNode* pCollectionNode = (const sDescTreeNode*)(FatalFuryS_SNES_UNITS[nUnitId].ChildNodes);
...
LPCTSTR CGame_FatalFuryS_SNES::GetDescriptionForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    {
        const sDescTreeNode* pCollection = (const sDescTreeNode*)FatalFuryS_SNES_UNITS[nUnitId].ChildNodes;
...
UINT16 CGame_FatalFuryS_SNES::GetPaletteCountForUnit(UINT16 nUnitId)
{
    ...
    else
    {
        ...
        const sDescTreeNode* pCompleteROMTree = FatalFuryS_SNES_UNITS;
...
#if FatalFuryS_SNES_DEBUG
        CString strMsg;
        strMsg.Format(_T("CGame_FatalFuryS_SNES::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n"), nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
...
const sGame_PaletteDataset* CGame_FatalFuryS_SNES::GetPaletteSet(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    const sDescTreeNode* pCurrentSet = (const sDescTreeNode*)FatalFuryS_SNES_UNITS[nUnitId].ChildNodes;
...
const sDescTreeNode* CGame_FatalFuryS_SNES::GetNodeFromPaletteId(UINT16 nUnitId, UINT16 nPaletteId, bool fReturnBasicNodesOnly)
{
    ...
    for (UINT16 nCollectionIndex = 0; nCollectionIndex < nTotalCollections; nCollectionIndex++)
    {
        ...
        {
            const sDescTreeNode* pCollectionNodeToCheck = (const sDescTreeNode*)(FatalFuryS_SNES_UNITS[nUnitId].ChildNodes);
...
BOOL CGame_FatalFuryS_SNES::UpdatePalImg(int Node01, int Node02, int Node03, int Node04)
{
    ...
    if (NodeGet->uUnitId != FatalFuryS_SNES_EXTRALOC)
    {
