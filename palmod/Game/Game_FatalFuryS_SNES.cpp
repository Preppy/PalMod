...
{
    CString strMessage;
    strMessage.Format(L"CGame_FatalFuryS_SNES::CGame_FatalFuryS_SNES: Loading ROM...\n");
...
sDescTreeNode* CGame_FatalFuryS_SNES::InitDescTree()
{
    ...
    strMsg.Format(L"CGame_FatalFuryS_SNES::InitDescTree: Building desc tree for FatalFuryS_SNES %s extras...\n", fHaveExtras ? L"with" : L"without");
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < FatalFuryS_SNES_EXTRALOC)
        {
            ...
            _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", FatalFuryS_SNES_UNITS[iUnitCtr].szDesc);
    ...
    strMsg.Format(L"CGame_FatalFuryS_SNES::InitDescTree: Loaded %u palettes for FatalFuryS\n", nTotalPaletteCount);
    OutputDebugString(strMsg);
...
#if FatalFuryS_SNES_DEBUG
        CString strMsg;
        strMsg.Format(L"CGame_FatalFuryS_SNES::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
