...
BOOL CGame_CFJ_S::LoadFile(CFile* LoadedFile, uint32_t nUnitId /* = 0 */)
{
    ...
    for (uint32_t nUnitCtr = 0; nUnitCtr < m_nUnitAmt; nUnitCtr++)
    {
        ...
        for (uint32_t nPalCtr = 0; nPalCtr < nPalAmt; nPalCtr++)
        {
            ...
            {
                LoadedFile->Seek(m_nCurrentPaletteROMLocation + (nCurrentPaletteOffset * 2), CFile::begin);
                LoadedFile->Read(&pColors[nCurrentPaletteOffset], min(nColorsToRead, 8) * 2);
                nColorsToRead -= min(nColorsToRead, 8);

                if (nColorsToRead)
                {
                    LoadedFile->Seek(m_nCurrentPaletteROMLocation + (nCurrentPaletteOffset * 2) + (16 * 2), CFile::begin);
                ...
                if (nColorsToRead)
                {
                    LoadedFile->Seek(m_nCurrentPaletteROMLocation + (nCurrentPaletteOffset * 2) + (8 * 2), CFile::begin);
                ...
                if (nColorsToRead)
                {
                    LoadedFile->Seek(m_nCurrentPaletteROMLocation + (nCurrentPaletteOffset * 2) + (24 * 2), CFile::begin);
