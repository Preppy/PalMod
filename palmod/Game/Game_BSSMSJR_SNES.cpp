...
CGame_BSSMSJR_SNES::~CGame_BSSMSJR_SNES()
{
    safe_delete_array(CGame_BSSMSJR_SNES::BSSMSJR_SNES_EXTRA_CUSTOM);
    ClearDataBuffer();
...
uint32_t CGame_BSSMSJR_SNES::GetExtraCt(uint32_t nUnitId, BOOL fCountVisibleOnly)
{
    return _GetExtraCount(rgExtraCountAll, BSSMSJR_SNES_NUMUNIT, nUnitId, BSSMSJR_SNES_EXTRA_CUSTOM);
}

uint32_t CGame_BSSMSJR_SNES::GetExtraLoc(uint32_t nUnitId)
{
...
sFileRule CGame_BSSMSJR_SNES::GetRule(uint32_t nUnitId)
{
...
uint32_t CGame_BSSMSJR_SNES::GetCollectionCountForUnit(uint32_t nUnitId)
{
...
uint32_t CGame_BSSMSJR_SNES::GetNodeCountForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
...
LPCWSTR CGame_BSSMSJR_SNES::GetDescriptionForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetDescriptionForCollection(BSSMSJR_SNES_UNITS, BSSMSJR_SNES_EXTRALOC, nUnitId, nCollectionId);
}

uint32_t CGame_BSSMSJR_SNES::GetPaletteCountForUnit(uint32_t nUnitId)
{
...
const sGame_PaletteDataset* CGame_BSSMSJR_SNES::GetPaletteSet(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetPaletteSet(BSSMSJR_SNES_UNITS, nUnitId, nCollectionId);
}

const sDescTreeNode* CGame_BSSMSJR_SNES::GetNodeFromPaletteId(uint32_t nUnitId, uint32_t nPaletteId, bool fReturnBasicNodesOnly)
{
...
const sGame_PaletteDataset* CGame_BSSMSJR_SNES::GetSpecificPalette(uint32_t nUnitId, uint32_t nPaletteId)
{
...
void CGame_BSSMSJR_SNES::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
    ...
    else // BSSMSJR_SNES_EXTRALOC
    {
        ...
        stExtraDef* pCurrDef = &BSSMSJR_SNES_EXTRA_CUSTOM[GetExtraLoc(nUnitId) + nPalId];
