...
uint32_t CGame_RANMAHB_SNES::GetExtraLoc(uint32_t nUnitId)
{
...
sFileRule CGame_RANMAHB_SNES::GetRule(uint32_t nUnitId)
{
...
uint32_t CGame_RANMAHB_SNES::GetCollectionCountForUnit(uint32_t nUnitId)
{
...
uint32_t CGame_RANMAHB_SNES::GetNodeCountForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
...
LPCWSTR CGame_RANMAHB_SNES::GetDescriptionForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetDescriptionForCollection(RANMAHB_SNES_UNITS, RANMAHB_SNES_EXTRALOC, nUnitId, nCollectionId);
}

uint32_t CGame_RANMAHB_SNES::GetPaletteCountForUnit(uint32_t nUnitId)
{
...
const sGame_PaletteDataset* CGame_RANMAHB_SNES::GetPaletteSet(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetPaletteSet(RANMAHB_SNES_UNITS, nUnitId, nCollectionId);
}

const sDescTreeNode* CGame_RANMAHB_SNES::GetNodeFromPaletteId(uint32_t nUnitId, uint32_t nPaletteId, bool fReturnBasicNodesOnly)
{
...
const sGame_PaletteDataset* CGame_RANMAHB_SNES::GetSpecificPalette(uint32_t nUnitId, uint32_t nPaletteId)
{
...
void CGame_RANMAHB_SNES::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
    ...
    else // RANMAHB_SNES_EXTRALOC
    {
        ...
        stExtraDef* pCurrDef = &RANMAHB_SNES_EXTRA_CUSTOM[GetExtraLoc(nUnitId) + nPalId];
