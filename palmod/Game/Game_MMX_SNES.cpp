...
uint32_t CGame_MMX_SNES::GetExtraCt(uint32_t nUnitId, BOOL fCountVisibleOnly)
{
    return _GetExtraCount(rgExtraCountAll, MMX_SNES_NUMUNIT, nUnitId, MMX_SNES_EXTRA_CUSTOM);
}

uint32_t CGame_MMX_SNES::GetExtraLoc(uint32_t nUnitId)
{
...
sFileRule CGame_MMX_SNES::GetRule(uint32_t nUnitId)
{
...
uint32_t CGame_MMX_SNES::GetCollectionCountForUnit(uint32_t nUnitId)
{
...
uint32_t CGame_MMX_SNES::GetNodeCountForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
...
LPCWSTR CGame_MMX_SNES::GetDescriptionForCollection(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetDescriptionForCollection(MMX_SNES_UNITS, MMX_SNES_EXTRALOC, nUnitId, nCollectionId);
}

uint32_t CGame_MMX_SNES::GetPaletteCountForUnit(uint32_t nUnitId)
{
...
const sGame_PaletteDataset* CGame_MMX_SNES::GetPaletteSet(uint32_t nUnitId, uint32_t nCollectionId)
{
    return _GetPaletteSet(MMX_SNES_UNITS, nUnitId, nCollectionId);
}

const sDescTreeNode* CGame_MMX_SNES::GetNodeFromPaletteId(uint32_t nUnitId, uint32_t nPaletteId, bool fReturnBasicNodesOnly)
{
...
const sGame_PaletteDataset* CGame_MMX_SNES::GetSpecificPalette(uint32_t nUnitId, uint32_t nPaletteId)
{
...
void CGame_MMX_SNES::LoadSpecificPaletteData(uint32_t nUnitId, uint32_t nPalId)
{
