...
sDescTreeNode* CGame_NeoBomberman_A::InitDescTree()
{
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr != NeoBomberman_A_EXTRALOC)
        {
            ...
            _snwprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", NeoBomberman_A_UNITS[iUnitCtr].szDesc);
...
#if NeoBomberman_A_DEBUG
        CString strMsg;
        strMsg.Format(L"CGame_NeoBomberman_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
