...
{
    CString strMessage;
    strMessage.Format(_T("CGame_RODSM2_A::CGame_RODSM2_A: Loading ROM...\n"));
...
UINT32 CGame_RODSM2_A::GetKnownCRC32DatasetsForGame(const sCRC32ValueSet** ppKnownROMSet, bool* pfNeedToValidateCRCs)
{
    ...
    {
        { _T("Ring of Destruction: Slam Masters 2"), _T("smbe.05b"),  0x0, 0 },
    };
...
int CGame_RODSM2_A::GetExtraCt(UINT16 nUnitId, BOOL bCountVisibleOnly)
{
    ...
    {
        int nDefCtr = 0;
        memset(rgExtraCountAll, 0, ((RODSM2_A_NUMUNITS + 1) * sizeof(int)));

        stExtraDef* pCurrDef = GetExtraDefForSM2(0);
        ...
        while (pCurrDef->uUnitN != INVALID_UNIT_VALUE)
        {
            ...
            nDefCtr++;
            pCurrDef = GetExtraDefForSM2(nDefCtr);
        }
    }
...
int CGame_RODSM2_A::GetExtraLoc(UINT16 nUnitId)
{
    if (rgExtraLoc[0] == -1)
    {
        ...
        memset(rgExtraLoc, 0, (RODSM2_A_NUMUNITS + 1) * sizeof(int));
...
sDescTreeNode* CGame_RODSM2_A::InitDescTree()
{
    ...
    _sntprintf_s(NewDescTree->szDesc, ARRAYSIZE(NewDescTree->szDesc), _TRUNCATE, _T("%s"), g_GameFriendlyName[RODSM2_A]);
    ...
    CString strMsg;
    bool fHaveExtras = (GetExtraCt(RODSM2_A_EXTRALOC) > 0);
    strMsg.Format(_T("CGame_RODSM2_A::InitDescTree: Building desc tree for RODSM2_A %s extras...\n"), fHaveExtras ? _T("with") : _T("without"));
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < RODSM2_A_EXTRALOC)
        {
            //Set each description
            _sntprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, _T("%s"), RODSM2_A_UNITS[iUnitCtr].szDesc);
        ...
        if (bUseExtra)
        {
            ...
            CollectionNode = &((sDescTreeNode*)UnitNode->ChildNodes)[(RODSM2_A_EXTRALOC > iUnitCtr) ? (nUnitChildCount - 1) : 0]; //Extra node
            ...
            for (UINT16 nExtraCtr = 0; nExtraCtr < nExtraCt; nExtraCtr++)
            {
                ...
                stExtraDef* pCurrDef = GetExtraDefForSM2(nExtraPos + nCurrExtra);

                while (pCurrDef->isInvisible)
                {
                    nCurrExtra++;

                    pCurrDef = GetExtraDefForSM2(nExtraPos + nCurrExtra);
                }

                _sntprintf_s(ChildNode->szDesc, ARRAYSIZE(ChildNode->szDesc), _TRUNCATE, pCurrDef->szDesc);

                ChildNode->uUnitId = iUnitCtr;
                ChildNode->uPalId = (((RODSM2_A_EXTRALOC > iUnitCtr) ? 1 : 0) * nUnitChildCount * 2) + nCurrExtra;
    ...
    strMsg.Format(_T("CGame_RODSM2_A::InitDescTree: Loaded %u palettes for GEMFIGHTER\n"), nTotalPaletteCount);
    OutputDebugString(strMsg);

    m_nTotalPaletteCountForSM2 = nTotalPaletteCount;
...
sFileRule CGame_RODSM2_A::GetRule(UINT16 nUnitId)
{
    ...
    _sntprintf_s(NewFileRule.szFileName, ARRAYSIZE(NewFileRule.szFileName), _TRUNCATE, _T("smbe.05b"));
...
UINT16 CGame_RODSM2_A::GetCollectionCountForUnit(UINT16 nUnitId)
{
    ...
    {
        return RODSM2_A_UNITS[nUnitId].uChildAmt;
    }
}
...
UINT16 CGame_RODSM2_A::GetNodeCountForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    {
        const sDescTreeNode* pCollectionNode = (const sDescTreeNode*)(RODSM2_A_UNITS[nUnitId].ChildNodes);
...
LPCWSTR CGame_RODSM2_A::GetDescriptionForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    {
        const sDescTreeNode* pCollection = (const sDescTreeNode*)RODSM2_A_UNITS[nUnitId].ChildNodes;
...
UINT16 CGame_RODSM2_A::GetPaletteCountForUnit(UINT16 nUnitId)
{
    ...
    else
    {
        ...
        const sDescTreeNode* pCompleteROMTree = RODSM2_A_UNITS;
...
#if RODSM2_A_DEBUG
        CString strMsg;
        strMsg.Format(_T("CGame_RODSM2_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n"), nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
...
const sGame_PaletteDataset* CGame_RODSM2_A::GetPaletteSet(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    const sDescTreeNode* pCurrentSet = (const sDescTreeNode*)RODSM2_A_UNITS[nUnitId].ChildNodes;
...
const sDescTreeNode* CGame_RODSM2_A::GetNodeFromPaletteId(UINT16 nUnitId, UINT16 nPaletteId, bool fReturnBasicNodesOnly)
{
    ...
    for (UINT16 nCollectionIndex = 0; nCollectionIndex < nTotalCollections; nCollectionIndex++)
    {
        ...
        {
            const sDescTreeNode* pCollectionNodeToCheck = (const sDescTreeNode*)(RODSM2_A_UNITS[nUnitId].ChildNodes);
...
BOOL CGame_RODSM2_A::UpdatePalImg(int Node01, int Node02, int Node03, int Node04)
{
    ...
    if (NodeGet->uUnitId != RODSM2_A_EXTRALOC)
    {
