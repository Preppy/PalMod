...
{
    CString strMessage;
    strMessage.Format(L"CGame_Kizuna_A::CGame_Kizuna_A: Loading ROM...\n");
...
int CGame_Kizuna_A::GetExtraCt(UINT16 nUnitId, BOOL bCountVisibleOnly)
{
    ...
    {
        int nDefCtr = 0;
        memset(rgExtraCountAll, 0, ((Kizuna_A_NUMUNIT + 1) * sizeof(int)));

        stExtraDef* pCurrDef = GetExtraDefForKizuna(0);
        ...
        while (pCurrDef->uUnitN != INVALID_UNIT_VALUE)
        {
            ...
            nDefCtr++;
            pCurrDef = GetExtraDefForKizuna(nDefCtr);
        }
    }
...
int CGame_Kizuna_A::GetExtraLoc(UINT16 nUnitId)
{
    if (rgExtraLoc[0] == -1)
    {
        ...
        memset(rgExtraLoc, 0, (Kizuna_A_NUMUNIT + 1) * sizeof(int));
...
sDescTreeNode* CGame_Kizuna_A::InitDescTree()
{
    ...
    _sntprintf_s(NewDescTree->szDesc, ARRAYSIZE(NewDescTree->szDesc), _TRUNCATE, L"%s", g_GameFriendlyName[KIZUNA_A]);
    ...
    CString strMsg;
    bool fHaveExtras = (GetExtraCt(Kizuna_A_EXTRALOC) > 0);
    strMsg.Format(L"CGame_Kizuna_A::InitDescTree: Building desc tree for Kizuna_A %s extras...\n", fHaveExtras ? L"with" : L"without");
    ...
    for (UINT16 iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
    {
        ...
        if (iUnitCtr < Kizuna_A_EXTRALOC)
        {
            //Set each description
            _sntprintf_s(UnitNode->szDesc, ARRAYSIZE(UnitNode->szDesc), _TRUNCATE, L"%s", Kizuna_A_UNITS[iUnitCtr].szDesc);
        ...
        if (bUseExtra)
        {
            ...
            CollectionNode = &((sDescTreeNode*)UnitNode->ChildNodes)[(Kizuna_A_EXTRALOC > iUnitCtr) ? (nUnitChildCount - 1) : 0]; //Extra node
            ...
            for (UINT16 nExtraCtr = 0; nExtraCtr < nExtraCt; nExtraCtr++)
            {
                ...
                stExtraDef* pCurrDef = GetExtraDefForKizuna(nExtraPos + nCurrExtra);

                while (pCurrDef->isInvisible)
                {
                    nCurrExtra++;

                    pCurrDef = GetExtraDefForKizuna(nExtraPos + nCurrExtra);
                }

                _sntprintf_s(ChildNode->szDesc, ARRAYSIZE(ChildNode->szDesc), _TRUNCATE, pCurrDef->szDesc);

                ChildNode->uUnitId = iUnitCtr;
                ChildNode->uPalId = (((Kizuna_A_EXTRALOC > iUnitCtr) ? 1 : 0) * nUnitChildCount * 2) + nCurrExtra;
    ...
    strMsg.Format(L"CGame_Kizuna_A::InitDescTree: Loaded %u palettes for Kizuna\n", nTotalPaletteCount);
    OutputDebugString(strMsg);

    m_nTotalPaletteCountForKizuna = nTotalPaletteCount;
...
UINT16 CGame_Kizuna_A::GetCollectionCountForUnit(UINT16 nUnitId)
{
    ...
    {
        return Kizuna_A_UNITS[nUnitId].uChildAmt;
    }
}
...
UINT16 CGame_Kizuna_A::GetNodeCountForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    {
        const sDescTreeNode* pCollectionNode = (const sDescTreeNode*)(Kizuna_A_UNITS[nUnitId].ChildNodes);
...
LPCWSTR CGame_Kizuna_A::GetDescriptionForCollection(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    {
        const sDescTreeNode* pCollection = (const sDescTreeNode*)Kizuna_A_UNITS[nUnitId].ChildNodes;
...
UINT16 CGame_Kizuna_A::GetPaletteCountForUnit(UINT16 nUnitId)
{
    ...
    else
    {
        ...
        const sDescTreeNode* pCompleteROMTree = Kizuna_A_UNITS;
...
#if Kizuna_A_DEBUG
        CString strMsg;
        strMsg.Format(L"CGame_Kizuna_A::GetPaletteCountForUnit: %u for unit %u which has %u collections.\n", nCompleteCount, nUnitId, nCollectionCount);
        OutputDebugString(strMsg);
#endif
...
const sGame_PaletteDataset* CGame_Kizuna_A::GetPaletteSet(UINT16 nUnitId, UINT16 nCollectionId)
{
    ...
    const sDescTreeNode* pCurrentSet = (const sDescTreeNode*)Kizuna_A_UNITS[nUnitId].ChildNodes;
...
const sDescTreeNode* CGame_Kizuna_A::GetNodeFromPaletteId(UINT16 nUnitId, UINT16 nPaletteId, bool fReturnBasicNodesOnly)
{
    ...
    for (UINT16 nCollectionIndex = 0; nCollectionIndex < nTotalCollections; nCollectionIndex++)
    {
        ...
        {
            const sDescTreeNode* pCollectionNodeToCheck = (const sDescTreeNode*)(Kizuna_A_UNITS[nUnitId].ChildNodes);
...
BOOL CGame_Kizuna_A::UpdatePalImg(int Node01, int Node02, int Node03, int Node04)
{
    ...
    if (NodeGet->uUnitId != Kizuna_A_EXTRALOC)
    {
