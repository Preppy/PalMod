...
int CGame_SFIII3_A::GetExtraCt(int nUnitId, BOOL bVisible)
{
	...
	if(rgExtraCt[0] == -1)
	{
		...
		stExtraDef * pCurrDef = SFIII3_A_EXTRA_CUSTOM[0];
		...
		while(pCurrDef->uUnitN != 0xFF )
		{
			...
			nDefCtr++;
			pCurrDef = SFIII3_A_EXTRA_CUSTOM[nDefCtr];
	...
	for(int iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
	{
		...
		if(bUseExtra)
		{
			...
			for(int nExtraCtr = 0; nExtraCtr < nExtraCt; nExtraCtr++)
			{
				...
				pCurrDef = SFIII3_A_EXTRA_CUSTOM[nExtraPos + nCurrExtra];
...
void CGame_SFIII3_A::GetPalOffsSz(int nUnitId, int nPalId)
{
	...
	{
		stExtraDef * pCurrDef = SFIII3_A_EXTRA_CUSTOM[GetExtraLoc(nUnitId) + (nPalId - nExtraPos)];
