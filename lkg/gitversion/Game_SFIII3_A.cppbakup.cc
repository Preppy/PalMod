...
int CGame_SFIII3_A::GetExtraCt(int nUnitId, BOOL bVisible)
{
	...
	if(rgExtraCt[0] == -1)
	{
		...
		stExtraDef * pCurrDef = const_cast<stExtraDef *>(&SFIII3_A_EXTRA[0]);
		...
		while(pCurrDef->uUnitN != 0xFF )
		{
			...
			nDefCtr++;
			pCurrDef = const_cast<stExtraDef *>(&SFIII3_A_EXTRA[nDefCtr]);
	...
	for(int iUnitCtr = 0; iUnitCtr < nUnitCt; iUnitCtr++)
	{
		...
		if(bUseExtra)
		{
			...
			for(int nExtraCtr = 0; nExtraCtr < nExtraCt; nExtraCtr++)
			{
				...
				pCurrDef = const_cast<stExtraDef *>(&SFIII3_A_EXTRA[nExtraPos + nCurrExtra]);
...
void CGame_SFIII3_A::GetPalOffsSz(int nUnitId, int nPalId)
{
	...
	if(!bUseExtra)
	{
		...
		{
			int nOffStart = (nPalId - nPortPos ? 0x764E00 : 0x76AE00);

			nOffset = nOffStart + ((nPalId - nPortPos) * 0x80);
			nPalSz = 0x80;
		}
	...
	{
		stExtraDef * pCurrDef = const_cast<stExtraDef *>(&SFIII3_A_EXTRA[GetExtraLoc(nUnitId) + (nPalId - nExtraPos)]);
				...
				case 0: //Tengu Stone (1)
					{
						...
						//BasePalGroup.AddSep(0, "wat", 16, 16); 
...
void CGame_SFIII3_A::UpdatePalData()
{
	for(int nPalCtr = 0; nPalCtr < MAX_PAL; nPalCtr++)
	{
		...
		if(srcDef->bAvail )
		{
			...
			if(srcDef->uPalId >= nBasicAmt && srcDef->uPalId  < nBasicAmt*2)//Portrait
			{
